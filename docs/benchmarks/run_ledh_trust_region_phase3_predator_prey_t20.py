#!/usr/bin/env python3
"""LEDH Trust-Region Phase 3: Tuning and Safety Evaluation - Predator-Prey T20.
...
DUAL_CAP_ARTIFACT = Path("docs/benchmarks/artifacts/genut_four_model_leaderboard_rerun_20260816/single_predator_prey_T20_dual_cap/result.json")
...
def _genut_design(dim: int, num_particles: int = 1008) -> tf.Tensor:
    ...
    if dim >= 18:
        return cubature_design(dim=dim, num_particles=num_particles)
    return replicate_positive_genut(gaussian_genut_design(dim=dim), num_particles=num_particles)


def _build_predator_prey_target() -> dict[str, Any]:
    """Build Predator-Prey T20 target from base module."""
    targets = base._build_targets()
    predator_prey = targets["predator_prey_T20"]

    # Add horizon field for evaluator
    horizon = int(predator_prey["observations"].shape[0])
    predator_prey["horizon"] = horizon
...
def run(output_root: Path) -> dict[str, Any]:
    ...
    with tf.device("/CPU:0"):
        target = _build_predator_prey_target()
    ...
    if selected:
        ...
        objectives = [row["value"] for row in selected["rows"]]
        print(f"  Mean objective: {statistics.mean(objectives):.3f}")
        print(f"  Min objective: {min(objectives):.3f}")
        print(f"  Max objective: {max(objectives):.3f}")

    print(f"\nResult written to: {result_json.relative_to(ROOT)}")
    print(f"{'='*80}\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("docs/benchmarks/artifacts") / f"ledh_trust_region_predator_prey_t20_{time.strftime('%Y%m%d')}",
