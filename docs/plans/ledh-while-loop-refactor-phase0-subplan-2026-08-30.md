...
2. Installing and configuring coverage tooling (pytest-cov in .coveragerc)
...
5. Verifying the refactor contract (mathematical, API, implementation constraints) established in the master program

**Success Criteria**:
- All parity tests in `test_ledh_canonical_batch_fused.py` pass (3/3)
- All parity tests in `test_ledh_canonical_batch.py` pass (3/3)
- Coverage tooling installed via `pip install pytest-cov` (pre-approved)
- Coverage configuration in `.coveragerc` with hard threshold and expanded scope
- Fresh baseline coverage of `ledh_canonical_batch_fused_tf.py` and `ledh_canonical_batch_tf.py` measured and recorded
- Phase 0 result document written

**Pre-conditions**: ONE upfront campaign authorization granted (per master program)

**Post-conditions**: Phase 1 may begin (single-direction while loop conversion)
...
**Arithmetic note** (Codex correction, 2026-09-01): 21 + 83 + 8 = 112, not 106. Fresh baseline measurement required after API-drift repair.
...
- No `.coveragerc`, `pyproject.toml`, `setup.cfg`, or `tox.ini`
...
**Repair Action** (Codex correction D3, 2026-09-01): 
1. Install `pytest-cov` in tftwogpu env: `pip install pytest-cov` (pre-approved)
2. Create `.coveragerc` file (coverage.py searches for this; pytest.ini is NOT searched):
   ```ini
   [run]
   source = bayesfilter
   omit = 
...
3. Hard coverage threshold: 80% line coverage minimum for `ledh_canonical_batch_fused_tf.py` and `ledh_canonical_batch_tf.py` combined
4. Expanded scope: measure both fused and non-fused kernels (test matrix covers both)
5. Test matrix dimensions:
   - Kernel variant: fused / non-fused
   - Batch size: B=1 / B>1
   - Input shapes: minimal / realistic (horizon=10 / horizon=50)
   - Substeps: default / non-default
...
3. Measure baseline coverage:
   ...
   CUDA_VISIBLE_DEVICES=-1 python -m pytest \
     ...
     --cov=bayesfilter.highdim.ledh_canonical_batch_tf \
...
**Changes** (4 substitutions, NOT 5 — Codex correction 2026-09-01):
```python
# Line 119 (inside test_fused_batch_size_one_parity)
...
**Line 109 must NOT change** — that call is `canonical_value_and_analytical_score(...)` (the single-cloud authority), whose parameter is correctly `flow_substeps=`.

**File 2**: `tests/highdim/test_ledh_canonical_batch.py`

**Changes** (5 substitutions, NOT 1 — Codex correction 2026-09-01):
```python
# Line 85 (inside test_batch_lane_value_score_parity)
...
**Total: 10 substitutions across 3 files** (4 fused + 5 non-fused + 1 kernel), corrected from original audit counts.
...
OLD: substeps=substeps
NEW: flow_substeps=substeps
```
...
**Fresh baseline required**: After API-drift repair, capture new test counts to correct the 21+83+8≠106 arithmetic error.

### Repair 2: Install Coverage Tooling

**Step 1**: Install pytest-cov (pre-approved)
```bash
pip install pytest-cov
```

**Step 2**: Create `.coveragerc` file (Codex correction D3, 2026-09-01)

Coverage.py searches `.coveragerc`, `pyproject.toml`, `setup.cfg`, or `tox.ini`. It does NOT search `pytest.ini`. Create `.coveragerc`:
...
**Hard threshold**: 80% line coverage minimum for `ledh_canonical_batch_fused_tf.py` and `ledh_canonical_batch_tf.py` combined.
...
**Record**: 
- Combined line coverage % for both kernels (minimum threshold: 80%)
- Missing lines (recorded in Phase 0 result document)
...
**After Repair 1+2 complete**, run the full canonical subset to capture fresh baseline:
```bash
cd /home/chakwong/BayesFilter/.claude/worktrees/ledh-canonical-rebuild
CUDA_VISIBLE_DEVICES=-1 python -m pytest tests/highdim/ \
  --ignore=tests/highdim/test_genut_shape_lm_tf.py \
...
**Expected**:
- Previously passing: 83 tests (unchanged from audit)
- API-drift repairs: 6 tests (formerly failed, now passing)
- Out of scope failures remain: 14 (phase5 fixtures) + 1 (phase1 leaderboard) + 8 errors (phase1 leaderboard)
- **Fresh total after repair**: exact counts to be measured and recorded

**Record**: Exact pass/fail/error counts in the Phase 0 result document
