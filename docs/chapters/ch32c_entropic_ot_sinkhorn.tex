...
unscented transform (GenUT).  It matches mean and covariance and uses the
remaining degrees of freedom to match each coordinate's standardized third and
fourth central moments.  This is attractive for non-Gaussian nonlinear models,
but it is not a full higher-order empirical distribution: for correlated states
only selected diagonal moments in a chosen square-root coordinate system are
matched.
...
finite higher-moment correction to the carried cloud.  GenUT supplies the
motivation and the selected diagonal moment targets, but this is not the
classical GenUT quadrature algorithm.  The existing Contract~E route remains
unchanged when the correction iteration count is zero.
...
axis rule of Proposition~\ref{prop:bf-eot-genut-axis}.  Mixed third- and
fourth-order tensors are deliberately not formed.
...
\end{equation}
For damping \(\delta>0\) and strength \(\rho\geq0\), the finite update is
\begin{align}
 c_a&=\rho(J_a^\top J_a+\delta I_2)^{-1}J_a^\top d_a,
 \label{eq:bf-eot-hm-coefficients}\\
 v_{ia}&=u_{ia}+c_{a,1}q_{3,ia}+c_{a,2}q_{4,ia}.
 \label{eq:bf-eot-hm-correction}
\end{align}
The corrected \(v_i\) are re-centered and re-whitened before the next finite
iteration.  After the declared number \(S\) of iterations, the standardized
rows \(u_i^H\) are mapped back by
\begin{equation}
...
Thus the directions preserve mean and variance to first order, while the
subsequent finite rewhitening enforces those two moments exactly.
\end{proof}
...
The update \eqref{eq:bf-eot-hm-coefficients} is a damped local
Gauss--Newton step, not an exact projection.  Rewhitening can change the third
and fourth moments, and the nonlinear moment-feasibility problem can be
infeasible or have multiple solutions.  The implementation therefore reports
the post-rewhitening residuals and makes no exact higher-moment matching claim.

\subsection{Complete algorithm and score statement}

One time step of the candidate is the following fixed composition:
\begin{enumerate}[label=(\arabic*)]
 \item propagate every particle row by \eqref{eq:bf-eot-hm-transition};
 \item compute the likelihood increment and normalized weights using
       \eqref{eq:bf-eot-hm-likelihood}--\eqref{eq:bf-eot-hm-weights};
 \item run the declared finite Sinkhorn and balance iterations in
       \eqref{eq:bf-eot-hm-sinkhorn}, form the row quotient
       \eqref{eq:bf-eot-hm-barycentric}, and check its marginal diagnostics;
 \item apply the residual injection \eqref{eq:bf-eot-hm-residual} and the
       Contract~E affine restoration \eqref{eq:bf-eot-hm-contract-e};
 \item if \(S>0\), compute \eqref{eq:bf-eot-hm-target-moments}, apply
       \eqref{eq:bf-eot-hm-coefficients}--\eqref{eq:bf-eot-hm-correction}
       exactly \(S\) times with rewhitening after each iteration, then map back
       by \eqref{eq:bf-eot-hm-map-back};
 \item carry \(x_i^H\), or \(x_i^E\) when \(S=0\), with uniform weights to
       the next time step.
\end{enumerate}
The particle count must satisfy the chosen residual-design rule: \(N\) is
divisible by \(2d\) for replicated cubature, and a positive GenUT design must
have exactly representable replication masses.  These requirements concern
the fixed residual design; the adaptive correction itself keeps all \(N\)
particle rows and uses \(O(Nd+d^2)\) storage beyond the OT workspace.
...
balance counts, \(\epsilon\), \(\lambda\), \(\rho\), \(\delta\), and \(S\).
Assume every required covariance is strictly positive definite, every row mass
is positive, the maximum branch in the cost scale is fixed locally, and every
validity condition holds.  If the recursive tangent includes every operation
...
Cholesky factors, triangular solves, \(2\times2\) damped solves, and finite
rewhitening maps are differentiable.  The ordinary chain rule therefore
...
directions \(q_3,q_4\), Jacobians \(J_a\), damped coefficients \(c_a\), and
both standardization maps.  Induction over time then shows that the carried
...
identity, exact post-correction empirical mean and covariance, and the total
derivative of one fixed finite program.  They do not establish global
