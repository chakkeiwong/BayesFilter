...
def test_phase6_generalized_sv_tiny_forward_scalar_artifact_replays_without_admission() -> None:
    ...
    assert artifact["normalization_checks"]["source_route_target_policy"] is True
    assert artifact["normalization_checks"]["flow_surface_is_proposal_only"] is True


def test_phase6_generalized_sv_tiny_artifact_is_rejected_as_admission() -> None:
    artifact = json.loads(ARTIFACT_PATH.read_text(encoding="utf-8"))
...
def test_phase6_generalized_sv_tiny_artifact_nonclaims_exclude_score_and_substitutions() -> None:
    artifact = json.loads(ARTIFACT_PATH.read_text(encoding="utf-8"))
    ...
    assert "not full generalized-SV row admission" in nonclaims
