...
parser.add_argument("--mode", choices=("prefix", "cached-gradient", "covariance", "eigensystem", "refinement", "full-filter", "factor-filter", "candidate", "preflight"), default="prefix")
...
with (CAMPAIGN / ".coordinator.lock").open("a+b") as lock:
    ...
    if args.mode == "candidate":
        ...
        start = json.loads((CAMPAIGN / "campaign-start.json").read_text())
        ledger = CampaignBudgetLedger(CAMPAIGN / "campaign_budget_ledger.json")
        original = json.loads((CAMPAIGN / "launches/runtime-c48f1f0535/strict-job.json").read_text())["sources"]
        current = recovery.source_hashes()
        changed = sorted(path for path in set(current) | set(original) if current.get(path) != original.get(path))
        if changed != ["bayesfilter/nonlinear/experimental_batched_svd_sigma_point_tf.py"]:
            raise RuntimeError("Candidate diagnostic must bind only the intended core repair")
        recovery._validate_source_migration(start, CAMPAIGN, original)
        payload = ledger.read()
        if payload["source_hash"] != payload_hash(start["sources"]) or payload["plan_hash"] != start["plan_hash"] or payload["total_budget_seconds"] != 86400.0:
            raise RuntimeError("Original budget binding changed")
    else:
    ...
    if args.mode == "preflight":
        seed_namespace = {"mode": "fixed_preoptimizer_bank_diagnostic", "execution_namespace": "numerical-repairs/eigh-refinement-r1"}
    ...
    source_name = {"covariance": "trace_covariance_diagnostic.py", "eigensystem": "gpu_eigensystem_diagnostic.py", "refinement": "gpu_eigensystem_diagnostic.py", "full-filter": "full_filter_refinement_diagnostic.py", "factor-filter": "full_filter_refinement_diagnostic.py", "candidate": "candidate_eigensolver_diagnostic.py", "preflight": "chart_preflight_diagnostic.py"}.get(args.mode, "replay_proposal_diagnostic.py")
    source = Path(__file__).with_name(source_name)
    mode_arguments = {"prefix": ("--sample-chain-results", "159"), "cached-gradient": ("--cached-gradient",), "covariance": (), "eigensystem": (), "refinement": ("--refine",), "full-filter": (), "factor-filter": ("--factor",), "candidate": (), "preflight": ()}[args.mode]
    ...
    durable_json(output / "strict-job.json", {
        ...
        "sources": recovery.source_hashes(), "scientific_sources_unchanged": args.mode != "candidate",
        "candidate_numerical_repair": args.mode == "candidate",
    })
    (output / source.name).write_bytes(source.read_bytes())
    (output / Path(__file__).name).write_bytes(Path(__file__).read_bytes())
    if args.mode in ("refinement", "full-filter", "factor-filter"):
    ...
    try:
        ...
        ledger.settle_arm(attempt_id=attempt, arm="strict",
                          ...
                          repair="candidate eigensolver repair check" if args.mode == "candidate" else "proposal telemetry localization; no numerical source change")
