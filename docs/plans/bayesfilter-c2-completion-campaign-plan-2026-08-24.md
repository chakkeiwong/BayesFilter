...
|---|---|---|---|---|---|
| Claim-lane device/dtype | A1 measured (proxy kernel): 4080-f64 jit 12.3 ms vs CPU-eager 26.4 ms vs CPU-jit 126 ms on lstsq [14336×468] | GPU-f64 competitive on these shapes; CPU-XLA lstsq lowers poorly | lstsq proxy may not represent the real ported kernel | Gate A re-measures the ACTUAL ported kernel; A3 at 9 axes | measured-proxy → confirmed at Gate A |
