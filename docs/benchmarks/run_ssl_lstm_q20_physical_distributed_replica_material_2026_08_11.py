...
CHECKPOINT = Path(
    ...
    "r4-four-chain-checkpoint/checkpoint.json"
)
MATERIALITY = Path(
    "docs/plans/artifacts/ssl-lstm-q20-physical-replica-travel-repair-2026-08-10/"
...
def run_material() -> Mapping[str, Any]:
    ...
    if checkpoint.get("status") != "FOUR_CHAIN_CHECKPOINT_PASSED_NOT_NOMINATED":
        raise MaterialReplicaError("bound checkpoint status changed")
    if not all(checkpoint["hard_gates"].values()):
        raise MaterialReplicaError("bound checkpoint hard gate failed")
    ...
    _append_log("starting 24x1 eight-hour material replica campaign")
    ...
    if (
        tuple(support.BETAS) != tuple(checkpoint["configuration"]["inverse_temperatures"])
        or tuple(support.STEPS) != tuple(checkpoint["configuration"]["step_sizes"])
        or int(support.LEAPFROG) != int(checkpoint["configuration"]["num_leapfrog_steps"])
        or int(support.CHAINS) != 4
    ):
        raise MaterialReplicaError("checkpoint kernel configuration drift")
    ...
    initial_physical = tf.reshape(
        initial_physical, (len(support.BETAS), support.CHAINS, PARAMETER_DIM)
    )
    ...
    with TFBatchValueScorePool(_pool_config()) as pool:
        ...
        while True:
            ...
            for _local_index in range(chunk_goal):
                ...
                swap_permutation = tf.logical_and(
                    tf.reduce_all(
                        ...
                        == tf.ones(
                            (len(support.BETAS), support.CHAINS), tf.int32
                        )
                    ),
                    tf.reduce_all(
            ...
            cache_value = tf.reshape(
                cache_value, (len(support.BETAS), support.CHAINS)
            )
            cache_score = tf.reshape(
                cache_score,
                (len(support.BETAS), support.CHAINS, PARAMETER_DIM),
            )
