...
are the analytic SVD-factor gradient and Hessian on a smooth simple-spectrum
branch.
...
\section{CUT-SVD Score and Hessian}
\label{sec:bf-cut-svd-score-hessian}

The conjugate unscented transform can be inserted into the same SVD derivative
recursion.  Replace the standardized unscented offsets $\xi^{(r)}$ in
\eqref{eq:bf-svd-sp-cloud} by the CUT offsets and weights from
Section~\ref{sec:bf-cut-rules}.  With fixed CUT tuning parameters, the point
derivatives remain
\[
  \dot\chi^{(r,i)}=\dot m^{(i)}+\dot C^{(i)}\xi^{(r)},\qquad
  \ddot\chi^{(r,ij)}=\ddot m^{(ij)}+\ddot C^{(ij)}\xi^{(r)}.
\]
The map derivatives, moment derivatives, innovation derivatives, and
solve-form likelihood score/Hessian are then exactly the formulas in
Section~\ref{sec:bf-svd-sp-analytic-recursion}, with the sums running over the
CUT point set.

Thus the analytic gradient and Hessian can be derived.  The derivation does not
require a new likelihood identity; it requires the following additional
contracts:
\begin{enumerate}[label=(\roman*)]
  \item the CUT points and weights are fixed or their tuning derivatives are
    included explicitly;
  \item all nonlinear transition and observation maps provide first and second
    derivatives at every CUT point;
  \item the covariance factor $C$ provides $\dot C$ and $\ddot C$ for the same
    implemented covariance $P_\star$ that places the CUT points;
  \item any SVD floor, rank truncation, sign/order convention, or fallback is
    part of the differentiated branch and is reported.
\end{enumerate}
Computationally, CUT4-G changes the constant in every derivative sum from
$2n+1$ points to $2n+2^n$ points.  The score cost scales like
$O(N_{\mathrm{CUT}})$ map-Jacobian evaluations plus factor derivative work; the
Hessian additionally requires second map derivatives and the second factor
derivative.  This is plausible for low-dimensional stochastic integration
blocks, but it is usually too expensive for large collapsed full-state DSGE
vectors.  The preferred design is therefore structural CUT-SVD: apply the CUT
rule to the declared low-dimensional stochastic block and use SVD only for the
covariance factor that truly needs spectral regularization.
