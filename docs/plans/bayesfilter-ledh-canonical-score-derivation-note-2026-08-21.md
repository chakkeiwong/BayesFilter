...
|---|---|---|---|
| S1 UKF predict | d(predicted mean, P^i)/dtheta through sigma points; Cholesky forward differential dL = L Phi(L^{-1} dS L^{-T}) | DERIVED (nonlinear fixture, chained state+parameter tangent) | GREEN |
...
| S5 UKF update | as S1 + gain differential dK = (dC - K dS) S^{-1} | DERIVED | GREEN |
