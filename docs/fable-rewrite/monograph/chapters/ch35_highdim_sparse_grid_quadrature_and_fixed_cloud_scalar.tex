...
\end{equation}
Applying the three-point standard-normal rule
$\{(-\sqrt3,\tfrac16),(0,\tfrac23),(\sqrt3,\tfrac16)\}$ to this standardized
integral gives
\begin{equation}
  \frac16(m-\sqrt3C)^2+\frac23m^2+\frac16(m+\sqrt3C)^2
  = m^2+C^2 = m^2+P,
  \label{eq:bf-hd-sgqf-scalar-x2-exact}
\end{equation}
so the rule reproduces this quadratic Gaussian moment exactly.  Once the state
has been standardized, the moment problem becomes a standard-normal weighted
integral.  The quadrature rule is therefore a numerical approximation to a
Gaussian expectation, not an extra modeling layer with a separate target.
...
\end{equation}
This concrete 3D sparse-grid reduction replaces the 27-point dense tensor
picture by a six-point merged cloud because the low-level signed combination
preserves the selected low-order axis structure of the declared Gaussian-moment
problem.  To see what is being approximated, take the explicit test function
\begin{equation}
  F(\xi)=\xi_1^2+\xi_1\xi_2+\xi_3 .
  \label{eq:bf-hd-sgqf-3d-test-function}
\end{equation}
Every stored node has exactly one nonzero coordinate, so the cross term and the
linear term both average to zero on the cloud, while the quadratic term
contributes $\tfrac16\cdot3+\tfrac16\cdot3=1$.  The six-point cloud therefore
reproduces the exact standard-normal value $\E[F(\xi)]=1$ for this declared 3D
example.
...
The low-level family used above now has an explicit exactness and UKF
relation.  For $(b,L)=(3,2)$ with the three-point rule, the merged cloud
reproduces every standard-normal moment generated by total-degree-at-most-three
monomials and the pure axis quartics $\E[\xi_j^4]=3$, while it does not
reproduce mixed quartics such as $\E[\xi_i^2\xi_j^2]$ for $i\ne j$; the point
count follows the bookkeeping path $10\to7\to6$ displayed in
\eqref{eq:bf-hd-sgqf-3d-merge-counts}.  Moreover, for general dimension $b$ at
level $L=2$, the merged cloud is exactly the classical UKF sigma-point set with
$\kappa=3-b$: the merged origin weight is $1-b/3=\kappa/(b+\kappa)$ and each
axis node $\pm\sqrt3\,e_j$ carries weight $1/6=1/(2(b+\kappa))$.  This identity
is explanatory.  It does not justify identifying sparse-grid filtering with UKF
filtering in general, and it does not remove Gaussian projection error.
...
Exported forward:
\begin{itemize}
  \item Chapter~\ref{ch:bf-highdim-fixed-cloud-sgqf-validation} takes up the
  value-path, numeric-oracle, and SGQF validation burden on the fixed cloud
  defined here.
  \item Chapter~\ref{ch:bf-highdim-low-rank-density-kr} takes up richer
  retained-object alternatives when one Gaussian is too narrow.
  \item Chapter~\ref{ch:bf-highdim-fixed-branch-same-scalar} may import the
  fixed-cloud scalar and branch-defining sparse-grid metadata only after the
  SGQF continuation chapter has stated them explicitly.
  \item Chapter~\ref{ch:bf-highdim-validation-defect-promotion} may import only
  the declared Gaussian-projection defect, point-explosion / active-dimension
  defect, and sparse-grid scalar contract stated across this chapter and its
  continuation.
\end{itemize}
