...
parser.add_argument("--mode", choices=("prefix", "cached-gradient", "covariance"), default="prefix")
...
with (CAMPAIGN / ".coordinator.lock").open("a+b") as lock:
    ...
    source = Path(__file__).with_name("trace_covariance_diagnostic.py" if args.mode == "covariance" else "replay_proposal_diagnostic.py")
    mode_arguments = {"prefix": ("--sample-chain-results", "159"), "cached-gradient": ("--cached-gradient",), "covariance": ()}[args.mode]
