# Independent R iAPF validation: all runs complete; comparison vetoes remain
...
QR has lower observed mean absolute terminal log error in every dimension,
but all five paired 99% intervals include zero: no accuracy ranking is
statistically supported. The repaired method has 14 observed conditional heuristic
...
fails its KKT check. The KKT residual measures numerical optimality for the frozen convex
objective and must meet its 1e-6 tolerance; it does not establish that the fitted guide captures
...
records across both learners. The repaired learner has seven vetoes against
QR and seven against the privileged analytic diagonal controls; none of its
observed vetoes is against the constant or observation-only guide. The following are up to eight largest observed
...
Passing this precision screen does not clear the other promotion vetoes.
For example, repaired d20/N250 has a nominal mean likelihood-ratio interval
[.8067, .9861]; QR d5/N1000 also flags calibration. These are warnings under
the predeclared screen, not multiplicity-adjusted evidence of estimator bias.
...
| Statistically supported ranking | None: all five predeclared paired intervals include zero |
...
Post-run skeptical review: the alternative explanation for stable completion
is that this small sample seldom exercises the numerical safeguards. Of
51,100 recorded repaired fits, 3,331 use positive ridge, four reach a curvature
bound and three invoke the active-set fallback. A zero-failure sample of 16
runs per dimension still allows a 17.1% one-sided upper failure-probability
bound. Weighted effective sample size can remain near one despite a valid
optimizer solution. These observations motivate a saved-case comparison of
learned guides with the analytic diagonal projections and full Gaussian guide;
they do not establish which mechanism caused the accuracy differences.
The weakest evidence remains rare likelihood tails and only two datasets per
dimension. Independent failures, stronger fresh accuracy/cost evidence or a
verified author implementation could change the conclusion. This candidate
fails the promotion screens; the iAPF research direction remains open.
...
Terminal review completed against the final CSV tables and all recorded hashes.
The 13 focused instrumentation/RNG/oracle checks and four terminal report
checks pass. All 80 final exact controls agree with Kalman within 3.64e-12.
The bounded scheduling repair preserves the original partial report and its
input index, replaces only the two unfinished cells with fresh attempts, and
counts every attempt against the same budget. This was a resource repair,
not a scientific-method change. Numerical work ended before the original hard
deadline; no further experiment was launched after its expiry.
