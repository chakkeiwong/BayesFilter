"""M9 complete no-hint regression fits with optional finite startup."""
import argparse
...
def worker(args,output):
    ...
    seed = seed_for(2026091940,"m9-no-hint-startup",args.case,args.phase,args.replication,"tuning")
    ...
    write_json(output/"manifest.json",{"source":source,"runtime":runtime,"command":sys.argv,
        ...
        "geometry_hint":None,"search_policy":"unchanged ordinary convenience search; startup option isolated"})
    run = tune_hmc_kernel(adapter=target,initial_position=initial_position,
        ...
        source_paths=[__file__,str(Path(sys.modules[PosteriordbTarget.__module__].__file__)),
            str(initialization_path),str(M8/"initialize_regression.py")])
    result = run.result
...
def main():
    ...
    output = ROOT/f"regression-{args.phase}-{args.replication}-gpu-r1"
    if args.worker:
        try:
            worker(args,output)
    ...
    if args.phase == "fresh":
        pilot = read(ROOT/"regression-pilot-0-gpu-r1/gpu-diagnostic-run.json")
        if pilot["returncode"] or pilot["elapsed_seconds"] * 1.5 > args.seconds:
            raise RuntimeError("pilot must complete with 50% execution margin before fresh fits")
