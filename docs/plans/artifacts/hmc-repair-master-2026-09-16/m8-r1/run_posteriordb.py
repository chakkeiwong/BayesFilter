"""Bounded public ordinary-tuner fits to two matched posteriordb references."""
import argparse
...
def worker(args,output):
    ...
    seed = seed_for(2026091840,"posteriordb",args.case,args.phase,args.replication,"tuning")
    ...
    run = tune_hmc_kernel(adapter=target,initial_position=tf.zeros(target.parameter_dim,tf.float64),
...
def main():
    ...
    output = ROOT/f"posteriordb-{tag}-{args.phase}-{args.replication}-gpu-r1"
    if args.worker:
        try:
            worker(args,output)
