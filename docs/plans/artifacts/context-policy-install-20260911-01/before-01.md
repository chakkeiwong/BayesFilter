<!-- BEGIN CODEX CONTEXT RECOVERY POLICY -->
## Context and recovery discipline

- Keep the active research question, current stage, checked findings, evidence
  paths, remaining budget, and exact next action in one concise checkpoint.
  Update it after each substantive result and before a large read or patch.
  Preserve detailed evidence in linked files rather than growing the checkpoint.
- Read only the source ranges and JSON fields needed for the next decision.
  Normally cap each tool response near 2,000 tokens and the combined output of
  a batch near 6,000. These are output budgets, not limits on scientific rigor:
  split necessary reads and retain complete logs on disk. Set explicit limits
  on exec/tool calls; do not rely solely on the configured default.
- Never dump raw saved-session JSONL lines or recursively print session search
  matches. Parse the exact session and select bounded fields when incident
  evidence is needed. Do not print instruction payloads or embedded old outputs.
- Return concise completion/error summaries from mutations and keep long patch
  or command output in files. Write findings promptly instead of reserving a
  large final report for the end of a nearly full context.
- When visible context approaches 90,000 tokens under the 120,000 trial
  compaction trigger, update the checkpoint before more large work. This is an
  operational warning level, not a measured provider limit or scientific stop.
- While remote compaction is unreliable, use fresh conversations for substantial
  stages, initialized from the checkpoint and governing instructions. Resume/fork
  retains history and is not a clean-context recovery. Within a working stage,
  continue authorized work normally; do not add human approval gates.
- After repeated pre-turn compaction failures, retain the failed session as
  incident evidence and recover through the checkpoint. Do not increase the
  declared context window, disable safeguards, or discard applicable research
  policies merely to force the old context through. Configuration limits alone
  do not guarantee provider reliability.

<!-- END CODEX CONTEXT RECOVERY POLICY -->

## Local Codex GPU/CUDA sandbox policy

This machine has a real NVIDIA GPU, but non-escalated Codex sandbox commands can hide or block GPU device access. This can make a healthy CUDA/TensorFlow setup look broken.

- Run any command that detects, initializes, benchmarks, or uses GPU/CUDA/NVIDIA devices with escalated sandbox permissions.
- This includes `nvidia-smi`, `/dev/nvidia*` checks, TensorFlow/JAX/PyTorch/CuPy GPU probes, XLA-GPU tests, GPU benchmarks, and GPU HMC/ML jobs.
- Treat non-escalated GPU failures as sandbox evidence only. Errors such as `GPU access blocked by the operating system`, missing `/dev/nvidia*`, empty framework GPU lists, or CUDA `cuInit` errors do not prove the driver, WSL, conda environment, CUDA stack, or framework install is broken until rerun with escalation.
- For deliberate CPU-only runs, set `CUDA_VISIBLE_DEVICES=-1` or the project-standard CPU-hiding variable before framework import and record that CPU-only choice in the artifact.
- Before debugging GPU setup, first run escalated `nvidia-smi` and an escalated framework-specific device probe.
