...
from tests.test_filter_repair_quadratic_rounds import fixture, public_record, reference


@pytest.mark.parametrize("dimension", [3, 5])
@pytest.mark.parametrize("arm", ["before", "graph", "xla"])
def test_public_paired_refinement_costs(arm, dimension, request):
    ...
    if arm != "before":
        ...
        kernel = controllers.paired_quadratic_controller(callback, dimension, options, jit_compile=arm == "xla")
    ...
    report = {"role": "descriptive_public_paired_refinement_cost", "baseline": "3582b4ac", "arm": arm,
    ...
    with (directory / "quadratic-public-memory.json").open("x") as handle:
