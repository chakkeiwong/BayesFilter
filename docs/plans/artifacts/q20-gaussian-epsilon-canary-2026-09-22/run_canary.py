...
def worker(output):
    ...
    try:
        ...
        for name, bank in banks.items():
            value, score, raw_status = adapter.log_prob_and_grad_status(bank)
            # Combined target evaluation returns model status. The public
            # telemetry method handles incomplete optional conditioning fields.
            status = adapter.target_status_telemetry(bank)
