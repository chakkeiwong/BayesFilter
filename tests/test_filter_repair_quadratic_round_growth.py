...
def test_complete_controller_capacity_and_warm_allocations(rounds, request):
    ...
    def record_progress(stage, **observed):
        ...
        with (directory / "quadratic-round-growth-progress.jsonl").open("a") as handle:
    ...
    kernel = make_paired_quadratic_controller(callback, 5, config)
    ...
    with (directory / "quadratic-round-growth.json").open("x") as handle:
