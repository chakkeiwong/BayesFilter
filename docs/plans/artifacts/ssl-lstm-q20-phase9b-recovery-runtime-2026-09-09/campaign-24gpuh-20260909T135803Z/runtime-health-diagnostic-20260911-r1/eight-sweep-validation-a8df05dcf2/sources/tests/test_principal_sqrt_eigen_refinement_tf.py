...
def test_phase9b_fixed_preflight_row_converges_without_relaxing_status(backend):
    """Native CPU reference for the September 11 failed factor bank row 15."""
    parameters = tf.constant([
        [1.6050387408383835, 7.740603524870586, 1.0118188620015613, 4.01455202940322],
    ], tf.float64)
    target = BatchNativeSSLLSTMComplexityPosteriorTarget(
        20, jit_compile=True, principal_sqrt_backend=backend,
    )
    value, score, _, _, status = target.batch_prior_likelihood_value_score_status(parameters)
    np.testing.assert_array_equal(status["valid_pre_regularized_score"].numpy(), [True])
    np.testing.assert_array_equal(status["placement_classified_invalid_count"].numpy(), [0])
    np.testing.assert_allclose(value.numpy(), [-1481.9561556810868], atol=1e-10, rtol=1e-12)
    np.testing.assert_allclose(score.numpy(), [
        [2.1662754062678893, -250.54140857894552, 170.26397214433257, -251.45968792421064],
