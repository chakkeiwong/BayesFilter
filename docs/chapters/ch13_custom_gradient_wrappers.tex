...
\end{equation}
where the likelihood terms use the same structural provider, masks, factor
...
The custom-gradient path is valid only if it returns
\begin{equation}
\label{eq:bf-custom-gradient-target}
  g_i(\psi)=\frac{\partial \mathcal{L}(\psi)}{\partial\psi_i}.
\end{equation}
...
mathematical tuple:
\[
  \operatorname{value\_and\_score}(\psi)
  =
  \left(
    \mathcal{L}(\psi),
...
know whether it is using the certified target derivative.
