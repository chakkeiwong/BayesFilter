...
def profile_density_loss(z, target, parameters):
    ...
    density=tf.exp(-tf.reduce_sum(log_sd)-.5*tf.reduce_sum(standardized**2,axis=1))
    scale=tf.reduce_sum(density*target)/tf.reduce_sum(target**2)
    residual=density-scale*target
    loss=tf.reduce_mean(residual**2)
    mean_gradient=2*tf.reduce_mean((residual*density)[:,None]*standardized*tf.exp(-log_sd),axis=0)
    log_sd_gradient=2*tf.reduce_mean((residual*density)[:,None]*(standardized**2-1),axis=0)
    shape_error=tf.reduce_sum(residual**2)/tf.reduce_sum(density**2)
    return loss,tf.concat([mean_gradient,log_sd_gradient],0),scale,shape_error
...
def bounded_density_fit(points, log_targets, *, mean_bound, sd_lower, sd_upper,
    ...
    target=tf.exp(log_targets-log_target_max)
    ...
    initial_loss,initial_gradient,_,_=profile_density_loss(z,target,parameters)
    ...
    def body(iteration,par,loss,gradient,step,healthy):
        def search(j,step,trial,trial_loss,accepted):
            ...
            trial_loss=profile_density_loss(z,target,trial)[0]
        ...
        loss,gradient,_,_=profile_density_loss(z,target,par)
    ...
    loss,gradient,scale,shape_error=profile_density_loss(z,target,parameters)
    ...
    log_lambda=tf.math.log(scale)+log_constant-log_target_max
...
def make_density_recursive_fit_kernel(d,o,N,T,mean_bound,sd_lower,sd_upper,
    ...
    def fit(theta,observations,clouds):
        ...
        floors=tf.TensorArray(dtype,size=T);diagnostics=tf.TensorArray(dtype,size=T,element_shape=[8])
