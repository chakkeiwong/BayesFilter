"""Fresh-process complete uniform-controller cost diagnostics; not runtime code."""
...
def test_complete_uniform_controller_costs(arm, dimension, request):
    ...
    kernel = None if arm == "before" else make_quadratic_controller(callback, dimension, config, jit_compile=arm == "xla")
    ...
    report = {"role": "descriptive_complete_uniform_controller_cost", "baseline": "3582b4ac",
    ...
    with (directory / "uniform-round-memory.json").open("x") as handle:
