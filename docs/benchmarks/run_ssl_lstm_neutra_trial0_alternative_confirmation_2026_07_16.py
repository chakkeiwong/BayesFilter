#!/usr/bin/env python3
"""Confirm predeclared trial-0 NeuTra policy on untouched G/H streams."""
...
SCHEMA = "bayesfilter.ssl_lstm_neutra.trial0_alternative_confirmation.v1"
POLICY_SCHEMA = "bayesfilter.ssl_lstm_neutra.trial0_alternative_policy.v1"
PLAN_PATH = Path(
...
PARENT_PLATEAU_SCHEDULE_KEYS = (
    "validation_check_every",
    "patience_steps",
    "patience_checks",
    "max_steps",
...
FRESH_STREAMS = (
    FreshStream("fresh-g", (20260716, 7105), (20260716, 8105), (20260716, 8205)),
    FreshStream("fresh-h", (20260716, 7106), (20260716, 8106), (20260716, 8206)),
)
EXCLUDED_SEED_ROWS = frozenset(
    {
        (20260715, 4101),
        (20260715, 4102),
        (20260715, 5101),
...
def stream_payload(stream: FreshStream) -> dict[str, Any]:
    """Return the canonical JSON representation used in persisted artifacts."""

    return {
        "label": stream.label,
        "initialization_seed": list(stream.initialization_seed),
        "training_seed": list(stream.training_seed),
        "validation_seed": list(stream.validation_seed),
    }
...
def load_policy(path: Path) -> dict[str, Any]:
    ...
    support_policy = ROOT / str(payload.get("support_policy_path"))
    if not support_policy.is_file():
        raise ConfirmationError("support-aware policy is unavailable")
    if sha256(support_policy) != payload.get("support_policy_sha256"):
        raise ConfirmationError("support-aware policy hash mismatch")
    support_payload = json.loads(support_policy.read_text(encoding="utf-8"))
    trial_record = ROOT / str(payload.get("selected_trial_record_path"))
    if not trial_record.is_file():
        raise ConfirmationError("selected trial-0 record is unavailable")
    if sha256(trial_record) != payload.get("selected_trial_record_sha256"):
        raise ConfirmationError("selected trial-0 record hash mismatch")
    trial_payload = json.loads(trial_record.read_text(encoding="utf-8"))
    expected_hyperparameters = {
        "learning_rate": float(trial_payload.get("params", {}).get("learning_rate")),
        "initialization_scale": float(
            trial_payload.get("params", {}).get("initialization_scale")
        ),
        "gradient_clip_norm": float(
            trial_payload.get("params", {}).get("gradient_clip_norm")
        ),
    }
    if trial_payload.get("status") != "SURVIVED" or trial_payload.get(
        "trial_number"
    ) != 0:
        raise ConfirmationError("selected trial record is not surviving trial 0")
    if payload.get("selected_hyperparameters") != expected_hyperparameters:
        raise ConfirmationError("alternative policy does not match trial 0")
    parent_plateau = support_payload.get("plateau_policy", {})
    repaired_plateau = payload.get("plateau_policy", {})
    if any(
        repaired_plateau.get(key) != parent_plateau.get(key)
        for key in PARENT_PLATEAU_SCHEDULE_KEYS
    ):
        raise ConfirmationError("alternative policy changed the support plateau schedule")
    for key in (
        "roundtrip_max_abs",
        ...
        if repaired_plateau.get(key) != parent_plateau.get(key):
            raise ConfirmationError("alternative policy changed support eligibility")
    support_audit = ROOT / str(payload.get("support_audit_path"))
    if not support_audit.is_file():
        raise ConfirmationError("support audit is unavailable")
    if sha256(support_audit) != payload.get("support_audit_sha256"):
        raise ConfirmationError("support audit hash mismatch")
    trial2_result = ROOT / str(payload.get("trial2_confirmation_path"))
    if not trial2_result.is_file():
        raise ConfirmationError("trial-2 confirmation result is unavailable")
    if sha256(trial2_result) != payload.get("trial2_confirmation_sha256"):
        raise ConfirmationError("trial-2 confirmation result hash mismatch")
    ...
    if len(all_fresh_seeds) != 6 or all_fresh_seeds & EXCLUDED_SEED_ROWS:
...
def controller_config(policy: dict[str, Any]) -> NeuTraPlateauConfig:
    ...
    return NeuTraPlateauConfig(
        ...
        roundtrip_max_abs=float(row["roundtrip_max_abs"]),
        moderate_shell_max_inverse_radius=float(
            row["moderate_shell_max_inverse_radius"]
        ),
    )
...
def checkpoint_probes(target: Any, trainer: Any, stream: FreshStream, step: int) -> dict[str, Any]:
    """Evaluate prospective export eligibility on the current trainer state."""

    frozen = trainer.frozen_transport_payload(
        transport_id=f"ssl-lstm-support-check-{stream.label}-{step}",
        target_signature=target.target_signature(),
    )
    loaded = load_frozen_neutra_artifact(
        frozen, expected_target_signature=target.target_signature()
    )
    return HELPERS._probe_diagnostics(target, loaded.transport)  # noqa: SLF001
...
def controller_observation(
    ...
    *,
    ...
    return controller.observe(
        ...
        all_finite=bool(probes["all_finite"]),
        roundtrip_max_abs=float(probes["roundtrip_max_abs"]),
        moderate_shell_max_inverse_radius=float(
            probes["moderate_shell_max_inverse_radius"]
        ),
        trainer_state_hash=trainer_state_hash,
    )


def support_diagnostics_match(
    observed: dict[str, Any] | None, expected: dict[str, Any]
) -> bool:
    if observed is None or set(observed) != set(expected):
        return False
    if bool(observed["all_finite"]) != bool(expected["all_finite"]):
        return False
    return all(
        math.isclose(
            float(observed[key]),
...
def run_stream(
    *,
    ...
    if resume:
        ...
        if progress.get("schema") != SCHEMA or progress.get("stream") != stream_payload(
            stream
        ):
        ...
        if not history:
            raise ConfirmationError("resume progress lacks history")
        if (controller.best_step is None) != (best_state is None):
            raise ConfirmationError("resume best-state/controller mismatch")
        initial = history[0]
        ...
        for row in checkpoints:
            existing_path = ROOT / str(row.get("path"))
            if not existing_path.is_file() or sha256(existing_path) != row.get("sha256"):
                raise ConfirmationError("resume checkpoint history hash mismatch")
    else:
        initial = host_validation(trainer, validation_z, 0)
        initial_state = trainer.state_payload()
        initial_probes = checkpoint_probes(target, trainer, stream, 0)
        initial_action = controller_observation(
            controller,
            validation=initial,
            probes=initial_probes,
            trainer_state_hash=initial_state["state_hash"],
        )
        best_state = initial_state if initial_action.meaningful_improvement else None
        history = [
            {
                **initial,
                "checkpoint_probes": initial_probes,
    ...
    for step in range(start_step, controller.config.max_steps + 1):
        ...
        probes = checkpoint_probes(target, trainer, stream, step)
        action = controller_observation(
            controller,
        ...
        write_json(checkpoint_path, joint, replace=resume and checkpoint_path.exists())
        checkpoints.append(
            {
                "step": step,
    ...
    probes = None
    paired = None
    if best_state is None or controller.best_step is None:
        vetoes.append("no_support_eligible_checkpoint")
    else:
        ...
        probes = HELPERS._probe_diagnostics(  # noqa: SLF001
        ...
        if probes["roundtrip_max_abs"] > controller.config.roundtrip_max_abs:
            vetoes.append("roundtrip_residual_above_threshold")
        if probes["moderate_shell_max_inverse_radius"] > (
            controller.config.moderate_shell_max_inverse_radius
        ):
            vetoes.append("moderate_shell_missing_support")
        expected_best_diagnostics = {
            "all_finite": bool(probes["all_finite"]),
            "saturation_fraction": float(
                next(
                    row["saturation_fraction"]
                    for row in history
                    if int(row["step"]) == int(controller.best_step)
                )
            ),
            "roundtrip_max_abs": float(probes["roundtrip_max_abs"]),
            "moderate_shell_max_inverse_radius": float(
        ...
        if not support_diagnostics_match(
            controller.best_checkpoint_diagnostics, expected_best_diagnostics
        ):
            vetoes.append("serialized_best_support_diagnostics_mismatch")
    ...
    return {
        ...
        "best_probes": probes,
        "best_checkpoint_diagnostics": controller.best_checkpoint_diagnostics,
        "terminal_state_path": terminal_state_path.relative_to(ROOT).as_posix(),
        "terminal_state_sha256": sha256(terminal_state_path),
        "best_state_path": (
            None if best_state_path is None else best_state_path.relative_to(ROOT).as_posix()
        ),
        "best_state_sha256": None if best_state_path is None else sha256(best_state_path),
        "best_frozen_path": (
            None if frozen_path is None else frozen_path.relative_to(ROOT).as_posix()
        ),
        "best_frozen_sha256": None if frozen_path is None else sha256(frozen_path),
        "run_manifest": {
        ...
        "nonclaims": [
            "support-eligible transport-training confirmation only",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.resume:
        ...
        previous_source_sha256 = previous_summary.get("source_bindings", {}).get(
            "source_sha256", {}
        )
        current_source_sha256 = bindings["source_sha256"]
        if any(
            previous_source_sha256.get(key) != current_source_sha256.get(key)
            for key in RESUME_NUMERICAL_BINDING_KEYS
        ):
            raise ConfirmationError("resume numerical source binding drift")
        resume_lineage = {
            "previous_summary_sha256": sha256(previous_summary_path),
            "previous_source_bindings": previous_summary.get("source_bindings"),
            "resume_runner_sha256": current_source_sha256["runner"],
            "resume_reason": "authorized_resource_extension_only",
        }
        ...
        previous_cap = float(
            previous_summary.get("run_manifest", {}).get("gpu_cap_seconds", 0.0)
        )
        if args.gpu_cap_seconds <= previous_cap:
            raise ConfirmationError(
                "resume requires a newly authorized cumulative cap above the prior cap"
            )
