...
construct moments. Let $\{(\zeta_\ell,\omega_\ell)\}$ be a standardized sparse
quadrature cloud and write $x_\ell=m+L\zeta_\ell$. Propagating these points
through the nonlinear transition and observation map gives weighted projected
moments
\begin{equation}
 m^+=\sum_\ell \widetilde\omega_\ell x_\ell^+,qquad
 P^+=\sum_\ell \widetilde\omega_\ell
       (x_\ell^+-m^+)(x_\ell^+-m^+)^\mathsf T,
 \qquad \sum_\ell\widetilde\omega_\ell=1.       \label{eq:sgqf-moments}
\end{equation}
...
The first SGQF experiment therefore takes $m^+$ and $P^+$ only to define
...
horizon, and observation regime. The primary quantities are model-corrected
normalizer error, coefficient of variation of $\widehat z_t$, effective sample
size, weight-tail diagnostics, and equal-compute wall time. Score MSE is
reported only after the proposal identity and target-preservation tests pass.
...
\end{equation}
Under four continuous derivatives and a symmetric fixed design, this cubic
derivative has fourth-order truncation, while a linear or quadratic symmetric
fit retains an $O(h^2)$ term. More points can lower observation-noise variance
or can amplify correlated particle noise; the order and the replicate variance
...
full-rank direction matrix $V=(v_1,\ldots,v_m)$. If $m>d$, recover the score
from the least-squares system $V^\mathsf T s=(s_{v_1},\ldots,s_{v_m})$ and
report the condition number of $V$. The ladder is calibrated on oracle-bearing
models by plotting error and replicate variance against $h$, checking slopes
near two for $D_2$ and near four for $D_4$ and the cubic fit, and freezing the
selected ladder on a validation/claim scope. It is a diagnostic or parity arm
when no oracle exists; it cannot by itself certify score bias or create an exact
control variate.
