...
record = {'command': command, 'started_utc': datetime.datetime.now(datetime.timezone.utc).isoformat(),
          ...
          'source_hashes': {str(p.relative_to(source_root)): hashlib.sha256(p.read_bytes()).hexdigest()
              for p in (source_root/'bayesfilter').rglob('*.py')},
          ...
          'plan_file': 'docs/plans/bayesfilter-hmc-m26-lifetime-and-policy-plan-2026-09-22.md',
          'result_file': str(root / 'execution.json'), 'timeout_seconds': args.seconds,
          'gpu_intentionally_hidden': True, 'data_version': 'declared development law or read-only M21 evidence; see child manifest'}
(root/'manifest.json').write_text(json.dumps(record, indent=2) + '\n')
started = time.monotonic()
with (root/'worker.log').open('x') as log:
    try:
        process = subprocess.Popen(command, cwd=source_root, env=env, stdout=log, stderr=subprocess.STDOUT, start_new_session=True)
