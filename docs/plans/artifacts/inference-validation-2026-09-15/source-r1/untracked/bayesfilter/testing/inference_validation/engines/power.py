...
def run(design,root,deadline=None):
    ...
    for trial in range(design.replications):
        ...
        truth=rng.normal(scale=2.,size=n); y=truth+rng.normal(size=n)
        # Independent normal conjugate sampler, tau=2, sigma=1, one observation.
        mean=.8*y; sd=np.sqrt(.8)
        draws=mean[:,None]+sd*rng.normal(size=(n,m))
        prior=2*rng.normal(size=(n,m))
        arm_results={}
        for arm,values in (("correct",draws),("ignored_data",prior)):
            parameter=(values<truth[:,None]).sum(1)
            likelihood=(stats.norm.logpdf(y[:,None]-values)<stats.norm.logpdf(y-truth)[:,None]).sum(1)
            tests={name:rank_uniform_test(r,m,null_draws=design.null_draws,
                seed=seed_for(design.seed,design.design_id,trial,arm,name),alpha=design.alpha,multiplicity=2)
                for name,r in (("parameter",parameter),("log_likelihood",likelihood))}
        ...
        for arm,shift in (("ks_null",0.),("ks_location_defect",1.)):
    ...
    count=len(trials)
    rates={}
    for arm in ("correct","ignored_data","ks_null","ks_location_defect"):
    ...
    result={"trials":trials,"rates":rates,"finding":"power_estimated" if count==design.replications else "incomplete",
        ...
        "hmc_mutation_power_established":False,"within_trial_multiplicity":2,
        "allocation":"normal conjugate tau=2 sigma=1; strong data-omission diagnostic, not default promotion"}
        ...
        for arm in arms:
            ...
            try:
                ...
                rows[arm]={"valid":valid,"detected":valid and finding in {"mechanics_discrepancy",
                    "inventory_discrepancy","pipeline_discrepancy","discrepancy_detected"},
