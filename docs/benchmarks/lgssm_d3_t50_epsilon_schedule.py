"""D=3 T=50 LGSSM score bias vs Sinkhorn epsilon (Corenflos eps_N = o(1/log N) test).
...
EPSILON_LADDER = [1.0, 0.5, 0.25, 0.145, 0.0725]
SINKHORN_LADDER = {1.0: 8, 0.5: 8, 0.25: 16, 0.145: 32, 0.0725: 64}
EPSILON = 1.0
SINKHORN_STEPS = 8
FLOW_SUBSTEPS = 12
...
def run_one_eps(obs, particles, seed, epsilon, sinkhorn_iters, direction=0):
    ...
    value, score = canonical_value_and_analytical_score(
        ...
        reset_epsilon=epsilon, reset_sinkhorn_steps=sinkhorn_iters,
        reset_balance_steps=sinkhorn_iters, reset_ridge=1e-5,
...
def main():
    ...
    print(f"D=3 T=50 LGSSM score error across Sinkhorn epsilon (N={PARTICLES})")
    ...
    header = (
        f"{'eps':>6} {'sink':>6}  {'value err':>11}  {'val SE':>9}  "
    ...
    for eps in EPSILON_LADDER:
        sinkhorn_iters = SINKHORN_LADDER[eps]
        print(f"{eps:6.4f} sink={sinkhorn_iters:2d}  ", end="", flush=True)
        vals, scores = [], []
        for seed in SEEDS:
            val, sc = run_one_eps(obs, PARTICLES, seed, eps, sinkhorn_iters, direction=0)
            vals.append(val)
            scores.append(sc)
    ...
    result = {
        "study": "D=3 T=50 LGSSM analytical score error across Sinkhorn epsilon",
        "date": "2026-08-29",
        ...
        "sinkhorn_ladder": SINKHORN_LADDER,
        "particles": PARTICLES,
        "seed_count": len(SEEDS),
        "rungs": rungs,
        "elapsed_seconds": elapsed,
        "hypothesis_under_test": (
            "Corenflos et al. (2021) Prop 3.3 requires eps_N = o(1/log N) for the "
            "entropic-OT DPF filtering/log-likelihood error to vanish. Production uses "
            "fixed eps=1.0, outside that schedule. c/log(1008) ~ 0.145 for c=1. This run "
            "asks whether shrinking eps along that schedule moves the measured score bias."
        ),
        "interpretation_rules": [
            "Descriptive for these 16 seeds and this single observation path.",
            "Sinkhorn iterations are raised as eps falls so that transport is not "
            "confounded with unconverged Sinkhorn; iteration count is therefore not held fixed.",
            "A bias that does not fall with eps points at the filtering-vs-smoothing "
            "conditioning gap (Corenflos Prop 3.1) rather than the entropic term.",
        ],
        "non_claims": [
            "No unbiasedness, convergence, Kalman-equivalence, or default-readiness claim.",
            "Corenflos Prop 3.3 is an upper bound under compactness and Lipschitz-OT "
            "assumptions not verified for this fixture; no rate is being asserted.",
        ],
