...
with (CAMPAIGN / ".coordinator.lock").open("a+b") as lock:
    ...
    output = CAMPAIGN / "launches" / ("proposal-diagnostic-" + uuid.uuid4().hex[:10])
    output.mkdir()
    attempt = output.name
    binding = {"source_hash": payload_hash(start["sources"]), "plan_hash": start["plan_hash"]}
    seed_namespace = {"mode": "proposal_localization_diagnostic", "original_transition_indices": [157, 158]}
    ...
    command = ("timeout", "--signal=TERM", "--kill-after=30s", f"{cap - 30:g}s", sys.executable,
               str(source), "--gpu-uuid", expected, "--output-dir", str(output / "strict"))
