...
opens the part on differentiable resampling and transport by showing, in the
smallest possible setting, how differentiability can be bought by changing the
resampling map itself before moving to other differentiable and
transport-based alternatives.  The neighboring literature includes
truncated-gradient differentiable particle filters
\citep{jonschkowski2018differentiable}, learned differentiable resampling
\citep{zhumurphyjonschkowski2020}, resampling-mixture particle-filter networks
\citep{karkus2018particle}, and entropy-regularized transport resampling
...
\citep{doucet2001sequential,chopin2020introduction}.
...
piecewise constant in those weights \citep{zhumurphyjonschkowski2020,jonschkowski2018differentiable}.
...
weighted mean of the cloud.  This chapter uses ``soft resampling'' in this
narrow local sense; it is a deliberately minimal teaching rule, and it is not
the same algorithm as the resampling-mixture rule of
\citet{karkus2018particle}.  Define
\begin{equation}
...
The next chapter keeps the equal-weighting goal but replaces the surrogate rule
by a declared transport object.
