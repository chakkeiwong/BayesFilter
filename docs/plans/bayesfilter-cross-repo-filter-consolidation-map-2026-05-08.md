...
| Linear analytic score/Hessian | MacroFinance `filters/tf_differentiated_kalman.py`, `filters/tf_solve_differentiated_kalman.py` | `bayesfilter/linear/kalman_derivatives_tf.py` | Choose dense first, solve-form if diagnostics are clearer. |
| QR/square-root robustness | MacroFinance `filters/tf_qr_sqrt_differentiated_kalman.py` | `bayesfilter/linear/kalman_qr_tf.py` or derivative helper module | Port after dense derivative path passes. |
...
| Linear analytic derivatives | MacroFinance `tests/test_filter_backend_parity.py`, `tests/test_one_country_analytic_backend_parity.py` | `tests/test_linear_kalman_derivatives_tf.py` |
| QR derivative identities | MacroFinance `tests/test_tf_qr_derivative_identities.py` | `tests/test_linear_qr_factor_derivatives_tf.py` |
...
4. Port linear SVD value backend.
5. Choose and port the first analytic derivative route.
...
Stop before analytic derivative, SVD nonlinear, CUT, GPU, or cross-repo
migration phases unless the previous gates pass and the next donor route is
made explicit.
