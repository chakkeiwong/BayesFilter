...
def accuracy_assessment(draws, reference, *, tolerance, finite_variance=True):
    ...
    for j in range(x.shape[-1]):
        ...
        se=math.sqrt(float(per_chain.var(ddof=1)/len(per_chain)+r.var(ddof=1)/len(r))) if len(per_chain)>1 else None
        ...
        if finite_variance:
            rows.append({"quantity":f"mean_{j}","error":float(x[:,:,j].mean()-ref[:,j].mean()),
                         "role":"descriptive","reference_mean_se":float(ref[:,j].std(ddof=1)/math.sqrt(len(ref)))})
    ...
    return {"finding":"reference_discrepancy" if any(r.get("descriptive_within_tolerance") is False for r in rows) else "within_descriptive_tolerance",
            "quantities":rows,"accuracy_established":False,
