"""Same-target KSC-SV LEDH score runner.

This module differentiates the finite-N KSC-SV LEDH value scalar used by
``benchmark_ledh_same_target_ksc_sv_value``.  The target likelihood is the KSC
finite Gaussian mixture for transformed observations:

``z_t = log(y_t**2 + 1e-8)``,
``z_t | x_t ~ sum_k w_k Normal(2 log(beta) + x_t + m_k, v_k)``.

This is a surrogate target row.  It is not exact native actual-SV likelihood.
The score route is compact forward sensitivity and does not use tape/autodiff
or all-time reverse records.
"""
...
def _as_theta(theta: tf.Tensor | Sequence[float]) -> tf.Tensor:
    ...
    if tensor.shape.rank != 1 or int(tensor.shape[0]) != len(PARAMETER_NAMES):
        raise ValueError("KSC-SV theta must have shape [2]")
    return tensor


def _gamma_beta(theta: tf.Tensor) -> tuple[tf.Tensor, tf.Tensor, tf.Tensor, tf.Tensor]:
    ...
    d_gamma = tf.constant([1.0, 0.0], dtype=DTYPE) * dgamma_dz
    d_beta = tf.constant([0.0, 1.0], dtype=DTYPE) * beta
    return gamma, beta, d_gamma, d_beta
...
def _require_compact_score_args(args: argparse.Namespace) -> None:
    ...
    if args.transport_ad_mode != "full":
        raise ValueError("KSC-SV compact score requires transport_ad_mode='full'")
    if args.transport_gradient_mode != core_tf.MANUAL_STREAMING_FINITE_TRANSPORT_GRADIENT_MODE:
        raise ValueError("KSC-SV compact score requires manual streaming finite transport")
    if args.transport_policy == "no-resampling":
        raise ValueError("KSC-SV admitted score requires active relaxed Sinkhorn transport")
...
def _ksc_mixture_log_density_jvp(
    *,
    residual: tf.Tensor,
    d_residual: tf.Tensor,
    weights: tf.Tensor,
    means: tf.Tensor,
    variances: tf.Tensor,
) -> tuple[tf.Tensor, tf.Tensor]:
    residual = tf.convert_to_tensor(residual, dtype=DTYPE)
    d_residual = tf.convert_to_tensor(d_residual, dtype=DTYPE)
    weights = tf.reshape(tf.convert_to_tensor(weights, dtype=DTYPE), [-1, 1, 1])
    means = tf.reshape(tf.convert_to_tensor(means, dtype=DTYPE), [-1, 1, 1])
    variances = tf.reshape(tf.convert_to_tensor(variances, dtype=DTYPE), [-1, 1, 1])
    component_log_terms = (
        tf.math.log(weights)
        ...
        * (
            ...
            + tf.square(residual[tf.newaxis, :, :] - means) / variances
        )
    )
    density = tf.reduce_logsumexp(component_log_terms, axis=0)
    responsibilities = tf.exp(component_log_terms - density[tf.newaxis, :, :])
    component_residual_score = -(residual[tf.newaxis, :, :] - means) / variances
    residual_score = tf.reduce_sum(responsibilities * component_residual_score, axis=0)
    tangent = residual_score[:, :, None] * d_residual
    return density, tangent
...
def _compact_forward_transport_jvp_tf(
    *,
    ...
    if args.transport_gradient_mode != core_tf.MANUAL_STREAMING_FINITE_TRANSPORT_GRADIENT_MODE:
        raise ValueError("compact KSC-SV score requires manual streaming finite transport")
    if args.transport_ad_mode != "full":
        raise ValueError("compact KSC-SV score requires transport_ad_mode='full'")
...
def _compact_ledh_flow_jvp_tf(
    *,
    ...
    if state_dim != 1:
        raise ValueError("KSC-SV compact score expects one-dimensional state")
    ...
    for block_index in range(num_blocks):
        ...
        def observation_jacobian_fn(points: tf.Tensor) -> tf.Tensor:
            ...
            if chunk_particles is None:
                raise ValueError("KSC-SV adapter requires static particle chunk dimension")
        ...
        if is_initial:
            ...
            d_prior_means = (
                ...
                + ancestor_chunk[:, :, :, None] * tf.reshape(d_gamma, [1, 1, 1, param_dim])
            )
...
def _common_tensors(args: argparse.Namespace) -> tuple[dict[str, tf.Tensor], dict[str, Any]]:
    tensors, semantics = value_mod._build_ksc_sv_tensors(args)  # noqa: SLF001
    return {
        ...
        "mixture_weights": tf.cast(tensors["mixture_weights"], DTYPE),
        "mixture_means": tf.cast(tensors["mixture_means"], DTYPE),
        "mixture_variances": tf.cast(tensors["mixture_variances"], DTYPE),
    }, semantics
    ...
    for time_index in range(int(args.time_steps)):
        ...
        if is_initial:
            ...
            d_proposal_variance = tf.tile(
                tf.reshape(d_stationary_variance, [1, param_dim]),
                [batch_size, 1],
            )
        else:
        ...
        target_observation, d_target_observation = _ksc_mixture_log_density_jvp(
            residual=residual,
...
def _manual_value_only_from_components(
    ...
    *,
    ...
    for time_index in range(int(args.time_steps)):
        observation = observations[time_index]
        is_initial = time_index == 0
        proposal_mean = tf.zeros_like(particles) if is_initial else gamma * particles
        proposal_variance = stationary_variance if is_initial else tf.square(sigma)
        proposal_covariance = tf.ones([batch_size, 1, 1], dtype=DTYPE) * proposal_variance
        pre_flow = proposal_mean + tf.sqrt(proposal_variance) * proposal_noise[:, time_index, :, :]
        ...
        residual = observation[0] - tf.constant(2.0, dtype=DTYPE) * tf.math.log(beta) - target_state
        target_observation = value_mod._ksc_mixture_log_density(  # noqa: SLF001
            residual,
            mixture_weights,
            mixture_means,
            mixture_variances,
    ...
    *,
    ...
    artifact = {
        ...
        "score_derivative_provenance": str(
            base.get("score_route", KSC_SV_COMPACT_SCORE_ROUTE_ID)
        ),
    ...
    artifact.update(
        {
            ...
            "target_semantics": {
                ...
                "transform_offset": value_mod.TRANSFORM_OFFSET,
                "flow_is_proposal_surface_only": True,
                "ksc_mixture_used": True,
