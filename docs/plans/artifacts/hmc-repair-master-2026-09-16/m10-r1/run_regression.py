"""M10 fresh no-hint serious fits with finite-window metric proposals."""
import argparse
...
def worker(args,output):
    ...
    seed = seed_for(2026092040,"m10-finite-window",args.case,args.phase,args.replication,"tuning")
...
def main():
    ...
    output = ROOT/f"regression-finite-window-{args.phase}-{args.replication}-gpu-r1"
    if args.worker:
        try:
            worker(args,output)
    ...
    if len(costs) >= 10 or sum(r["elapsed_seconds"] for r in costs) + args.seconds > 9000.:
        raise RuntimeError("M10 attempt/worker-wall budget exhausted")
    if args.phase == "fresh":
        pilot = read(ROOT/"no-hint-finite-window-gpu-r1/gpu-diagnostic-run.json")
        prepared = read(ROOT/"no-hint-finite-window-gpu-r1/preparation_progress.json")
