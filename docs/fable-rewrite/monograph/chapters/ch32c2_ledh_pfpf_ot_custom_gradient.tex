...
The canonical reset in this chapter is Contract E--Chol.  In the current
repository policy, the canonical reset identifier is
\texttt{contract\_e\_chol\_v1}, and the canonical total-derivative composition
identifier is
\texttt{contract\_e\_chol\_total\_direct\_moments\_weights\_plus\_streaming\_transport\_v1}.
A raw barycentric cloud, compact forward sensitivity of that raw scalar, or
full-history/manual reverse derivative of that raw scalar is historical
diagnostic evidence only; the repository route label for those historical value-
correct but noncanonical reset semantics is
\texttt{historical\_raw\_barycentric\_diagnostic\_only}.  Even an algebraically
correct derivative of the raw-barycentric scalar is wrong relative to the
canonical Contract E total-gradient claim.  There is no canonical-to-raw
fallback.
...
\paragraph{Teacher-projection status note.}
The name \emph{Contract E--teacher projection} is a convenience label for this
chapter’s extension route, not a canonical, leaderboard, default, or HMC-facing
identity.  Any release wording must preserve that distinction.
...
annealing schedule, iteration budget, the repository-selected row/column chunk
extent recorded for reproducibility, stabilization, and stop-gradient policy.
For active canonical and candidate routes, the recorded chunk fields must equal
the repository chunk-policy selector output from
\texttt{dpf\_transport\_exact\_divisor\_cap3000\_v1}; they are provenance
fields, not caller-chosen tuning knobs.  The selector requires equal row and
column chunks, \(K=N\) for \(N\le 3000\), and for larger particle counts the
largest divisor of \(N\) not exceeding 3000, with fail-closed behavior if no
such divisor exists.  The canonical LEDH route also requires a repository-issued
per-scope tuning artifact: any claim-bearing run must use controls tuned for the
exact model/route/horizon/particle-count/dtype/chunk-policy scope, and a setting
borrowed from another scope is only a warm start, not a default.  The auxiliary
record should contain only mathematical
...
is why the custom-gradient design in this chapter emphasizes the OT VJP.  The
practical forward-versus-reverse choice also depends on memory traffic,
batching, output shape, and compiled-graph structure, not on parameter
dimension alone.  Separately, if any executed dependency is wrapped in a
stop-gradient, the returned cotangent is a partial derivative of the same
finite program with that dependency held constant; it equals the total
derivative only when the omitted chain contribution is exactly zero, and the
text must say which of those two objects is being reported.
