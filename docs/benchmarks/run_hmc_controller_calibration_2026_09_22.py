"""Fresh actual-controller diagnostic; see the M25 gap-closure plan."""
import argparse
import hashlib
import json
import math
...
def main():
    ...
    p.add_argument('--method',choices=('lugsail','autocorrelation'),default='autocorrelation')
    ...
    p.add_argument('--counts',type=Path,help='predeclared posterior and fixed-arm counts')
    p.add_argument('--max-results-per-chain',type=int,default=10000)
    p.add_argument('--count-budget-reason')
    a=p.parse_args()
    assert a.replications>0 and a.seconds>0
    counts = dict(warmup_chunk_results=500,warmup_min_results=2000,warmup_check_window_results=1000,
        warmup_max_results=10000,retained_chunk_results=500,retained_min_results=1000,
        retained_max_results=10000,fixed_warmup=10000,fixed_retained=10000)
    if a.counts is not None:
        overrides = json.loads(a.counts.read_text())
        if set(overrides) - counts.keys():
            raise ValueError('unknown posterior count fields')
        counts.update(overrides)
    if any(type(value) is not int or value <= 0 for value in counts.values()):
        raise ValueError('positive integer counts required')
    controller_counts = {key:value for key,value in counts.items() if not key.startswith('fixed_')}
    ...
    for relative,digest in source_manifest['files'].items():
        if hashlib.sha256((a.source/relative).read_bytes()).hexdigest()!=digest:
    ...
    policy=HMCPosteriorAssessmentPolicy(precision=HMCPrecisionPolicy(
        (HMCPrecisionTarget('x',mcse_absolute_max=.05),),method=a.method,jit_compile=gpu))
    rho,regime=CASES[a.case]
    transition=GaussianAR1Transition(rho,jit_compile=gpu)
    manifest={'command':[sys.executable,*sys.argv],'python':sys.executable,'tensorflow':tf.__version__,'tfp':tfp.__version__,
        'source':str(a.source.resolve()),'source_manifest':str(a.source_manifest),
        'source_identity':source_manifest['source_identity'],
        'git_commit':source_manifest['git_commit'],
        'method':a.method,
        'driver_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
        'plan':'docs/plans/bayesfilter-hmc-gap-closure-continuation-2026-09-22.md',
        ...
        'planned_replications':a.replications,'maximum_seconds':a.seconds,
        'counts':counts,'counts_path':str(a.counts) if a.counts else None,
        'counts_sha256':hashlib.sha256(a.counts.read_bytes()).hexdigest() if a.counts else None,
        'max_results_per_chain':a.max_results_per_chain,'count_budget_reason':a.count_budget_reason,
        'environment':{k:os.environ.get(k) for k in ('CUDA_VISIBLE_DEVICES','TF_FORCE_GPU_ALLOW_GROWTH','TF_NUM_INTRAOP_THREADS','TF_NUM_INTEROP_THREADS','OMP_NUM_THREADS','OPENBLAS_NUM_THREADS')},
        'data_version':'synthetic exact Gaussian AR1 law in this manifest','actual_controller_stopping_test':True,
    ...
    rows=[]
    for rep in range(a.replications):
        ...
        try:
            config=SequentialExactTransitionConfig(transition.signature,streams['warmup'],streams['retained'],
                **controller_counts,assessment_policy=policy,max_results_per_chain=a.max_results_per_chain,
                count_budget_reason=a.count_budget_reason)
            ...
            stopped=mean_interval(result['private_retained_beta_one'],jit_compile=gpu,method=a.method)
            ...
            if time.monotonic()<deadline:
                ...
                fixed_warmup=transition(fixed_initial,num_results=counts['fixed_warmup'],seed=tf.constant(streams['fixed_warmup']),stage='fixed')['posterior_samples']
                fixed=transition(fixed_warmup[-1],num_results=counts['fixed_retained'],seed=tf.constant(streams['fixed_retained']),stage='fixed')['posterior_samples']
                write_tensor(root/'fixed-warmup.tensor',fixed_warmup);write_tensor(root/'fixed-retained.tensor',fixed)
                law=fixed_mean_law(rho,[float(x) for x in tf.unstack(fixed_initial[:,0])],warmup=counts['fixed_warmup'],draws=counts['fixed_retained'],stationary_start=regime=='stationary')
                estimate=float(tf.reduce_mean(fixed))
                row.update(fixed=mean_interval(fixed,jit_compile=gpu,method=a.method),fixed_exact_law=law,
    ...
    calibration=summarize_controller_records(rows,a.replications,oracle_alpha=.05/2,coverage_floor=.90)
    summary={'calibration':calibration,'manifest':manifest,'rows':rows,'completed_replications':sum(r['status']=='complete' for r in rows),
