...
self-normalized program.  A new two-state Gaussian campaign finds higher
model-score mean squared error for the calibrated repaired program in both
tested particle/horizon scopes, with paired intervals supporting the losses.
The derivative checks pass, but score improvement does not follow.  Neither
route is the derivative of the unchanged atom program.  Extending the method
to parameter-dependent DSGE support also requires a state-motion derivative;
a volume Jacobian alone is insufficient.
\end{abstract}
...
now implements \eqref{eq:affine-chart-state}--\eqref{eq:affine-chart-observation},
...
model-score error under fresh model-specific calibration.  An existing model
