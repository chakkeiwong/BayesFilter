...
output_root="${repo_root}/docs/plans/artifacts/ssl-lstm-q20-tempered-rkl-transport-ensemble-2026-09-04/phase9a-full-replay-gpu10000"
...
case "${profile}:${scope_start}:${scope_limit}" in
  phase9a_full_replay_canary_gpu10000_v1:3:1)
    material_cap_seconds=1800
    ;;
  phase9a_full_replay_gpu10000_v1:0:6)
    material_cap_seconds=10000
    ;;
  *)
    printf '%s\n' \
      'profile/scope combination must be either' \
      'phase9a_full_replay_canary_gpu10000_v1 with scope 3/1 or' \
      'phase9a_full_replay_gpu10000_v1 with scope 0/6' >&2
    exit 2
    ;;
esac
...
# The repository GPU policy requires on-demand allocation before TensorFlow
# import.  The runner also records and verifies this setting.
