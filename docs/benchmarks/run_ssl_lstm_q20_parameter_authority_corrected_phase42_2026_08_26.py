"""Replicate the frozen-training theta audit on two independent banks.

Each arm creates one trainer, trains only on the frozen v2.2 training split,
and evaluates both fresh banks only after the final optimizer update.  The
phase is diagnostic and does not launch HMC.
"""
...
if os.environ.get("CUDA_VISIBLE_DEVICES") == "-1":
    raise RuntimeError("Phase 42 requires a visible trusted GPU")
if os.environ.get("TF_FORCE_GPU_ALLOW_GROWTH") != "true":
    raise RuntimeError("Phase 42 requires TF_FORCE_GPU_ALLOW_GROWTH=true")
...
SPEC = importlib.util.spec_from_file_location("phase31_helpers_phase42", HELPER_PATH)
...
EXPECTED_SCHEMA = "bayesfilter.ssl_lstm.q20.corrected_theta_neutra_boundary.v5_two_independent_banks"


class Phase42Error(RuntimeError):
    """Raised when the two-bank boundary cannot be audited."""
...
def _write_json(path: Path, payload: Mapping[str, Any]) -> None:
    if path.exists():
        raise Phase42Error(f"refusing to overwrite artifact: {path}")
...
def _receipt(root: Path, name: str) -> tuple[Path, Mapping[str, Any]]:
    if root.is_absolute() or ".." in root.parts:
        raise Phase42Error(f"path must be repository-relative: {root}")
    path = ROOT / root / name
    if not path.is_file():
        raise Phase42Error(f"missing receipt: {path}")
...
def _load_tensor(receipt: Mapping[str, Any]) -> tf.Tensor:
    ...
    if hashlib.sha256(encoded).hexdigest() != str(receipt["sha256"]):
        raise Phase42Error(f"tensor hash mismatch: {path}")
...
def _cloud(arm: Mapping[str, Any]) -> Mapping[str, tf.Tensor]:
    ...
    if values["theta"].shape.rank != 2 or values["theta"].shape[1] != 4:
        raise Phase42Error(f"theta shape is not [N,4]: {values['theta'].shape}")
    if values["weights"].shape != (n,) or values["roots"].shape != (n,):
        raise Phase42Error("cloud vector shape mismatch")
...
def _run_arm(
    *,
    ...
    # Both independent banks are observed only after the last update, using
    # the same trainer state and no checkpoint-selection branch.
    audits: dict[str, Any] = {}
    ...
    return {
        "status": "PASS_NEUTRA_BOUNDARY_CANDIDATE" if all(gates.values()) else "PHASE42_CANDIDATE_FAIL_REPAIR",
        ...
        "one_trainer_state_for_both_banks": True,
        "nonclaims": [
            "The two fresh banks are finite audits, not IID or posterior proofs.",
...
def _markdown(result: Mapping[str, Any]) -> str:
    ...
    for key, arm in result["arms"].items():
        ...
        lines.append(
            f"| {key} | {arm['precondition']} | `{arm['status']}` | {float(a['latent_mean_max_abs']):.6f} / {float(a['latent_covariance_max_abs_offdiag']):.6f} | {float(b['latent_mean_max_abs']):.6f} / {float(b['latent_covariance_max_abs_offdiag']):.6f} |"
        )
    lines.extend([
        "",
...
def main() -> int:
    ...
    if int(args.steps) <= 0:
        raise Phase42Error("steps must be positive")
    for path in (args.authority_root, args.fresh_root_a, args.fresh_root_b, args.output_root):
        if path.is_absolute() or ".." in path.parts:
            raise Phase42Error("all paths must be repository-relative")
    output = ROOT / args.output_root
    if output.exists():
        raise Phase42Error(f"refusing to overwrite output root: {output}")
    ...
    pilot_paths = (authority_path, fresh_a_path, fresh_b_path)
    if len({_sha(path) for path in pilot_paths}) != 3:
        raise Phase42Error("authority and fresh pilot receipts are not all distinct")
    ...
    old = _cloud(old_m0)
    banks = {"bank_a": _cloud(fresh_a_m0), "bank_b": _cloud(fresh_b_m0)}
    tensor_hashes: dict[str, dict[str, str]] = {}
    for label, arm in (("authority", old_m0), ("bank_a", fresh_a_m0), ("bank_b", fresh_b_m0)):
        tensor_hashes[label] = {
    ...
    for label, arm in (("authority", old_c0), ("bank_a", fresh_a_c0), ("bank_b", fresh_b_c0)):
        c0_tensor_hashes[label] = {
            key: str(arm["receipts"][key]["sha256"])
            for key in ("final_theta", "final_normalized_weights", "final_roots")
        }
    for key in tensor_hashes["authority"]:
        if len({tensor_hashes[label][key] for label in tensor_hashes}) != 3:
            raise Phase42Error(f"tensor hash collision for {key}")
        if len({c0_tensor_hashes[label][key] for label in c0_tensor_hashes}) != 3:
            raise Phase42Error(f"C0 tensor hash collision for {key}")
    ...
    if not all(split.get(key) is True for key in ("root_disjoint", "row_partition_complete", "row_partition_disjoint")):
        raise Phase42Error("frozen root-group split invariant failed")
    ...
    result = {
        ...
        "status": "PASS_V2_4_TWO_BANK_BOUNDARY" if candidate_pass else "PHASE42_CANDIDATE_FAIL_REPAIR",
        "plan_version": EXPECTED_VERSION,
        "role": "frozen_v2_2_training_measure_two_independent_fresh_theta_audits",
        ...
        "arms": arms,
        "one_trainer_state_per_arm_for_both_banks": True,
        "fresh_rows_used_for_training": False,
        "fresh_rows_used_for_selection": False,
        "hmc_launched": False,
        "device": {
        ...
        "nonclaims": [
            "Two fresh banks are finite audits, not an IID or posterior proof.",
