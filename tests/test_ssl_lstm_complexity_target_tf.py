...
def test_q1_directional_target_matches_locked_target() -> None:
    ...
    for offset in (0.0, 0.1, -0.15):
        ...
        tf.debugging.assert_near(actual_value, expected_value, atol=1e-10, rtol=1e-10)
        tf.debugging.assert_near(actual_score, expected_score, atol=1e-9, rtol=1e-9)
...
def test_selected_score_equals_full_score_at_q2() -> None:
    ...
    selected, _ = tf_ssl_lstm_svd_ukf_score(
        target.config.observations,
        full,
        target.config.static_config,
        evidence_path=PLAN,
        derivative_parameter_indices=target.config.free_indices,
        spectral_gap_tolerance=tf.constant(1e-10, tf.float64),
    )
    complete, _ = tf_ssl_lstm_svd_ukf_score(
        target.config.observations,
        full,
        target.config.static_config,
        evidence_path=PLAN,
...
def test_q2_directional_score_matches_finite_difference() -> None:
    ...
    for index in range(4):
        ...
        plus = target.value_and_score(point + step * direction)[0]
        minus = target.value_and_score(point - step * direction)[0]
        finite.append((plus - minus) / (2.0 * step))
    tf.debugging.assert_near(score, tf.stack(finite), atol=8e-4, rtol=5e-3)
