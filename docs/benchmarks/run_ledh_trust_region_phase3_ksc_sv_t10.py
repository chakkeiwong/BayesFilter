#!/usr/bin/env python3
"""LEDH Trust-Region Phase 3: Tuning and Safety Evaluation - KSC SV T10.
...
DUAL_CAP_ARTIFACT = Path("docs/benchmarks/artifacts/genut_four_model_leaderboard_rerun_20260816/single_ksc_sv_T10_dual_cap/result.json")
...
def _build_ksc_sv_target() -> dict[str, Any]:
    """Build KSC SV T10 target from base module."""
    targets = base._build_targets()
    ksc_sv = targets["ksc_sv_T10"]

    # Add horizon field for evaluator
    horizon = int(ksc_sv["observations"].shape[0])
    ksc_sv["horizon"] = horizon
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("docs/benchmarks/artifacts") / f"ledh_trust_region_ksc_sv_t10_{time.strftime('%Y%m%d')}",
