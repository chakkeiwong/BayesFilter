...
def _require_compact_score_args(args: argparse.Namespace) -> None:
    require_historical_raw_diagnostic_opt_in(
        args, route_name="generalized-SV raw value/score route"
    )
...
def _prepare_compact_xla_inputs(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if len(args.batch_seeds) != 1:
        raise ValueError("generalized-SV XLA score shards require exactly one seed")
...
def _score_artifact_from_diagnostic(
    ...
    *,
    ...
    if not isinstance(base_raw, Mapping):
        raise ValueError("generalized-SV compact diagnostic must include base mapping")
    base = base_raw
    if base.get("score_route") != GENERALIZED_SV_COMPACT_SCORE_ROUTE_ID:
        raise ValueError("generalized-SV compact diagnostic must use compact score route")
    if base.get("no_autodiff_score_route") is not True:
        raise ValueError("generalized-SV compact diagnostic must declare no_autodiff_score_route")
    if base.get("value_score_route_status") != LEDH_SCORE_VALUE_ROUTE_STATUS_SAME:
        raise ValueError("generalized-SV compact diagnostic must be same_route_value_score")
    if tuple(diagnostic.get("parameter_names", ())) != tuple(PARAMETER_NAMES):
        raise ValueError("generalized-SV compact diagnostic parameter_names must match parameter order")
    score = tf.convert_to_tensor(base["gradient_tensor"], dtype=DTYPE)
    ...
    if require_all_parameter_correctness:
        ...
        if int(base.get("num_particles", -1)) != int(value_core["num_particles"]):
            raise ValueError(
                "generalized-SV compact full admission requires N=10000 diagnostic shape"
            )
        if int(base.get("time_steps", -1)) != int(value_core["time_steps"]):
            raise ValueError("generalized-SV compact full admission requires full time_steps")
        if tuple(int(seed) for seed in base.get("batch_seeds", ())) != tuple(
            int(seed) for seed in value_core["batch_seeds"]
        ):
            raise ValueError("generalized-SV compact full admission requires full batch_seeds")
    artifact = {
    ...
    if require_all_parameter_correctness:
        ...
        raise ValueError(
            "generalized-SV raw-barycentric score route is historical diagnostic only"
        )
    return artifact
