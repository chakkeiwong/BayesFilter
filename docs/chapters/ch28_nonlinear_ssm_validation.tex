...
The analytic first-order score is currently certified only on a smooth affine
fixture with an explicit structural derivative provider.  Models B--C remain
value-only for analytic-score purposes until their transition and observation
Jacobian and parameter-derivative providers are added.  Consequently, GPU/XLA
and HMC evidence for nonlinear Models B--C is deliberately deferred.
