"""M9 complete no-hint regression fits with both optional repairs."""
import argparse
...
def worker(args,output):
    ...
    write_json(output/"manifest.json",{"source":source,"runtime":runtime,"command":sys.argv,
        ...
        "geometry_hint":None,"search_policy":"same operational preparation; both repairs; two refinement rounds"})
...
def main():
    ...
    output = ROOT/f"regression-combined-{args.phase}-{args.replication}-gpu-r1"
    if args.worker:
        try:
            worker(args,output)
