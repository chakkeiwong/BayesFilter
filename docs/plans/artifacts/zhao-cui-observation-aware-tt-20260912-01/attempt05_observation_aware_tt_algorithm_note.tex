...
guide fails. The fitting safeguard remains empirical rather than a guarantee
that TT refinement preserves SGQF accuracy on unseen data.
\end{abstract}
...
comparisons. Clearing those simple proposals is the primary filtering
criterion; comparisons among the TT variants remain descriptive. Lower fit
discrepancy cannot replace the downstream comparison.

The completed candidate fails this criterion. In one dimension it has
...
The experiment rejects the tested candidate while leaving two distinguishable
repair questions. Signed quadrature must first provide valid moments on the
observations where the guide is needed. Separately, TT conversion and
validation selection must preserve useful accuracy already present in the
analytical joint. The current observation sequences have now been inspected;
a repair needs fresh calibration and confirmation sequences under a reviewed
protocol. A lower fitting discrepancy or a favorable table conditioned on
guide success would not answer both questions.
...
inference tables. The independent terminal interpretation review agrees with
the stated rejection and uncertainty limits. This section remains a
