...
4-arm sweep: 1×, 10×, 100×, 1000× damping ratios.
Target: LGSSM T=50, d=3, N=5000 particles.
Protocol: 4 chains × 10k warm-up + 10k retained per arm.
Metrics: Acceptance rate ≥ 0.15, ESS/grad ≥ 0.3× baseline.
"""
...
def main():
    ...
    d = 3
    N = 1000  # Reduced from 5000 to avoid OOM
    ...
    # True theta (LGSSM parameters: [sigma_x, sigma_y, sigma_z, rho_xy, rho_xz])
    ...
    print(f"\n[6/6] Summary:")
    print("\n  Damping  Accept  ESS/param  Wall Time")
    print("  -------  ------  ---------  ---------")
    ...
    for ratio in damping_ratios:
        ...
        print(f"  {ratio:6.0f}×  {mean_accept:.3f}  {mean_ess:9.0f}  {mean_time:7.1f}s  "
              f"(ESS ratio: {ess_ratio:.2f})")

    print("\n  Acceptance rate target: ≥ 0.15")
    print("  ESS/grad target: ≥ 0.3× baseline")
