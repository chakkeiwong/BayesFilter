...
def worker(args, output):
    ...
    config = TensorFlowHMCKernelTuningConfig(parameter_dimension=target.parameter_dim,
        ...
        verification_repair_rounds=0, step_repair_factor=2., mass_shrinkage=.10,
    ...
    result = {"case": args.case, "completion": resumed.result.completion_status,
        ...
        "observations": len(resumed.result.observations), "resumed_prefix_unchanged": unchanged,
...
def main():
    ...
    output = ROOT / ("position-field-" + args.device + "-" + args.case + "-r1")
    if args.worker:
        try:
            worker(args, output)
