...
For actual-SV rows, this chapter must now be read with a stricter boundary.
The fixed-cloud construction below defines a deterministic Gaussian-projection
scalar: the cloud approximates predictive and observation moments, and the
resulting one-step scalar is the Gaussian innovation log likelihood in
\eqref{eq:bf-hd-sgqf-loglik}.  That construction is mathematically valid as a
Gaussian-projection filter description, but the previous actual-SV ``Lane~B''
framing was wrong for the intended inference target.  For transformed actual SV,
the intended one-step normalizer comes from the exact transformed observation law,
so a route that replaces that law by Gaussian closure is not a co-equal lane of
the same problem.  The fixed-cloud machinery remains useful only when the scalar
it differentiates is already the right target.
...
\end{align}
This is the one-step scalar exported by the Gaussian-projection value path.  It
is defined by predictive and observation moments \(\bar z_t\), \(S_t\), and
\(C_{xz,t}\), not by direct quadrature of the exact normalizer
\(Z_t(\theta)\).  That distinction is decisive for actual SV: if the intended
observation law is the exact transformed law
\(
 z_t=\log(y_t^2),
 z_t-\log(\beta^2)-h_t\sim\log(\chi_1^2)
\),
then the fixed cloud can support the intended likelihood only inside a
same-target direct reweighting path whose one-step scalar is the exact
transformed-law normalizer; the Gaussian innovation scalar
\eqref{eq:bf-hd-sgqf-loglik} is not that normalizer.  The older
actual-SV use of \eqref{eq:bf-hd-sgqf-loglik} as a standalone Lane-B likelihood
was therefore wrong, not merely an alternative convention.
...
the reader back to the earlier derivation.  The key runtime objects are:
\begin{center}
\small
