#!/usr/bin/env python3
"""Confirm support-eligible SSL-LSTM NeuTra selection on untouched streams."""
...
SCHEMA = "bayesfilter.ssl_lstm_neutra.support_eligible_confirmation.v2"
POLICY_SCHEMA = "bayesfilter.ssl_lstm_neutra.support_eligible_policy.v2"
PLAN_PATH = Path(
...
DEFAULT_OUTPUT_ROOT = Path(
    ...
    "support-eligible-confirmation-2026-07-16"
)
...
FRESH_STREAMS = (
    FreshStream("fresh-e", (20260716, 7103), (20260716, 8103), (20260716, 8203)),
    FreshStream("fresh-f", (20260716, 7104), (20260716, 8104), (20260716, 8204)),
)
...
def load_policy(path: Path) -> dict[str, Any]:
    ...
    parent_policy = ROOT / str(payload.get("parent_policy_path"))
    if not parent_policy.is_file():
        raise ConfirmationError("parent tuning policy is unavailable")
    if sha256(parent_policy) != payload.get("parent_policy_sha256"):
        raise ConfirmationError("parent tuning policy hash mismatch")
    parent_payload = json.loads(parent_policy.read_text(encoding="utf-8"))
    if payload.get("selected_hyperparameters") != parent_payload.get(
        "selected_hyperparameters"
    ):
        raise ConfirmationError("repaired policy changed selected hyperparameters")
    parent_plateau = parent_payload.get("plateau_policy", {})
    ...
    if any(
        ...
        raise ConfirmationError("repaired policy changed the parent plateau schedule")
