...
| --- | --- | --- |
| `fixed_center_curvature.consensus_shrunk_precision` | Python matrix symmetrization and eager final eigenvalue check repaired in this checkpoint.19 CPU checks pass in03291 and03293; GPU preflight declines. | One stable XLA call now includes validation, symmetrization, consensus and SPD decision. Pinned3582b4ac values/errors, unchanged-input HLO, changed operands and derivatives pass. Qualify GPU, preserve the new nonfinite-output veto, and include the endpoint in final source renewal. |
...
Their previous rationale said optimizer-wide XLA was outside F18. That stale
scope assertion is removed. It cannot exempt the numerical stages above, and
the exception count/AST permissions have not increased.

The pending initializer agreement and GPU availability do not prevent source
review or preparation of these segments. Before each segment's numerical unit,
freeze its exact original authority and consumer cases and allocate a bounded
worker reservation from the unchanged campaign caps. A Python API that cannot
be represented by a tensor contract requires a documented boundary; replacing
its scientific behavior or dropping callbacks is not an execution repair.

The other partially guarded modules remain under contextual review. For example,
`source_route_negative_log_assembly_derivative_carry` iterates over three binding
identities and then uses direct tensor arithmetic. That loop is metadata
validation, but its presence does not establish an enclosing XLA caller.
`_p59_fixed_ttsirt_transport_from_values` builds basis topology and copies fit
reports while invoking a numerical fitter; the numerical call chain still needs
its own evidence. Austria training helpers pack heterogeneous cores and tangent
banks; they must be classified by actual operation and caller rather than by a
function name. No Zhao-Cui source-faithfulness claim or algorithm change follows
from this review.

Review outcome: continue the existing repair program. No F01--F20 terminal
finding closes on these observations. The complete endpoint inventory, source
renewal, qualified GPU evidence/costs, pending public initializers, actual DZ5
integration and remote integration/retest remain mandatory before main merge.
