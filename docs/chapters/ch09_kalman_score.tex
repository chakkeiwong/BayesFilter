...
\label{eq:bf-score-predicted-cov-first}
  ...
  +\dot Q_t^{(i)}.
\end{align}
These are source labels \texttt{eq:dm\_pred\_note} and
\texttt{eq:dP\_pred\_note} in the MacroFinance analytic derivative note.  They
also define the initial-condition contract: if the implementation fixes
$(m_{0|0},P_{0|0})$ as data, the initial derivatives are zero; if it uses a
stationary initial covariance, the derivatives must come from the corresponding
derivative Lyapunov equations and be reported as part of the structural
provider.
...
\label{eq:bf-score-innovation-cov-first}
  ...
  +\dot R_t^{(i)}.
\end{align}
These are source labels \texttt{eq:dv\_note} and \texttt{eq:dS\_note}.
...
reference is the MacroFinance solve differentiated Kalman backend.  The trace
term may be computed by explicit materialization in small tests, but production
code should prefer factor solves or trace estimators appropriate to the
dimension and backend.
