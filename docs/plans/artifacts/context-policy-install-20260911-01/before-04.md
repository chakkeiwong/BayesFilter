...
- If multiple reasonable interpretations exist, surface them instead of silently choosing.
- If a simpler approach would work, prefer it and say so.
- If something material is unclear, stop and clarify before changing code.

### Simplicity first
- Implement the minimum change that solves the requested problem.
- Do not add features, abstractions, configurability, or validation that were not requested.
...
- Do not refactor, reformat, or rewrite adjacent code unless necessary.
- Match existing local style and patterns.
- Remove only the unused code created by your own change; mention unrelated cleanup separately.

### Goal-driven execution
- Define concrete success criteria before non-trivial changes.
- Verify the result with the right check for the task: tests, numerical checks, script output, or build/compile validation.
- For multi-step tasks, keep a short plan with a verification step for each part.

### Skeptical plan audit before execution
- Before executing any non-trivial plan, audit it as a skeptical developer first. Do this before running implementation, experiment, benchmark, or long diagnostic commands.
- The audit must explicitly look for wrong baselines, proxy metrics being treated as promotion criteria, missing stop conditions, unfair comparisons, hidden assumptions, stale context, environment mismatches, and commands whose artifacts would not answer the stated question.
- If the audit finds a material flaw, do not run the plan yet. Revise the plan, document the issue, or ask for direction when the correct fix is not clear.
- If the audit passes, record the reason briefly in the plan, reset memo, or execution note before proceeding.
- This audit is required even when the user asks to "execute the plan"; execution begins after the plan survives the skeptical audit.

### Evidence and uncertainty discipline
- Preserve uncertainty explicitly. Label claims as assumptions, hypotheses, smoke evidence, engineering evidence, empirical scientific evidence, or mathematical claims as appropriate.
- Do not present a working harness, passing smoke test, or short diagnostic run as proof of correctness, convergence, or scientific validity.
- When interpreting results, state what the evidence supports, what it does not support, and what observation would change the conclusion.
- Make AI advice earn trust with artifacts. Prefer recommendations backed by a diff, reproducible command, test result, benchmark table, failing case, reset memo, or documented experiment note.
- If advice cannot yet be tied to an artifact, mark it as a hypothesis or proposal and give the smallest next artifact that would test it.

### Academic research threat model
- This is a trusted academic economic-research repository, not a service that executes adversarial user-supplied programs. For project-authored code and reviewed in-repository model definitions, optimize controls for scientific correctness, numerical validity, reproducibility, accidental-regression detection, and honest claims.
- Do not impose security-style proof obligations against a malicious repository contributor unless the scoped workflow actually ingests untrusted code, graphs, checkpoints, serialized tensors, plugins, datasets with executable content, or external artifacts. Record the applicable trust boundary before designing provenance or validation gates.
- For trusted in-repository TensorFlow/JAX/PyTorch policies, exact source revision, configuration, graph or compiled representation when practical, variable schema and values/checkpoint, seeds, environment, data bindings, and run artifacts are reproducibility evidence. Source-file lists may be declared provenance; do not claim they mechanically prove complete callable ancestry unless that property is actually implemented.
- Fixed graph constants and ordinary framework conversions are not failures merely because they are not trainable variables. They become scientific or engineering vetoes only when they change the declared model specification, conceal an unintended free parameter, break differentiation/determinism, evade artifact binding, or enter from an untrusted source.
- Trusted learned scientific components must still test declared inputs/outputs, structural identities, trainable and nontrainable state, gradient connectivity, determinism where required, RNG and mutation policy, row/shock locality, dtype/shape, finiteness, serialization/reload parity when applicable, and downstream behavior required by the scientific claim.
- Untrusted external models, graphs, checkpoints, serialized tensors, or executable payloads remain fail-closed until separately audited or converted through a reviewed safe ingestion path. Do not generalize the trusted-repository exception to those inputs.
- A stricter implementation may be retained as defense in depth if it is cheap and compatible, but it must not become a scientific promotion criterion or phase blocker without a threat-model justification. Distinguish reproducibility provenance from adversarial tamper resistance.

### Epistemic contract before research actions
- Before non-trivial experiments, sweeps, HMC/MCMC comparisons, surrogate evaluations, or default-policy changes, state the evidence contract before executing. If the contract cannot be stated clearly, stop and write or revise the plan before running commands.
- The evidence contract must explicitly include:
  - What is the scientific or engineering question?
  - What is the exact baseline or comparator?
  - What is the primary promotion or pass/fail criterion?
  - Which diagnostics can veto, and under what condition?
  - Which diagnostics are explanatory only?
  - What would not be concluded even if the run passes?
  - What exact artifact will preserve the result?
- Treat this contract as a required pre-run checklist for research-grade runs, not as an optional summary after the run.
- Do not let proxy metrics silently become promotion criteria. Validation loss, probe-point accuracy, heldout residuals, shell Hessians, replay diagnostics, smoke tests, and short chains can nominate, explain, or veto only under a stated contract; they do not by themselves establish posterior correctness, convergence, or production readiness.
- For learned components used inside Bayesian computation, distinguish "good predictor" from "good sampler component." Promotion normally requires posterior/HMC/reference evidence unless the plan explicitly justifies a different primary criterion.
- Before broad sweeps, state the rival explanations that could account for the result, such as tuning, Monte Carlo variance, budget mismatch, reference mismatch, seed effects, or implementation error.
- Treat default changes as a higher evidence bar than optional features. A promising result may justify an optional path while still being insufficient for a new default.

### Research-engineering workflow
- Prefer the smallest focused diagnostic before a full ladder, sweep, or long benchmark.
- Before any long or research-decision-making run, create or fill an experiment plan under `docs/plans`.
- Use an experiment plan before:
  - runs expected to take more than about five minutes,
  - ladder, sweep, or grid comparisons,
  - HMC/MCMC diagnostics intended to guide research direction,
  - changes to default numerical policy,
  - comparisons of objectives, architectures, force weights, samplers, or target variants,
  - interpreting results as evidence for a scientific claim.
- A quick command may skip the experiment template only when it is a smoke test, import check, compile check, shape check, or explicitly debugging-only run that will not be used for a research decision.
- Each experiment plan should state the question, mechanism tested, success criteria, diagnostics, failure modes, what would change the next step, and exact commands/environment.
- After a meaningful run, record the command actually run, diagnostics, interpretation, decision, and next step in the experiment note, reset memo, or result note.
- Separate engineering observations, empirical scientific evidence, and mathematical claims. Do not upgrade one category into another without the required evidence.
- Do not treat short diagnostic HMC runs as convergence evidence unless the experiment plan explicitly justifies that interpretation.
- Write or update a reset memo after meaningful state changes: policy changes, nontrivial blocker resolutions, experimental direction changes, or context a future agent would otherwise rediscover.
- Periodically red-team AI-generated docs for unsupported claims, stale assumptions, and mismatch with current code.
- Use `MathDevMCP` when planning or auditing mathematical/numerical work that depends on derivations, LaTeX documentation, shape semantics, code-document consistency, Kalman/HMC formulas, or proof-style obligations. It is available as a Codex MCP server named `mathdevmcp`; the CLI fallback is:
  `PYTHONPATH=/home/ubuntu/python/MathDevMCP/src /home/ubuntu/.venvs/mathdevmcp-mcp/bin/python -m mathdevmcp.cli ...`.
- Use `ResearchAssistant` when planning requires literature context, source-paper inspection, citation neighborhoods, local paper summaries, or claim-support audits. It is installed from `/home/ubuntu/python/ResearchAssistant`; prefer explicit local workspaces, for example:
  `ra --root /tmp/ra-bayesfilter-planning doctor`.
- For BayesFilter, SVD-filter, analytic Kalman derivative, and large-model HMC planning, first consider whether `MathDevMCP` can audit the math/code contract and whether `ResearchAssistant` can ground the literature trail before writing the plan.

### Scientific coding development policies
- Serious numerical, statistical, sampler, surrogate, and research-method result notes must include a decision table: decision, primary criterion status, veto diagnostic status, main uncertainty, next justified action, and what is not being concluded.
- Serious runs must include a run manifest with the git commit, command, environment or conda env, CPU/GPU status, data version when applicable, random seeds, wall time, output artifact paths, plan file, and result file. Use `N/A` only when the field genuinely does not apply.
- Method comparisons must use a baseline ladder when applicable: naive baseline, best tuned classical baseline, plain proposed method, and enhanced proposed method. Do not compare only against a weak or convenient baseline unless the plan explicitly justifies that scope.
- Interpret sampler and numerical-method results with veto diagnostics first. If divergences, R-hat, posterior/reference agreement, numerical validity, or other stated veto checks fail, do not rank configurations by speed, ESS/grad, validation loss, or secondary metrics as if the comparison were valid.
- Treat Monte Carlo uncertainty as part of the result. One-seed or short-run differences are diagnostic only unless the plan explicitly justifies stronger interpretation; promotion normally requires multi-seed replication, uncertainty intervals, or a documented reason those are unnecessary.
- Negative results must separate implementation failure, tuning failure, diagnostic failure, and evidence against the scientific idea. State what failed, what hypothesis is weakened, what remains viable, what would rescue the idea, and the next smallest discriminating artifact.
- Before long runs, include a pre-mortem: how the run could pass while misleading us, how it could fail for implementation or tuning reasons rather than scientific reasons, and what cheap diagnostic would distinguish those explanations.
- Keep separate ledgers for engineering correctness, numerical or sampler validity, and scientific interpretation. Do not promote evidence from one ledger into another without the required checks.
- After important results, include a post-run red-team note: strongest alternative explanation, what result would overturn the conclusion, and weakest part of the evidence.

### TensorFlow graph and compilation policy
- Repeated TensorFlow scientific kernels must execute through `tf.function` with an explicit, stable `input_signature`. Shape polymorphism and retracing must be bounded and justified. Eager execution is reserved for diagnostics, smoke checks, and documented exceptions that explain why a stable graph is unsuitable.
- TensorFlow pfor requires prior written approval. This includes direct pfor calls and implicit pfor selected by APIs such as `tf.vectorized_map`, `GradientTape.jacobian`, or `GradientTape.batch_jacobian`. The approval must explain why `tf.while_loop` or another native TensorFlow loop with a single traced body cannot satisfy the mathematical and engineering contract; state expected graph, host-memory, device-memory, and compilation complexity; and define bounded compatibility, numerical-equivalence, memory, and runtime checks. Without that approval, callers must explicitly select or implement a non-pfor derivative engine.
- Evaluate XLA for pure numerical hot kernels. Enable `jit_compile=True` only after compatibility, peak host and device memory, numerical equivalence, compilation cost, steady-state performance, and downstream scientific checks pass under a recorded evidence contract. An XLA failure or regression is a documented exception, not authority to fall back to an unreviewed eager or pfor path.
- Keep provenance, strings, file I/O, Python callbacks, artifact writing, and validation that XLA cannot safely compile outside the pure numerical kernel. Do not describe a traced subcomponent or pfor-created internal function as XLA compilation of the enclosing scientific step.

### GPU/CUDA sandbox policy
- Any command that detects, initializes, benchmarks, or uses GPU/CUDA/NVIDIA devices must be run with escalated sandbox permissions. This includes `nvidia-smi`, `/dev/nvidia*` checks, TensorFlow/JAX/PyTorch/CuPy GPU probes, XLA-GPU tests, and GPU benchmark or HMC jobs.
- Treat non-escalated GPU failures as sandbox evidence only. Messages such as `GPU access blocked by the operating system`, missing `/dev/nvidia*`, missing `/dev/dxg`, empty TensorFlow GPU lists, or CUDA `cuInit` errors do not establish that the machine, driver, conda environment, or TensorFlow install is broken until the same check has been rerun with escalation.
- For deliberate CPU-only runs, set `CUDA_VISIBLE_DEVICES=-1` or the project-standard CPU-hiding variable before any TensorFlow/JAX/PyTorch import. CPU-only runs do not need GPU escalation, but their artifacts must say they intentionally hid GPU devices.
- Before spending time debugging GPU setup, run an escalated `nvidia-smi` and an escalated framework-specific device probe. Record whether the command was escalated in experiment notes and reset memos.

### NeuTra gate discipline
- For NeuTra/neural-transport HMC work, every plan or execution phase must begin with an explicit gate audit. If the current gate or any prior gate is not passed, stop and rewrite the plan instead of continuing to later work.
- Gate 1: replicate the plain NeuTra paper baseline first, independently with this repository's own code. The purpose of Gate 1 is to prove that the local implementation reproduces the paper's plain NeuTra results, not merely that the original Google-Research code can still run. Use canonical reverse-KL NeuTra and exact transformed-target HMC on the paper benchmark suite before treating the implementation as established. A smoke run, a single funnel screening pass, or an original-runner-only result is not enough.
- Original NeuTra paper code may be inspected and run only as source grounding, an oracle, a debugging comparator, or a reference-result generator. It must not be treated as the final Gate-1 pass artifact. Any document that says the official runner is the canonical Gate-1 baseline is superseded by this local-code replication rule.
- Gate 2: only after Gate 1 passes, test project enhancements against plain NeuTra on exactly the same paper tests, seeds, budgets, metrics, and posterior-reference checks. This includes force-aware losses, replay-tail losses, high-radius objectives, group/minimax variants, architecture changes, and related training modifications.
- Gate 3: only after Gate 2 passes, move to HMC surrogate tests for DSGE/macrofinance generalization. The surrogate gate must compare plain NeuTra and the Gate-2-promoted enhancements under exact transformed-HMC diagnostics, including divergences, acceptance, ESS/R-hat, and posterior/reference agreement.
- Do not promote or continue DSGE-like surrogate work as the mainline while Gate 1 or Gate 2 is still open. Surrogate experiments before then are historical diagnostics only unless the user explicitly overrides the gate order.

### Dynare parity prerequisite
- For any DSGE model with an applicable executable Dynare representation, independent Dynare parity is a prerequisite before scientific admission, default promotion, long training, likelihood or HMC use, or interpretation of solver comparisons.
- The Dynare representation must be independently sourced or derived from the documented model equations. It must not be generated from the local solution matrices, residual implementation, or derivatives being tested.
- Before admission, check as applicable: equation and timing correspondence; steady state and determinacy; declaration, decision-rule, state, and shock ordering; first-order policy and transition matrices; reduced-form shock impact; second-order policy and state Hessians; volatility constants; observable mappings; and selected transformed-parameter derivatives at multiple interior parameter points.
- Explicitly audit Dynare's second-order `1/2` convention, Kronecker ordering, shock scaling, and lag-state versus current-state conventions. Record all coordinate conversions and verify their reconstruction residuals.
- A failed or incomplete Dynare parity gate is a continuation veto for long scientific runs that depend on the affected reference. Passing self-residual, finite-difference, smoke, or internal cross-implementation tests cannot substitute for an independent reference.
- If Dynare is unavailable or structurally inappropriate, document why and require another genuinely independent executable reference before scientific admission or promotion. This exception does not authorize promotion using only internally generated checks.
- Passing Dynare parity establishes local agreement with the declared reference only. It does not establish global nonlinear accuracy, filtering or posterior correctness, HMC convergence, or empirical validity.
- Historical runs made before independent parity are engineering or diagnostic evidence only until their model and reference artifacts are revalidated under this gate. Ordinary unit development that makes no scientific or promotion claim may proceed while the gate is open.

### BGS inference backend policy
- The official BGS inference and HMC implementation is `/home/chakwong/python/BayesFilter`. Admitted BGS posterior, inference, HMC, sampler, tuning, admission, or candidate-runtime code must use TensorFlow tensors and TensorFlow Probability algorithms through BayesFilter.
- NumPy and SciPy are forbidden in admitted BGS posterior, inference, HMC, sampler, tuning, admission, and candidate-runtime paths. This includes preprocessing inside the target, host callbacks, `tf.numpy_function`, `tf.py_function`, handwritten leapfrog/HMC, and a local `tfp.mcmc` runner that bypasses BayesFilter.
- The repaired NumPy/SciPy BGS descriptor, state-space, likelihood, and sensitivity implementation is an independent frozen reference comparator only. It may support source-equation, Dynare, coefficient, decision-rule, state-space, likelihood, and score parity tests. It is not an implementation fallback and may not be imported by an admitted candidate runtime.
- Historical BGS Phase 15-22 sampler artifacts are diagnostic evidence only. They do not establish HMC correctness, posterior correctness, convergence, production readiness, or default authority under the BayesFilter migration.
- A BGS BayesFilter adapter remains fail-closed until graph-native TensorFlow descriptor semantics at `qz_criterium=0.999999`, gauge-free score parity, BayesFilter QR Kalman value/score parity, and the backend-policy gate pass. Do not differentiate unordered QZ/Schur vectors, use pfor, or reintroduce NumPy/SciPy to bypass a failed gate.

### Repo-specific note
- For numerical, HMC, and monograph work, verification may be a targeted experiment, diagnostic, or document build rather than a unit test.
