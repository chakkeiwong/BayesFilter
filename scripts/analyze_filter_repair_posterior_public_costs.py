"""Post-run diagnostic analysis of complete public posterior cost comparisons."""
...
def analyze(root, first, last, devices):
    groups, identities, global_identity, uuids = {}, {}, None, []
    for number in range(first, last + 1):
        directory = root / f'run-{number:05d}'
        result_path = directory / 'posterior-public-memory.json'
        ...
        assert result['schema'] == 'filter_posterior_public_cost.v1'
        assert result['numerical_authority'] == '3582b4ac' and result['mechanism_baseline'] == 'dba39e048'
        ...
        assert result['result']['accepted'] and result['changed_result']['accepted']
        ...
        assert common == global_identity, 'Unmatched source or execution environment'
    ...
    return {'schema': 'filter_posterior_public_cost_comparison.v1', 'comparisons': comparisons,
        'devices': devices, 'source_sha256': global_identity[0],
        'nonclaims': ['Three process replicates are descriptive cost/repair evidence, not a general performance ranking.',
            ...
            'This public posterior endpoint cannot close other endpoints, whole-repository compliance or HMC readiness.']}
