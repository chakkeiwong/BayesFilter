"""M17 current-source public fits to matched references; CPU/GPU evidence separate."""
import argparse
...
def worker(args,output):
    ...
    target = PosteriordbTarget(args.case,case["data"],jit_compile=jit)
    initial_position = tf.zeros(target.parameter_dim,tf.float64)
    initialization = None
    extra_sources = []
    if args.case == "sblrc-blr":
        initialization_path = ROOT.parent/"m8-r1/regression-data-initialization.json"
        ...
        if (not initialization["passed"] or initialization["reference_draws_used"]
            ...
            raise ValueError("data-only initialization does not match target")
        initial_position = tf.constant(initialization["initial_position"],tf.float64)
        value, score = target.log_prob_and_grad(initial_position)
        if not bool(tf.math.is_finite(value)) or float(tf.reduce_max(tf.abs(score))) > initialization["gradient_tolerance"]:
            raise ValueError("initialization score differs from checked data-only solution")
        extra_sources = [str(initialization_path), str(ROOT.parent/"m8-r1/initialize_regression.py")]
    seed = seed_for(2026092197,"m17-posteriordb",args.device,args.case,args.phase,args.replication,"tuning")
    cfg = HMCKernelTuningConfig(preset="serious",use_xla=jit,target_scope="inference_validation",
        metric_evidence_policy="finite_window", metric_probe_num_results=16, preparation_max_restarts=3,
        bootstrap_initialization_rounds=20,
    ...
    execution = HMCCandidateExecutionConfig(measurement_num_results=128,verification_num_results=128,
        num_warmup_steps=8,seed=seed,use_xla=jit,target_status_trace_policy="none",
        non_xla_reason="explicit M17 CPU reference" if not jit else None,
    ...
    write_json(output/"manifest.json",{"source":source,"runtime":runtime,"command":sys.argv,
        ...
        "initialization":initialization,"reference_used_for_tuning":False,"member_selection":"first verified identity before posterior sampling"})
    run = tune_hmc_kernel(adapter=target,initial_position=initial_position,
        ...
        source_paths=[__file__,str(Path(sys.modules[PosteriordbTarget.__module__].__file__)),*extra_sources])
    result = run.result
    ...
    summary = {"case":args.case,"phase":args.phase,"replication":args.replication,
        ...
        "finding":"no_verified_members","posterior_available":False,"accuracy_screen_passed":False}
    ...
    if selected and time.monotonic() < deadline:
        ...
        policy = HMCPosteriorAssessmentPolicy(precision=HMCPrecisionPolicy(
            tuple(HMCPrecisionTarget(name,mcse_absolute_max=.1) for name in target.model_names),method="lugsail",jit_compile=jit))
        config = SequentialNeuTraHMCConfig(step_size=member.step_size,num_leapfrog_steps=member.num_leapfrog_steps,
            jit_compile=jit,warmup_chunk_results=500,warmup_min_results=2000,warmup_check_window_results=1000,
        ...
        if int(draws.shape[0]) >= 4:
            ...
            ref = mean_precision(reference,method="lugsail",jit_compile=jit)
            observed = mean_precision(draws,method="lugsail",jit_compile=jit)
...
def main():
    ...
    if args.phase != "fresh" or args.replication != 0 or args.seconds > (1000 if args.device == "gpu" else 900):
        raise ValueError("unallocated matched-reference experiment")
    tag = "schools" if args.case.startswith("eight_schools") else "regression"
    output = ROOT/f"posteriordb-{tag}-{args.phase}-{args.replication}-{args.device}-r1"
    if args.worker:
        try:
            worker(args,output)
    ...
    record = {"command":command,"started_utc":when,"elapsed_seconds":time.monotonic()-started,
        "returncode":code,"device":args.device,"gpu_intentionally_hidden":args.device=="cpu_reference","environment":sys.executable,"plan_file":PLAN,
    ...
    (output/"diagnostic-run.json").write_text(json.dumps(record,indent=2)+"\n")
