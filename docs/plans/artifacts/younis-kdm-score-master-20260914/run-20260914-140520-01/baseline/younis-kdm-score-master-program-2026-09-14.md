...
At this amendment, the proposed coordinator/package/configuration paths are
absent and numerical integration obligations remain open. They are work to
perform in the master, not external preconditions for starting it. A missing
method cannot be silently removed to declare the full matrix implemented.
...
|---|---|---|
| Coordinator, study inputs, and resume/report services | 0B/0C/0G | Proposed paths remain absent; build and test them as part of master execution. |
| KDM integrated/resampling endpoint | 0D, with shared baseline regression in 0C | Static trace/callback conflict in the current scalar executor; reconcile the ongoing repair and run endpoint wiring regressions before KDM rows. |
| Parameter-dependent initialization | 0C and every affected 0D--0G adapter | Supplied initial-state/covariance tangents start at zero; implement and demonstrate full outer initialization sensitivity on a nonconstant fixture. |
| UKF/KDM/SGQF moment lifecycle | 0C--0E | Implement prediction, observation conditioning, persistent component state and reset, then test the actual moments and sensitivities consumed by LEDH. SGQF integration remains unverified. |
| Twisting/iAPF | 0E | Derive, implement, and test actual ancestor/state laws, normalizing integrals, correction and derivative terms. Preserve paper/source anchors for any faithfulness claim. |
| KDM as a LEDH control variate | 0D | Supply the center under the actual law or an independent unbiased center, including cost/variance; otherwise the exact-CV row remains blocked and any heuristic retains its own label. |
...
| Additional models, row matrix, and final reports | 0G | Build and check each adapter/reference/configuration before its study, including capacity pilots and terminal report coverage. |
