#!/usr/bin/env python3
"""LEDH Trust-Region Phase 3: Tuning and Safety Evaluation - LGSSM T50.
...
DUAL_CAP_ARTIFACT = Path("docs/benchmarks/artifacts/genut_four_model_leaderboard_rerun_20260816/single_lgssm_T50_dual_cap/result.json")
...
def _build_lgssm_target() -> dict[str, Any]:
    """Build LGSSM T50 target from base module."""
    targets = base._build_targets()
    lgssm = targets["lgssm_T50"]

    # Add horizon field for evaluator
    horizon = int(lgssm["observations"].shape[0])
    lgssm["horizon"] = horizon
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("docs/benchmarks/artifacts") / f"ledh_trust_region_lgssm_t50_{time.strftime('%Y%m%d')}",
