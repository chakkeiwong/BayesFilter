...
| --- | --- | --- | --- | --- | --- |
| Workspace hygiene | Current pass has scoped dirty files and unrelated reset-memo edits from a formal-section pass. | Scoped staging by path/hunk; reset memo phase log. | BayesFilter | `scoped_pass` | None after scoped commit. |
...
| DSGE adapter pilot | DSGE models exist in `/home/chakwong/python`, but BayesFilter must not infer structural economics. | Client model exposes explicit state names, stochastic/deterministic indices, innovation dimension, and completion map. | DSGE client repo | `blocked_pending_client_metadata` | Implement and test SmallNK/Rotemberg/SGU adapters in `/home/chakwong/python`. |
