...
2. Tuned controls still provide benefit
...
def _generate_10d_t120_lgssm(seed: int) -> tuple[tf.Tensor, tf.Tensor]:
    ...
    Uses the same dynamics structure as run_sqmc_tuning but with 10D state.
    """
    rng = np.random.default_rng(seed)

    # 10D state dynamics (stable)
    F = np.eye(STATE_DIM) * 0.95 + rng.normal(0, 0.02, (STATE_DIM, STATE_DIM))
    # Make symmetric and ensure stability
    F = (F + F.T) / 2
    eigenvalues = np.linalg.eigvals(F)
    if np.max(np.abs(eigenvalues)) > 0.98:
        F = F * (0.95 / np.max(np.abs(eigenvalues)))

    Q = np.eye(STATE_DIM) * 0.1  # process noise
    C = np.eye(STATE_DIM)  # observation matrix (observe all states)
    R = np.eye(STATE_DIM) * 0.2  # observation noise
    ...
    for t in range(HORIZON):
        if t > 0:
            x = F @ x + rng.multivariate_normal(np.zeros(STATE_DIM), Q)
        obs = C @ x + rng.multivariate_normal(np.zeros(STATE_DIM), R)
        observations.append(obs)

    obs_tensor = tf.constant(np.array(observations), dtype=R.DTYPE)

    # Theta: simplified parameterization (just scale F)
    theta = tf.constant([1.0], dtype=R.DTYPE)
...
def _oracle_score_10d_t120(observations: tf.Tensor, theta: tf.Tensor, seed: int) -> tf.Tensor:
    ...
    """
    # TODO: Implement proper Kalman oracle for 10D case
    # For now, just use SQMC as the target (no oracle comparison)
    return tf.zeros([STATE_DIM], dtype=R.DTYPE)
...
def _evaluate_route_on_seed(
    ...
    result = R._evaluate_controls(
        ...
        theta=theta,
        oracle_score=tf.zeros([STATE_DIM], dtype=R.DTYPE),  # No oracle comparison
        seed=seed,
        horizon=HORIZON,
...
def main() -> int:
    ...
    parser.add_argument("--output-dir", default="artifacts/sqmc-10d-t120-20260917")
