...
\begin{enumerate}[label=\textbf{Step \arabic*:}]
  ...
  \item \textbf{Weight:}
    let $w_{t-1}^{\star(i)} = 1/N$ if resampling was just performed and
    $w_{t-1}^{\star(i)} = w_{t-1}^{(i)}$ otherwise.  Compute
    $\tilde w_t^{(i)} = N\,w_{t-1}^{\star(i)}\,p_\theta(y_t \mid x_t^{(i)})$
    and normalize the weights.  On the resample-every-step branch this reduces
    to the familiar $\tilde w_t^{(i)} = p_\theta(y_t \mid x_t^{(i)})$.
  \item \textbf{Resample:}
    ...
    together with its normalized weights, until the next resampling decision.
\end{enumerate}
...
particle system gives an unbiased score.  Indeed, by Jensen's inequality,
$\mathbb E[\log \hat p_\theta^N(y_{1:T})]\le\log p_\theta(y_{1:T})$, with strict
inequality unless the estimator is almost surely constant, so the log of the
...
time $t-1$.  Let $\bar\pi_{t-1}^N$ denote the \emph{post-decision measure} from
which the transition propagation is drawn.  If resampling is used,
$\bar\pi_{t-1}^N$ is the unweighted empirical measure of the resampled cloud and
...
\[
  ...
  \hat\pi_{t-1}^N(\psi)
\]
for bounded $\psi$.  If resampling is not used, the post-decision measure is
instead the weighted carried measure
\[
  \bar\pi_{t-1}^N(\psi)
  =
  \sum_{i=1}^N w_{t-1}^{\star(i)}\psi(x_{t-1}^{(i)}),
\]
so the same identity holds trivially because
$\bar\pi_{t-1}^N=\hat\pi_{t-1}^N$ as weighted measures on that branch.

After bootstrap propagation and weighting with the carried post-decision
weights,
\[
  Z_t^N \hat\pi_t^N(\varphi)
  =
  \frac{1}{N}\sum_{i=1}^N
  N\,w_{t-1}^{\star(i)}\,p_\theta(y_t\mid x_t^{(i)})\varphi(x_t^{(i)}).
\]
...
stated.  In finite precision the estimator should be accumulated in log domain,
using a per-time log-sum-exp over incremental log weights, because the raw
product form underflows quickly for realistic horizons.  A gradient through the third object is not automatically a gradient of
