"""Offline M11 integrity, attempt, test, and worker-wall reconciliation."""
...
def main():
    ...
    opening_path = ROOT.parent / "m10-r1/reconciliation-terminal.json"
    opening = read(opening_path)["budget_seconds"]
    expected = ["fresh-1-trace-replay-gpu-r1", "fresh-0-bounded-replay-gpu-r1",
                *(f"regression-bounded-boundary-fresh-{i}-gpu-r1" for i in range(3))]
    ...
    exceeded = (tranche["cpu_reference"] > 4000. or tranche["gpu"] > 7200.
                or gpu_attempts > 6 or any(v < 0 for v in remaining.values()))
    summary = {"schema": "bayesfilter.hmc_m11_reconciliation.v1", "terminal": True,
        ...
        "budget_seconds": {"authorized": opening["authorized"], "opening_charged": opening["cumulative_charged"],
            "m11_measured": costs, "overhead": overhead, "m11_total": tranche,
