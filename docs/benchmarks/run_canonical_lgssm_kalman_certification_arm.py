#!/usr/bin/env python3
"""Run one frozen GPU/XLA arm of canonical LGSSM Kalman certification."""
...
def _execute(time_steps: int, arm: str) -> dict[str, Any]:
    ...
    estimator_seeds = ESTIMATOR_SEEDS_BY_HORIZON[time_steps]
    ...
    return {
        ...
        "python_horizon_unroll": True,
        "xla_production_loop_policy_status": "open_not_certified",
    }
...
def main() -> None:
    ...
    parser.add_argument(
        "--time-steps", type=int, choices=tuple(ESTIMATOR_SEEDS_BY_HORIZON), required=True
    )
    ...
    payload: dict[str, Any] = {
        "schema_version": "bayesfilter.canonical_lgssm_kalman_arm.v1",
        ...
        "estimator_seeds": list(ESTIMATOR_SEEDS_BY_HORIZON[args.time_steps]),
    }
    started = time.perf_counter()
    try:
        payload["result"] = _execute(args.time_steps, args.arm)
        payload["status"] = (
