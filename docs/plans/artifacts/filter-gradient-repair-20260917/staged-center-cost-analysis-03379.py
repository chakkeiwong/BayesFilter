"""Post-run GPU diagnostic; validate complete adapted-reference records and unshared provenance."""
import hashlib
import json
import statistics
import sys
...
from filter_repair_cost_provenance import validate_cost_device, require_same_physical_gpu
...
for number in range(3374, 3380):
    ...
    assert run["device"] == "GPU" and data["gpu"] is True
    assert run["state"] == "passed"
    assert data["original_compatibility"]["classification"] == "original_with_int64_accounting_only"
    assert data["original_compatibility"]["occurrences"] == 4
    assert data["original_compatibility"]["float_arithmetic_changed"] is False
    ...
    common = (run["environment"], provenance, data["original_source_sha256"], data["original_compatibility"])
    if source is None:
    ...
    if passed:
        uuids.append(validate_cost_device(run, provenance, data["gpu_process_observation"]))
    else:
    ...
    row = {"run": number, "state": run["state"], "eligible_for_matched_costs": passed,
        ...
        "gpu_allocator_peak_bytes": max(s["gpu"]["peak"] for s in snapshots),
        "gpu_allocator_current_bytes": {k: s["gpu"]["current"] for k,s in data["stages"].items()},
...
for dimension in (1, 3):
    ...
    device_ratio = candidate["gpu_allocator_peak_bytes"] / prior["gpu_allocator_peak_bytes"]
    if device_ratio > 2:
        triggers.append("gpu_allocator_peak_above_2x")
    comparisons.append({"dimension": dimension,
        "gpu_allocator_ratio": device_ratio,
...
result = {"schema": "filter_staged_center_cost_analysis.v1", "device": "GPU", "gpu_uuid": uuids[0],
    ...
    "limits": ["One process per arm/extent and three warm calls: descriptive localization only.",
        "All GPU arms pass full records; the original has the explicit four-counter int64 compatibility adaptation.",
        ...
        "Public integration, repeated terminal costs and repository completion remain open."]}
output = ROOT / "staged-center-costs-gpu-03379.json"
