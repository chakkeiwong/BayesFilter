...
The first CPU nonlinear benchmark compares three SVD/eigen sigma-point value
filters:
\[
...
the implemented SVD cubature, SVD-UKF, and SVD-CUT4 likelihoods match the
analytic scores on the selected smooth branch.
...
value benchmark still has zero phase variance.  Under the current smooth
simple-spectrum/no-active-floor SVD score branch, that default law is blocked
by an active placement floor.  This is an intentional diagnostic: the default
deterministic phase law needs a future fixed-null-direction derivative policy
before it can be promoted to a score-ready target.

Consequently, GPU/XLA and HMC evidence for nonlinear Models B--C remains
deferred until the score-ready parameter boxes, branch diagnostics, and any
future fixed-null derivative branch have been selected and tested.
