"""KSC finite-mixture transformed-SV LEDH-PFPF-OT value runner.

This runner targets the KSC surrogate SV row
``zhao_cui_sv_ksc_gaussian_mixture_surrogate_T1000``.  By default it emits a tiny
...
The target correction is the declared KSC transformed-SV Gaussian mixture:

``z_t = log(y_t**2 + 1e-8)``,
``z_t | x_t ~ sum_k w_k Normal(log(beta**2) + x_t + m_k, v_k)``.

The Gaussianized LEDH observation surface is a proposal surface only. The
finite mixture is the target likelihood for this row, not exact native SV.
"""
...
def _parse_args() -> argparse.Namespace:
    ...
    if args.run_scope == "tiny-smoke" and _full_row_requested(args):
        raise ValueError("adapter smoke runner must not run the full KSC-SV row")
    if args.run_scope == "full-row-diagnostic" and not _exact_full_row_requested(args):
        raise ValueError(
            "full-row-diagnostic requires exact KSC-SV full row settings: "
...
def _ksc_mixture_log_density(residual: tf.Tensor, weights: tf.Tensor, means: tf.Tensor, variances: tf.Tensor) -> tf.Tensor:
    ...
    return tf.reduce_logsumexp(component_log_terms, axis=0)


def _build_ksc_sv_tensors(args: argparse.Namespace) -> tuple[dict[str, tf.Tensor], dict[str, Any]]:
    ...
    transformed64 = transformed_sv_observations(raw_observations64, offset=TRANSFORM_OFFSET)
    ...
    tensors = {
        ...
        "beta": beta,
        "sigma": sigma,
        "mixture_weights": tf.cast(mixture.weights, DTYPE),
        "mixture_means": tf.cast(mixture.means, DTYPE),
        "mixture_variances": tf.cast(mixture.variances, DTYPE),
    ...
    semantics = {
        ...
        "source_model_row_id": SOURCE_MODEL_ROW_ID,
        ...
        "mixture_weights": [float(value) for value in mixture.weights.numpy()],
        "mixture_means": [float(value) for value in mixture.means.numpy()],
        "mixture_variances": [float(value) for value in mixture.variances.numpy()],
...
def _ksc_sv_value_core(
    *,
    ...
    def body(
        ...
        target_observation = _ksc_mixture_log_density(
            residual,
            mixture_weights,
            mixture_means,
            mixture_variances,
...
def main() -> None:
    ...
    forward_contract = make_ksc_sv_forward_contract(
    ...
    artifact: dict[str, Any] = {
        ...
        "target_observation_policy": "ksc_log_chi_square_gaussian_mixture_surrogate",
        "target_density_used_for_correction": True,
        "ksc_sv_semantics": ksc_sv_semantics,
        ...
        "normalization_checks": {
            ...
            "target_density_used_for_correction": True,
            "transform_offset_matches_contract": (
                ksc_sv_semantics["transform_offset"] == TRANSFORM_OFFSET
            ),
            "legacy_raw_gaussian_callback_used": False,
            "ksc_mixture_used": True,
            "ksc_mixture_is_target_likelihood": True,
