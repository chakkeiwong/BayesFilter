"""M27 same-device study of persistent versus isolated complete fits.

GPU designs require trusted execution, explicit device selection and verified
memory growth in each numerical process. CPU runs remain reference checks.
"""
...
def main():
    ...
    if os.environ.get("TF_FORCE_GPU_ALLOW_GROWTH") != "true":
        raise RuntimeError("memory growth must be enabled before worker initialization")
    visible = os.environ.get("CUDA_VISIBLE_DEVICES")
    if design.device == "cpu_reference" and visible != "-1":
        raise RuntimeError("CPU reference study requires hidden GPU")
    if design.device == "gpu" and (not visible or visible == "-1"):
        raise RuntimeError("GPU study requires explicit device selection")
    args.output.mkdir(parents=True, exist_ok=False)
    started = time.monotonic()
    manifest = {"source": source_state(), "command": [sys.executable, *sys.argv],
                "design": design.payload(), "mode": args.mode, "gpu_intentionally_hidden": design.device == "cpu_reference",
                "visible_devices": visible, "device_scope": design.device,
