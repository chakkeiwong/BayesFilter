...
\section{Principal-Square-Root UKF Backend}
\label{sec:bf-svd-principal-sqrt-backend}

The preceding eigenderivative formulas define the historical
\texttt{tf\_svd\_ukf} branch.  BayesFilter also has a distinct promoted
strict-SPD backend, \texttt{tf\_principal\_sqrt\_ukf}, whose factor is the
principal matrix square root rather than a signed or ordered eigenvector column
factor.  The historical eigenderivative branch is retained for diagnostic
comparison and bounded regression use, not as the HMC-facing production route on
the promoted strict-SPD scope.  The two names are backend contracts, not
aliases.
...
The same strict-SPD rule applies at the update stage to the innovation
covariance.  Let
\[
  S_t\in\mathbb S_{++}^{n_y},\qquad
  R_t=S_t^{1/2}=R_t^\top\succ0,
\]
where $S_t$ is the innovation covariance assembled from the sigma-point moment
recursion in
\eqref{eq:bf-svd-sp-transformed-cov}--\eqref{eq:bf-svd-sp-cross-second}.  Then
for each parameter component $\theta_i$ the promoted principal-square-root UKF
contract uses
\begin{equation}
\label{eq:bf-svd-principal-sqrt-innovation-first}
  R_t\,\dot R_t^{(i)}+\dot R_t^{(i)}R_t=\dot S_t^{(i)}
\end{equation}
rather than differentiating an eigendecomposition of $S_t$.  Likewise the
second-order update branch, when needed, is defined by
\begin{equation}
\label{eq:bf-svd-principal-sqrt-innovation-second}
  R_t\,\ddot R_t^{(ij)}+\ddot R_t^{(ij)}R_t
  =
  \ddot S_t^{(ij)}
  -\dot R_t^{(i)}\dot R_t^{(j)}
  -\dot R_t^{(j)}\dot R_t^{(i)} .
\end{equation}
On this branch the repeated-positive-eigenvalue hazard is handled by the same
strict-SPD Sylvester operator as in the placement stage; the backend must not
reintroduce eigen-gap denominators by differentiating innovation eigenvectors or
innovation eigenvalues under a separate SVD branch.

This update-side contract also keeps the Gaussian score and gain derivatives in
solve form.  Let $S_tw_t=v_t$ as in
\eqref{eq:bf-svd-sp-score}.  Then
\begin{equation}
\label{eq:bf-svd-principal-sqrt-innovation-solve-first}
  S_t\,\dot w_t^{(i)}
  =
  \dot v_t^{(i)}-\dot S_t^{(i)}w_t,
\end{equation}
and
\begin{equation}
\label{eq:bf-svd-principal-sqrt-innovation-logdet-first}
  \partial_i\log\det S_t
  =
  2\tr\left(R_t^{-1}\dot R_t^{(i)}\right)
  =
  \tr\left(S_t^{-1}\dot S_t^{(i)}\right).
\end{equation}
Therefore the promoted principal-square-root branch may use a symmetric square
root of $S_t$ for value-side solves and diagnostics, but its derivative contract
is the solve/Sylvester form in
\eqref{eq:bf-svd-principal-sqrt-innovation-first}--
\eqref{eq:bf-svd-principal-sqrt-innovation-logdet-first}, not an
innovation-eigendecomposition derivative.
...
represented and differentiated.  A promoted principal-square-root UKF backend
must fail closed whenever the strict-SPD assumptions for either placement or
innovation covariance are violated.  An active floor, a semidefinite innovation
law, or a structural-null completion rule is a different backend branch, not a
silent reinterpretation of
\eqref{eq:bf-svd-principal-sqrt-first} or
\eqref{eq:bf-svd-principal-sqrt-innovation-first}.  Required diagnostics for
this backend include the Sylvester residuals in
\eqref{eq:bf-svd-principal-sqrt-first} and
\eqref{eq:bf-svd-principal-sqrt-innovation-first}, the reconstruction residuals
$\dot P^{(i)}-(\dot S^{(i)}S+S\dot S^{(i)})$ and
$\dot S_t^{(i)}-(\dot R_t^{(i)}R_t+R_t\dot R_t^{(i)})$, the minimum eigenvalues
of $P$ and $S_t$, any jitter or floor policy, and eager/compiled parity for the
complete value-and-score path.
...
\end{enumerate}
For industrial-size models, prevention remains preferable to debugging: use
the direct-factor QR route on admitted full-rank or fixed-rank charts, and keep
the principal-square-root and historical eigenderivative SVD-UKF branches as
diagnostic comparators rather than default production routes.

\section{Current Role of SVD in the Direct-Factor Architecture}
\label{sec:bf-svd-current-direct-factor-role}

The preceding spectral recursion is retained as historical and diagnostic
material.  It is not the default SR-UKF implementation.  The current default
propagates factors and applies direct QR to residual stacks as described in
Section~\ref{sec:bf-srukf-implemented-direct-qr}; it does not reconstruct a
covariance and spectrally decompose it at each time step.

SVD still has a useful bounded role for an unknown singular direct stack: it
can discover numerical rank and nominate a fixed QR chart for a subsequent
value/score run.  Discovery is value-only because singular-vector gauges are
nonunique at repeated singular values and derivatives contain inverse spectral
gaps.  Once rank and chart are frozen, the admitted derivative is the
rectangular QR derivative, not an SVD derivative.  This separation preserves
SVD's rank-revealing robustness without importing equal-eigenvalue gauge
singularities into every temporal score evaluation.

For \(P_\varepsilon=GG^\top+\varepsilon I_n\), SVD also explains the limit but
does not make the raw ambient likelihood finite as \(\varepsilon\to0\).  With
rank \(r<n\), the raw likelihood tends to \(+\infty\) on support because of the
null-space normalization and to \(-\infty\) off support because of the
null-space quadratic.  Only the renormalized on-support limit
\[
 \lim_{\varepsilon\downarrow0}
 \left[\ell_\varepsilon+\frac{n-r}{2}\log(2\pi\varepsilon)\right]
\]
equals the finite affine-support likelihood.  It is a density with respect to
a different measure, so using it is an explicit statistical contract rather
than a numerical trick.
