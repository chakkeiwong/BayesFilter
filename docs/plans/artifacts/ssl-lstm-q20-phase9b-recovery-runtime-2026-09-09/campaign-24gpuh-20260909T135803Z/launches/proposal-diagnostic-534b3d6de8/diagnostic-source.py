...
@tf.function(input_signature=(tf.TensorSpec((4, 4), tf.float64), tf.TensorSpec((2,), tf.int32), tf.TensorSpec((), tf.int32), tf.TensorSpec((), tf.int32)), jit_compile=True, autograph=False)
def replay(current_state, seed, transition, leapfrogs):
...
rows = []
for transition, leapfrogs in ((157, 3), (158, 3), (158, 1), (158, 2)):
    current = samples[transition - 1]
    call_started = time.monotonic()
    values = replay(current, tf.constant(inputs["seed"], tf.int32), tf.constant(transition, tf.int32), tf.constant(leapfrogs, tf.int32))
