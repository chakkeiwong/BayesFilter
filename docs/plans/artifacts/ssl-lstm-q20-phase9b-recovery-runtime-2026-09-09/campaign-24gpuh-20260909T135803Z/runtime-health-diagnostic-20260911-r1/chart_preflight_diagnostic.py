...
parser.add_argument("--cpu-jit", action="store_true")
parser.add_argument("--native-reference", action="store_true")
args = parser.parse_args()
if args.cpu_jit and not args.cpu_only:
    parser.error("--cpu-jit requires --cpu-only")
jit_compile = not args.cpu_only or args.cpu_jit
...
with DurableTensorCheckpoint(OUTPUT / "tensors", {"sources": sources, "cpu_only": args.cpu_only,
    ...
    for arm in recovery.ARMS:
        ...
        bridge = make_q20_tempered_bridge(20, jit_compile=jit_compile,
        ...
        config = WeightedNeuTraConfig(dimension=source.DIMENSION, hidden_layers=(16, 16), stages=2,
            ...
            learning_rate=1.0e-3, jit_compile=jit_compile)
...
durable_json(OUTPUT / "run_manifest.json", {
    ...
    "native_eigensolver_cpu_reference": args.native_reference, "jit_compile": jit_compile,
