"""Execute the three predeclared M12 fresh fits sequentially, preserving failures."""
...
def main():
    ...
    for replication in range(3):
        command = [sys.executable, str(ROOT / "run_regression.py"), "--source",
                   str(ROOT / "source-gpu-r1"), "--case", "sblrc-blr", "--phase", "fresh",
                   "--replication", str(replication), "--seconds", "1500"]
        code = subprocess.run(command, stdout=subprocess.DEVNULL).returncode
        output = ROOT / f"regression-sequential-probe-fresh-{replication}-gpu-r1"
