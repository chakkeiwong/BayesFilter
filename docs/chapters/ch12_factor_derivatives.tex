...
\label{eq:bf-factor-cholesky-first}
  \dot L^{(i)} = L G^{(i)}.
\end{equation}
This is the BayesFilter restatement of source label
\texttt{eq:first\_cholesky\_derivative}.
...
\label{eq:bf-factor-cholesky-second}
  \ddot L^{(ij)} = L H^{(ij)}.
\end{equation}
This restates source label \texttt{eq:second\_cholesky\_derivative}.  The
formulas are local smooth-branch formulas: they require a positive diagonal and
...
\end{align}
Equation~\eqref{eq:bf-factor-qr-r-first} restates source label
\texttt{eq:first\_qr\_r\_derivative}.
...
\end{align}
Equation~\eqref{eq:bf-factor-qr-r-second} restates source label
\texttt{eq:second\_qr\_r\_derivative}.  The QR formulas are local to a fixed
rank and sign branch.  Pivoting, column-rank loss, or diagonal sign corrections
are branch events and must be reported as derivative-policy events, not hidden
inside a smooth Hessian claim.
