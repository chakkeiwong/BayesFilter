"""Focused diagnostic: dlgssm score bias vs T and value bias vs N.

Debug-only (not a research decision by itself): classifies the Q3
dlgssm deviations as accumulating estimator bias vs something else,
against exact Kalman references.
"""
import os, sys, time
...
from run_q3_leaderboard_20260824 import make_callbacks, canonical_filter_values
...
print("== score bias vs T (plain score lane, N=1008, dir 0, 8 seeds) ==", flush=True)
for T in (5, 10, 25, 50):
    sc = analytic_scores(T, 1008, range(8))
    ex = exact_score(T)
    se = sc.std(ddof=1)/np.sqrt(len(sc))
    print(f"[T={T}] mean={sc.mean():.3f} exact={ex:.3f} bias={sc.mean()-ex:+.3f} SE={se:.3f}", flush=True)

print("== score bias vs N (plain, T=50, dir 0) ==", flush=True)
for n, seeds in ((252, range(8)), (1008, range(8)), (4032, range(4))):
    sc = analytic_scores(50, n, seeds)
    ex = exact_score(50)
    se = sc.std(ddof=1)/np.sqrt(len(sc))
    print(f"[N={n}] mean={sc.mean():.3f} exact={ex:.3f} bias={sc.mean()-ex:+.3f} SE={se:.3f}", flush=True)

print("== filter VALUE bias vs N (with OT reset, T=50, vs exact Kalman) ==", flush=True)
initial_mean = tf.zeros([3], DTYPE)
cb = make_callbacks(model, theta0, "diag_lgssm_biasdiag", 3, 3, initial_mean)
