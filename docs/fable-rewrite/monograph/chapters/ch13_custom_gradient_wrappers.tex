...
A custom gradient is the exact HMC force only when it returns the derivative of the same scalar log target used in the Metropolis correction. A deterministic nearby force field can still leave the invariant law exact when the acceptance step evaluates the exact target value, but then the approximation changes leapfrog efficiency rather than the target. Bias arises when the value path is also changed, when the chain is left unadjusted, or when the force construction breaks the reversible volume-preserving integrator used by the correction step.
...
\]
for the same scalar in \eqref{eq:bf-custom-primal-target}.  The posterior-curvature matrix is $-H(\psi)$. If the chapter or implementation also reports likelihood-only observed information, it must state that object separately as the negative Hessian of the likelihood term alone, together with the sign convention and any prior contribution.  A Hessian path
