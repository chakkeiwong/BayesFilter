...
the four-dimensional reference compares transition particle filters along
the ladder $32768$, $65536$, $131072$, stopping when two successive levels
pass the declared checks. Its declared mean-MCSE, log-evidence
...
Suppose the training set is finite, each $h_i,b_i,w_i$ is finite, $w_i\ge0$,
and the denominator in~\eqref{eq:robust-amplitude-scale} is strictly positive.
For a fixed reference measure $\mu$, suppose also that $c^*$ is finite and
nonzero and $0<\int h(u)^2\,d\mu(u)<\infty$. Replacing $h$ by
$c^*h$ is valid even when $c^*<0$. If the represented TT density is the
normalized square $q_h(x)=h(x)^2/\int h(u)^2\,d\mu(u)$ with respect to
$\mu$, then
...
For the polynomial TT amplitude, $\mu$ is the Gaussian reference measure;
the polynomial alone need not be square-integrable with respect to Lebesgue
measure. The implementation now checks the denominator, finiteness and $|c^*|>0$ and
retains the signed scalar. Positive-scale cases are unchanged to roundoff.
Its Gaussian-reference conditional density has the form
$q(u\mid v)=(h(u,v)^2+\tau)\phi(u)/(Z(v)+\tau)$.
A global sign change leaves this density unchanged even with fixed $\tau$.
For a change in amplitude magnitude, the same invariance requires scaling
$\tau$ by $c^2$ as well as $Z(v)$; the implemented choice
$\tau=Z_{\rm joint}\delta/(1-\delta)$ has this homogeneity when
the declared internal defensive mass $\delta$ is fixed. An independently
fixed additive $\tau$ would not have magnitude invariance. The signed
least-squares adjustment occurs before fitting and before this defense is
constructed; no fixed additive term is silently rescaled during sampling.
Confirmation-01 d4-s05 had $c^*=-0.3106276393$ and is preserved as the
repair trigger. Its missing stable/full fits remain visible in that original
comparison. An exact replay now completes, and a fresh confirmation evaluates
the repaired endpoint on disjoint data.

\subsection{Fresh filtering results after the numerical repair}

The repeated calibration selected exactly the same controls and reproduced
the earlier selection statistics. Fresh confirmation then completed all eight
methods on all 24 sequences. None of the TT arms had a nonfinite consumer
step, inverse-CDF bracket failure or log-evidence disagreement under the
declared checks. One four-dimensional reference remained too imprecise:
at 131072 particles its maximum normalized mean MCSE was $0.02362$, above
the $0.02$ limit. Its log-evidence MCSE was $0.03533$, below the $0.10$
limit. This case remains in the record and is excluded from accuracy
comparisons for every method. It is a reference limitation, not a TT failure.

\begin{table}[htbp]
\centering\small
\begin{tabular}{clrrr}
\toprule
$d$ & TT proposal & Mean error $M$ & Mean ESS & Minimum ESS\\
\midrule
1 & A09 baseline & 0.00187975 & 383.29 & 62.46\\
1 & Repaired guide & 0.00175544 & 383.61 & 143.32\\
1 & Independent charts & 0.00165335 & 375.14 & 164.18\\
1 & Charts and physical mixture & 0.00152322 & 374.61 & 114.69\\
\midrule
4 & A09 baseline & 0.00241227 & 325.01 & 46.18\\
4 & Repaired guide & 0.00239518 & 323.52 & 45.74\\
4 & Independent charts & 0.00311999 & 304.27 & 2.62\\
4 & Charts and physical mixture & 0.00274654 & 303.89 & 14.06\\
\bottomrule
\end{tabular}
\caption{Fresh A10 filtering comparison, $N=512$, $T=20$, four repetitions.
Mean error uses twelve scalar and eleven four-dimensional reference-valid
sequences. ESS uses all twelve numerically completed sequences in each
dimension. These aggregates are descriptive except for the stated scalar
paired comparison; extremes have no uncertainty interval.}
\label{tab:robust-fresh-filtering}
\end{table}

In the scalar problem the full protection reduces mean error by $19.0\%$
relative to A09. The predeclared exploratory paired interval for the absolute
difference is $[-0.0007837013,-0.0000048193]$. All three repairs satisfy the
declared scalar $10\%$ non-harm comparison, but only the full arm's absolute
difference interval excludes zero. No ranking among the repaired arms follows.
In dimension four, the full arm has $13.86\%$ larger observed error than A09
on the eleven matched valid cases. Independent charts alone have $29.34\%$
larger error, while the guide-only difference is $-0.71\%$. The missing
twelfth valid reference prevents the planned four-dimensional interval;
these differences remain descriptive.

Conditional comparisons identify a further limitation. On ordinary scalar
observations, all three repairs have larger observed error than the transition
proposal, despite favorable averages across the full scalar experiment. In
dimension four, guide-only has larger error than the SGQF joint near zero;
independent charts have larger error in all three observation regimes; the
full arm has larger error near zero and in the ordinary regime. These
observations fail the predeclared conditional comparison rule for recommending
the new routes. They do not establish a statistically certain heuristic
ranking or invalidate TT as the representation needed by the filter.

The numerical mechanisms nevertheless perform their stated tasks. Out of
240 guide updates per dimension, positive Laplace quadrature repairs seven
scalar and 77 four-dimensional updates. Three four-dimensional updates remain
unresolved and explicitly use the predictive Gaussian. The other updates
pass the signed-rule resolution comparison. All 1920 particle-filter steps
using the physical mixture satisfy its checked density lower bound. Its
observed physical-component fraction is $0.04968$, with $\varepsilon=0.05$.
The minimum ESS of $14.06$ in the full four-dimensional arm illustrates why
the finite conditional second-moment result is not an ESS floor.

Observed four-dimensional preparation times, including the appropriate guide
and TT fitting, are $18.97$, $19.48$, $21.34$ and $21.34$ seconds for the four
TT arms in table order. Particle times per $N=512$, $T=20$ repetition are
$1.32$, $1.10$, $1.10$ and $1.24$ seconds. The SGQF joint requires $2.62$
seconds of preparation and $0.24$ seconds per repetition. Compilation and
fixed execution order affect these measurements, so they do not establish
a general speed ranking. Full defense reuses the independent-chart fit;
its standalone preparation time includes that fit.

The remaining question is whether the fixed rank and polynomial degree have
enough capacity to fit well in the broader coordinates. That is a plausible
explanation of the four-dimensional degradation, not an established cause.
A separate capacity study and more precise references are needed before
recommending the protected route. The exposed covariance and amplitude-sign
failures have been repaired, while approximation quality, rare-event behavior
and the total analytical parameter derivative remain distinct requirements.
The numerical record and complete comparison are retained in
\path{docs/plans/observation-aware-tt-robust-guide-20260916-result.md}.
MathDevMCP inspected the new labeled equations but had incomplete derivation
coverage and formalization abstentions; it did not issue a formal proof
certificate. The nine proposition proofs and their explicit assumptions
therefore remain the mathematical argument, supported by 38 focused tests
of the implemented identities and consumer path.
