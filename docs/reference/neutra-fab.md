...
| Fresh weighted loss | `train/fab_without_buffer.py::fab_loss_smc_samples` | Paper Eq. 7 weighted sum. JAX uses mean after normalized weights, introducing 1/batch; this port deliberately follows the paper's scaling |
...
| Initial invalid states | `sampling/smc.py::replace_invalid_samples_with_valid_ones` | Deliberate stricter departure: fail the attempt; no replacement that silently changes the initial q law |
...
different target, numerical configuration or seed is rejected.
