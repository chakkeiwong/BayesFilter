...
declared radius in one iteration.  The corrected \(v_i\) are re-centered and
...
\begin{enumerate}[label=(\arabic*)]
 ...
 \item if \(S>0\), compute \eqref{eq:bf-eot-hm-target-moments}, apply
       \eqref{eq:bf-eot-hm-scaled-lm-system}--\eqref{eq:bf-eot-hm-smooth-trust-cap}
...
column-scale floor \(\varepsilon_s\), trust radius \(\tau\), and \(S\).
...
smooth trust caps, and finite rewhitening maps are differentiable.  The ordinary chain rule therefore
