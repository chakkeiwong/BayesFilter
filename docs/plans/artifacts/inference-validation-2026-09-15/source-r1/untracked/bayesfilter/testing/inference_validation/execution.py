...
def source_state():
    ...
    return {"commit":subprocess.run(["git","rev-parse","HEAD"],cwd=REPO,capture_output=True,text=True,check=True).stdout.strip(),
...
def worker(design_file,root,budget,attempt=1):
    ...
    try:
        ...
        manifest={"design":design.payload(),"runtime":runtime,
            ...
            "plan_file":"docs/plans/bayesfilter-inference-validation-execution-2026-09-15.md",
...
def run_suite(suite,root,*,resume=False,max_jobs=None):
    ...
    with (root/".coordinator.lock").open("a+b") as handle:
        ...
        return _run_suite(suite,root,resume=resume,max_jobs=max_jobs)


def _run_suite(suite,root,*,resume=False,max_jobs=None):
    ...
    for job in plan["jobs"]:
        ...
        if max_jobs is not None and launched>=max_jobs: break
        ...
        index["jobs"][key]={**prior,"status":"running","started_at":time.time(),"log":str(log),"reserved_seconds":remaining}
        write_json(index_path,index)
        print(f"running {key}: {d.engine}/{d.scenario.target}/{d.scenario.route}",flush=True)
        with log.open("w") as handle:
            env=dict(os.environ, TF_FORCE_GPU_ALLOW_GROWTH="true", BAYESFILTER_PRELOAD_CUSTOM_OP="0")
            if d.device=="cpu_reference": env["CUDA_VISIBLE_DEVICES"]="-1"
            process=subprocess.Popen(command,cwd=REPO,stdout=handle,stderr=subprocess.STDOUT,env=env,start_new_session=True)
            try:
        ...
        record={"attempt":attempt,"elapsed_seconds":elapsed,"exit_code":code,"log":str(log),"status":status,"command":command}
        result_path=job_root/f"attempt-{attempt:03d}-result.json"
        index["jobs"][key]={"status":status,"attempts":[*prior["attempts"],record],"result":str(result_path) if code==0 else None}
        if code==0:
            index["jobs"][key]["result_sha256"]=file_hash(result_path)
        write_json(index_path,index); launched+=1
        print(f"{key}: {status} ({elapsed:.2f}s)",flush=True)
    write_json(index_path,index)
