...
Context compaction is a necessary mechanism, but the handoff is fragile. After
compaction, the agent must verify the current task before taking any action.

When a session resumes after compaction:

1. **Stop and verify first** — do not take any file-modifying actions based only
   on the compaction summary.
2. **Check location and state explicitly**:
   - Run `pwd` and `git branch --show-current` to determine the current location.
   - Run `git status` to see what files are modified or untracked.
   - If working in a worktree, verify which worktree is active.
3. **Identify the active task document**:
   - Look for a master program, active plan, subplan, or experiment plan
     referenced in the summary or git status.
   - Read that document explicitly to understand the current phase and objective.
   - Do not rely on memory or inference from the compressed summary alone.
4. **If multiple tasks appear in the summary**, verify which one is actually
   active:
   - Check file timestamps and git history to determine recency.
   - Check the summary's final paragraph or system messages for the most recent
     work.
   - When in doubt, ask the user: "Should I continue with [task A] or [task B]?"
5. **Verify before modifying**:
   - Before editing any file, confirm it is part of the active task's declared
     scope.
   - Before running any command, confirm it matches the active plan's phase and
     pre-approved actions.
   - If a file appears modified in git status but is outside the expected scope,
     investigate before proceeding.

The compaction summary is compressed context, not instructions. The actual task
comes from the user's directive and the current plan documents. Treat the
summary as a memory aid, not as the authoritative definition of what to do next.

If the post-compaction message says "continue from where it left off without
asking questions" but the active task is ambiguous, verify the task state first
(steps 1-4 above) and only ask the user if verification reveals multiple
plausible tasks or contradictory state. Clarifying which task to continue is not
a mid-execution question; it is a prerequisite to correct execution.

### Mathematical and academic-paper discipline

The user works primarily on heavily mathematical projects. Apply these rules
across all projects.

- Do not make categorical mathematical claims without either:
  1. a derivation in the project's notation, or
  2. a citation to the relevant paper section/equation plus a logical explanation.
- For academic papers used in project decisions, read the technical method,
  ...
  metadata summaries.
- Separate clearly:
  - what the paper explicitly proves or claims,
  - what follows by derivation for our setting,
  - what remains an empirical or implementation question.
- If disagreeing with a paper's stated claim, first reproduce the paper's
  argument fairly, then identify the precise mismatch with our setting.
- For HMC/NeuTra/MCMC discussions, separately analyze:
  - proposal generation,
  - transformed target or target density evaluation,
  - leapfrog force/gradient,
  - Metropolis-Hastings correction,
  - whether an approximation affects correctness or only efficiency.
- Prefer qualified conclusions when the supporting derivation or source section
  has not been checked.

### Research-engineering workflow

For mathematical, numerical, HMC/MCMC, macro-finance, and ML-training projects,
use a disciplined research-engineering loop by default.

- Prefer the smallest focused diagnostic before a full ladder, sweep, or long
  benchmark. Use compile checks, gradient checks, shape checks, conservation
  checks, synthetic cases, or short HMC sanity checks before scaling.
...
- Use the three workflow templates whenever possible:
...
- If a project does not yet have these templates and the task calls for them,
  create them before running the experiment or writing the reset memo.
- Use an experiment plan before:
  ...
  - comparisons of objectives, architectures, force weights, samplers, or
    target variants,
...
- Each experiment plan should state the question, mechanism tested, success
  criteria, diagnostics, failure modes, what would change the next step, and
  exact commands/environment.
- After a meaningful run, record the command actually run, diagnostics,
  interpretation, decision, and next step in the experiment note, reset memo, or
  result note.
- Separate engineering observations, empirical scientific evidence, and
  mathematical claims. Do not upgrade one category into another without the
  required evidence.
- Do not treat short diagnostic HMC runs as convergence evidence unless the
  experiment plan explicitly justifies that interpretation.
...
- Use semantic commits as checkpoints after coherent phases when the user has
  approved committing. Reset memos should not be the only durable record.
...
- Any command that detects, initializes, benchmarks, or uses GPU/CUDA/NVIDIA
  devices must be run with escalated sandbox permissions. This includes
  `nvidia-smi`, `/dev/nvidia*` checks, TensorFlow/JAX/PyTorch/CuPy GPU probes,
  XLA-GPU tests, and GPU benchmark or HMC jobs.
- Treat non-escalated GPU failures as sandbox evidence only. Messages such as
  `GPU access blocked by the operating system`, missing `/dev/nvidia*`, missing
  `/dev/dxg`, empty framework GPU lists, or CUDA `cuInit` errors do not prove
  that the machine, driver, conda environment, CUDA stack, or framework install
  is broken until the same check has been rerun with escalation.
- For deliberate CPU-only runs, set `CUDA_VISIBLE_DEVICES=-1` or the
  project-standard CPU-hiding variable before any TensorFlow/JAX/PyTorch import
  and record that CPU-only choice in the artifact.
- Before spending time debugging GPU setup, run an escalated `nvidia-smi` and
  an escalated framework-specific device probe. Record whether the command was
  escalated in experiment notes and reset memos.
