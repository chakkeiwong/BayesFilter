...
parser.add_argument("--mode", choices=("prefix", "cached-gradient", "covariance", "eigensystem", "refinement"), default="prefix")
...
with (CAMPAIGN / ".coordinator.lock").open("a+b") as lock:
    ...
    source_name = {"covariance": "trace_covariance_diagnostic.py", "eigensystem": "gpu_eigensystem_diagnostic.py", "refinement": "gpu_eigensystem_diagnostic.py"}.get(args.mode, "replay_proposal_diagnostic.py")
    source = Path(__file__).with_name(source_name)
    mode_arguments = {"prefix": ("--sample-chain-results", "159"), "cached-gradient": ("--cached-gradient",), "covariance": (), "eigensystem": (), "refinement": ("--refine",)}[args.mode]
