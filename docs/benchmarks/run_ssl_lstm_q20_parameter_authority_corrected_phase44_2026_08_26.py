"""Audit N=256 and N=512 theta banks under one reconstructed trainer state.

Each arm creates one trainer, trains only on the frozen v2.2 training split,
and evaluates banks A, B, C, and N=512 only after the final optimizer update.
The terminal state hash must match the v2.4 reference audit. The phase is
...
if os.environ.get("CUDA_VISIBLE_DEVICES") == "-1":
    raise RuntimeError("Phase 44 requires a visible trusted GPU")
if os.environ.get("TF_FORCE_GPU_ALLOW_GROWTH") != "true":
    raise RuntimeError("Phase 44 requires TF_FORCE_GPU_ALLOW_GROWTH=true")
...
SPEC = importlib.util.spec_from_file_location("phase31_helpers_phase44", HELPER_PATH)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("cannot load audited Phase 31 helper")
...
EXPECTED_SCHEMA = "bayesfilter.ssl_lstm.q20.corrected_theta_neutra_boundary.v7_four_bank_mixed_n"
REFERENCE_AUDIT_SCHEMA = "bayesfilter.ssl_lstm.q20.corrected_theta_neutra_boundary.v5_two_independent_banks"


class Phase44Error(RuntimeError):
    """Raised when the larger-N support boundary cannot be audited."""
...
def _write_json(path: Path, payload: Mapping[str, Any]) -> None:
    if path.exists():
        raise Phase44Error(f"refusing to overwrite artifact: {path}")
...
def _receipt(root: Path, name: str) -> tuple[Path, Mapping[str, Any]]:
    if root.is_absolute() or ".." in root.parts:
        raise Phase44Error(f"path must be repository-relative: {root}")
    path = ROOT / root / name
    if not path.is_file():
        raise Phase44Error(f"missing receipt: {path}")
...
def _load_tensor(receipt: Mapping[str, Any]) -> tf.Tensor:
    ...
    if hashlib.sha256(encoded).hexdigest() != str(receipt["sha256"]):
        raise Phase44Error(f"tensor hash mismatch: {path}")
...
def _arm(
    ...
    *,
    ...
    if pilot.get("status") != "PASS_THETA_MEASURE_PILOT":
        raise Phase44Error("pilot is not a passing theta-measure receipt")
    arm = pilot.get("arms", {}).get(name)
    if not isinstance(arm, Mapping) or arm.get("status") != "PASS_THETA_MEASURE_PILOT":
        raise Phase44Error(f"{name} arm is not passing")
    if arm.get("target_signature") != EXPECTED_TARGET_SIGNATURE:
        raise Phase44Error(f"{name} target signature mismatch")
    if arm.get("protocol", {}).get("measure") != EXPECTED_MEASURE:
        raise Phase44Error(f"{name} measure mismatch")
    if arm.get("configuration", {}).get("protocol_hash") != expected_protocol:
        raise Phase44Error(f"{name} protocol hash mismatch")
    if int(arm.get("configuration", {}).get("particles", -1)) != int(expected_particles):
        raise Phase44Error(f"{name} particle count mismatch")
    calibration = pilot.get("calibration", {})
    if int(calibration.get("particle_count", -1)) != int(expected_calibration_particles):
        raise Phase44Error(f"{name} calibration particle count mismatch")
    return arm
...
def _cloud(arm: Mapping[str, Any]) -> Mapping[str, tf.Tensor]:
    ...
    if values["theta"].shape.rank != 2 or values["theta"].shape[1] != 4:
        raise Phase44Error(f"theta shape is not [N,4]: {values['theta'].shape}")
    if values["weights"].shape != (n,) or values["roots"].shape != (n,):
        raise Phase44Error("cloud vector shape mismatch")
...
def _run_arm(
    *,
    ...
    return {
        "status": "PASS_NEUTRA_BOUNDARY_CANDIDATE" if all(gates.values()) else "PHASE44_CANDIDATE_FAIL_REPAIR",
        ...
        "one_trainer_state_for_four_banks": True,
        "nonclaims": [
            "The N=256 and N=512 fresh banks are finite audits, not IID or posterior proofs.",
...
def _markdown(result: Mapping[str, Any]) -> str:
    ...
    for key, arm in result["arms"].items():
        ...
        n512 = arm["fresh_audits"]["bank_n512"]
        lines.append(
            f"| {key} | {arm['precondition']} | `{arm['gates']['reference_state_hash_match']}` | {float(a['latent_mean_max_abs']):.6f} / {float(a['latent_covariance_max_abs_offdiag']):.6f} | {float(b['latent_mean_max_abs']):.6f} / {float(b['latent_covariance_max_abs_offdiag']):.6f} | {float(c['latent_mean_max_abs']):.6f} / {float(c['latent_covariance_max_abs_offdiag']):.6f} | {float(n512['latent_mean_max_abs']):.6f} / {float(n512['latent_covariance_max_abs_offdiag']):.6f} |"
        )
    lines.extend([
        "",
...
def main() -> int:
    ...
    if int(args.steps) <= 0:
        raise Phase44Error("steps must be positive")
    for path in (args.authority_root, args.fresh_root_a, args.fresh_root_b, args.fresh_root_c, args.fresh_root_n512, args.reference_audit_root, args.output_root):
        if path.is_absolute() or ".." in path.parts:
            raise Phase44Error("all paths must be repository-relative")
    output = ROOT / args.output_root
    if output.exists():
        raise Phase44Error(f"refusing to overwrite output root: {output}")
    ...
    fresh_n512_path, fresh_n512_pilot = _receipt(args.fresh_root_n512, "pilot.json")
    ...
    if reference_audit.get("schema") != REFERENCE_AUDIT_SCHEMA or reference_audit.get("status") != "PASS_V2_4_TWO_BANK_BOUNDARY":
        raise Phase44Error("reference audit is not a passing v2.4 boundary receipt")
    if reference_audit.get("target_signature") != EXPECTED_TARGET_SIGNATURE:
        raise Phase44Error("reference audit target signature mismatch")
    ...
    for arm_key in ("identity:compact", "identity:wide_low_lr", "affine:compact", "affine:wide_low_lr"):
        ...
        if not isinstance(item, Mapping) or not isinstance(item.get("state_hash"), str):
            raise Phase44Error(f"missing v2.4 reference state hash for {arm_key}")
        expected_state_hashes[arm_key] = str(item["state_hash"])
    pilot_paths = (authority_path, fresh_a_path, fresh_b_path, fresh_c_path, fresh_n512_path)
    if len({_sha(path) for path in pilot_paths}) != 5:
        raise Phase44Error("authority and fresh pilot receipts are not all distinct")
    old_m0 = _arm(authority_pilot, "M0", EXPECTED_M0_PROTOCOL, expected_particles=256, expected_calibration_particles=64)
    fresh_a_m0 = _arm(fresh_a_pilot, "M0", EXPECTED_M0_PROTOCOL, expected_particles=256, expected_calibration_particles=64)
    fresh_b_m0 = _arm(fresh_b_pilot, "M0", EXPECTED_M0_PROTOCOL, expected_particles=256, expected_calibration_particles=64)
    fresh_c_m0 = _arm(fresh_c_pilot, "M0", EXPECTED_M0_PROTOCOL, expected_particles=256, expected_calibration_particles=64)
    n512_m0 = _arm(fresh_n512_pilot, "M0", EXPECTED_M0_PROTOCOL, expected_particles=512, expected_calibration_particles=128)
    old_c0 = _arm(authority_pilot, "C0", EXPECTED_C0_PROTOCOL, expected_particles=256, expected_calibration_particles=64)
    fresh_a_c0 = _arm(fresh_a_pilot, "C0", EXPECTED_C0_PROTOCOL, expected_particles=256, expected_calibration_particles=64)
    fresh_b_c0 = _arm(fresh_b_pilot, "C0", EXPECTED_C0_PROTOCOL, expected_particles=256, expected_calibration_particles=64)
    fresh_c_c0 = _arm(fresh_c_pilot, "C0", EXPECTED_C0_PROTOCOL, expected_particles=256, expected_calibration_particles=64)
    n512_c0 = _arm(fresh_n512_pilot, "C0", EXPECTED_C0_PROTOCOL, expected_particles=512, expected_calibration_particles=128)
    old = _cloud(old_m0)
    banks = {"bank_a": _cloud(fresh_a_m0), "bank_b": _cloud(fresh_b_m0), "bank_c": _cloud(fresh_c_m0), "bank_n512": _cloud(n512_m0)}
    tensor_hashes: dict[str, dict[str, str]] = {}
    for label, arm in (("authority", old_m0), ("bank_a", fresh_a_m0), ("bank_b", fresh_b_m0), ("bank_c", fresh_c_m0), ("bank_n512", n512_m0)):
        tensor_hashes[label] = {
    ...
    for label, arm in (("authority", old_c0), ("bank_a", fresh_a_c0), ("bank_b", fresh_b_c0), ("bank_c", fresh_c_c0), ("bank_n512", n512_c0)):
        c0_tensor_hashes[label] = {
            key: str(arm["receipts"][key]["sha256"])
            for key in ("final_theta", "final_normalized_weights", "final_roots")
        }
    for key in tensor_hashes["authority"]:
        if len({tensor_hashes[label][key] for label in tensor_hashes}) != 5:
            raise Phase44Error(f"M0 tensor hash collision for {key}")
        if len({c0_tensor_hashes[label][key] for label in c0_tensor_hashes}) != 5:
            raise Phase44Error(f"C0 tensor hash collision for {key}")
    ...
    if not all(split.get(key) is True for key in ("root_disjoint", "row_partition_complete", "row_partition_disjoint")):
        raise Phase44Error("frozen root-group split invariant failed")
    ...
    result = {
        ...
        "status": "PASS_V2_6_LARGER_N_BOUNDARY" if candidate_pass else "PHASE44_CANDIDATE_FAIL_REPAIR",
        "plan_version": EXPECTED_VERSION,
        "role": "frozen_v2_2_training_measure_three_n256_plus_n512_fresh_theta_audits",
        ...
        "fresh_banks": {
            ...
            "bank_n512": {"root": args.fresh_root_n512, "pilot_sha256": _sha(fresh_n512_path), "m0_protocol_hash": EXPECTED_M0_PROTOCOL, "particles": int(banks["bank_n512"]["theta"].shape[0]), "calibration_particles": 128, "untouched": True},
        },
        ...
        "arms": arms,
        "one_trainer_state_per_arm_for_four_banks": True,
        "fresh_rows_used_for_training": False,
        "fresh_rows_used_for_selection": False,
        "hmc_launched": False,
        "device": {
        ...
        "run_manifest": {
            ...
            "source_sha256": {
                ...
                "fresh_n512_pilot": _sha(fresh_n512_path),
                "reference_audit": _sha(reference_audit_path),
            },
        },
        "nonclaims": [
            "N=256 and N=512 fresh banks are finite audits, not an IID or posterior proof.",
