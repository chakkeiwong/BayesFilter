...
REPO = ROOT.parents[4]
SOURCE = ROOT.parent / "m13-r1/source-final-r2"
...
def main():
    ...
    design = read(ROOT / "preparation-design.json")
    if args.worker:
    ...
    for row in design["rows"]:
        ...
        if (out / "gpu-diagnostic-run.json").exists():
            continue
        costs = sum(read(p)["elapsed_seconds"] for p in ROOT.glob("preparation-*/gpu-diagnostic-run.json"))
        if costs + row["seconds"] > 3960.:
