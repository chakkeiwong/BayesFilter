"""Bounded M27 worker with explicit device policy and preserved attempt costs."""
import argparse
import datetime
import hashlib
import json
...
manifest = {'command': command, 'cwd': str(source), 'git_commit': commit,
    ...
    'plan_file': 'docs/plans/bayesfilter-hmc-post-m26-next-phase-2026-09-22.md',
    'execution_note': 'docs/plans/bayesfilter-hmc-m27-integration-and-execution-2026-09-22.md',
...
with (root / 'worker.log').open('x') as log:
    try:
        if args.device == 'gpu':
            ...
            if any(line.split(',')[0].strip() == selected_uuid for line in processes.splitlines()):
