...
The scalable-OT survey sharpens the target-status split inside this rung.  Exact
online or streamed Sinkhorn implementations can still belong to the same relaxed
entropic target class if they preserve the same declared teacher object and the
same executed scalar.  Their main changes are engineering changes in storage,
blocking, or GPU execution, not changes in the target definition itself.  By
contrast, factorized kernel routes, low-rank coupling routes, sliced/subspace
transports, and localized/block transports should not all inherit that same
status automatically.  Some remain best read as retained-teacher approximations,
while others are better read as explicit new transport rules.
...
become an exact categorical resampling layer because its gradient exists.  Nor
does a faster implementation become a new target simply because its execution
path is different, provided the same scalar and same transport object are still
being evaluated.
...
The scalable-OT survey helps separate two cases that should not be conflated.
Retained-teacher accelerators, such as warm-start predictors or other
teacher-preserving approximations, may still be judged first against the same
transport teacher object.  Their HMC burden is therefore to show that the value
and gradient are consistent for the same learned-or-corrected scalar and that
teacher-preservation residuals are understood on the tested envelope.  Broader
transport learners, sliced or localized replacement rules, and dynamic/operator
families do not even begin with that teacher-preservation presumption.  They are
better read as learned or replacement targets unless a separate correction or
error argument returns the chain to the teacher or original target.
...
smooth differentiable filtering path as operational-claim evidence before the
target and validation ladder have been passed.
...
the inference stack
\citep{greydanus2019hamiltonian,dhulipala2022latent,hoffman2019neutra,parno2018transport}.
Chapter~\ref{ch:bf-hnn-surrogate-hmc} gives the complete construction, proves
the discrete proposal conditions, and explains why a true endpoint likelihood
value is not by itself a universal correction.  These methods attempt to
improve movement through an already chosen target, or to use a surrogate
proposal that is later corrected against that target.  DPF methods may instead
alter the filtering likelihood itself through flow approximation, relaxed
resampling, or learned transport.  The distinction is operational:
\begin{itemize}
...
For the differentiable particle-flow route studied in this monograph, the
serious experimental path is therefore not a
