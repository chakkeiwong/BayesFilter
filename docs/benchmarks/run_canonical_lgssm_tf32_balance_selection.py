#!/usr/bin/env python3
"""Select the TF32 terminal-balance count at the N=1024 target."""
...
SCHEMA_VERSION = "bayesfilter.canonical_lgssm_tf32_balance_selection.v1"
CAMPAIGN_ID = "canonical-lgssm-tf32-balance-horizon-continuation-20260718"
BALANCE_CANDIDATES = (2, 3, 5, 8)
DESIGN_SEEDS = tuple(range(81600, 81608))
AUDIT_SEEDS = tuple(range(81620, 81628))
TIME_STEPS = 2
NUM_PARTICLES = 1024
...
def _evaluate(seeds: tuple[int, ...], balance_steps: int) -> dict[str, Any]:
    ...
    observations = _lgssm_dataset(runner.DATASET_SEED)["observations"][:TIME_STEPS]
    ...
    post_quotient_column_error = tf.boolean_mask(
        first["quotient_post_quotient_column_residual_history"], active
    )
    ...
    work_valid = (
        work["sinkhorn_state_constructions"] == TIME_STEPS
        and work["terminal_balance_state_constructions"] == TIME_STEPS
        and work["transport_tile_sweeps"] == TIME_STEPS
    ...
    return {
        ...
        "maximum_post_quotient_column_absolute_error": float(
            tf.reduce_max(post_quotient_column_error)
        ),
        "tv_column_error_by_seed_time": first["tv_column_error_history"].numpy().tolist(),
