...
def worker(replication, output):
    ...
    search = HMCControllerConfig(primary_l_grid=grid, epsilon_by_l=tuple((l, interior) for l in grid),
        total_budget_units=108, repair_reserve_units=0, candidate_reserve_units=6,
    ...
    write(output/"manifest.json", {"command": sys.argv, "source": source, "runtime": runtime,
        ...
        "budget_provenance": "108 work units = 18 pairs times 6 possible looks; 60 seconds reserved for setup/reporting",
...
def main():
    ...
    if args.worker is not None:
        output = ROOT/f"regression-interior-{args.worker}-gpu-r1"
        try:
    ...
    for i in range(3):
        output = ROOT/f"regression-interior-{i}-gpu-r1"
