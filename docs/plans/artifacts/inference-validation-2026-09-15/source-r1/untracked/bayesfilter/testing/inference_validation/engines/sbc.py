...
def one_output(design,data,root,dataset,fit,deadline):
    ...
    selected=sorted((m for m in output["members"] if m.get("L")==design.member_l),key=lambda m:m["candidate_id"])
    if not selected: return None,{"reason":"no_member_in_declared_L_group","pipeline":output}
    member=selected[0]
