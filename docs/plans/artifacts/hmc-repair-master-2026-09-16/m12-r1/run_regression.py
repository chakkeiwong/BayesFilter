"""M12 fresh no-hint fits with longer sequential metric-boundary probes."""
import argparse
...
def worker(args,output):
    ...
    seed = seed_for(2026092060,"m12-sequential-probe",args.case,args.phase,args.replication,"tuning")
    cfg = HMCKernelTuningConfig(preset="serious",metric_evidence_policy="finite_window",metric_probe_num_results=16,use_xla=True,target_scope="inference_validation",
...
def main():
    ...
    output = ROOT/f"regression-sequential-probe-{args.phase}-{args.replication}-gpu-r1"
    if args.worker:
        try:
            worker(args,output)
    ...
    if len(costs) >= 8 or sum(r["elapsed_seconds"] for r in costs) + args.seconds > 9000.:
        raise RuntimeError("M12 attempt/worker-wall budget exhausted")
    if args.phase == "fresh":
        pilot = read(ROOT/"fresh-2-sequential-probe-replay-gpu-r1/gpu-diagnostic-run.json")
        if pilot["returncode"] != 0:
            raise RuntimeError("matched preparation did not pass")
        prepared = read(ROOT/"fresh-2-sequential-probe-replay-gpu-r1/preparation_progress.json")
        updates = [event["details"]["window"]["metric_decision"] for event in prepared["events"]
                   if event["phase"].endswith("metric_decision")]
        if not any(item and item["update_applied"] for item in updates):
            raise RuntimeError("matched optional preparation must apply a qualified metric")
