...
reconstruction audits.  The monograph does \emph{not} yet derive the full
ordered signed rank-one update/downdate recurrence derivative needed to admit
all negative-weight analytical-score branches.  Therefore a factor-propagating
SR-UKF score is fully admitted only on reviewed branches whose implemented
update/downdate derivative primitive is supplied externally and whose
reconstruction identity is checked directly.  On other signed branches, the
value path remains useful, but the derivative route is conditional rather than
fully derived in-book.
