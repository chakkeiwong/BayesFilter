...
def fit_pair_features(features: tf.Tensor, target: tf.Tensor, row_weights: tf.Tensor,
    ...
    if initial is None:
        ...
        for axis in range(dimension):
            ...
            # Every channel must be excited: zero inactive channels would
            # trap one-site ALS at rank one regardless of the stored ranks.
            core = tf.random.stateless_normal(shape, [7919, axis], dtype=D) / tf.sqrt(tf.cast(n*n*ranks[axis+1], D))
            core = tf.tensor_scatter_nd_add(core, [[0, 0, 0, 0]], [tf.constant(1., D)])
            initial.append(core)
    ...
    return cores, {"minimum_objective_decrease": minimum_decrease,
                   ...
                   "initialization": "stateless_7919_unit_expected_local_row_energy_plus_constant",
                   "kkt_residual": kkt,
