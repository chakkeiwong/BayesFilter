...
def affine_restore_cloud_jvp(
    ...
    """Restore a uniform cloud to the weighted source mean and covariance."""
...
def _pairwise_shape_iteration_jvp(
    ...
    *,
    ...
    output, output_tangent = _standardize_uniform_jvp(
...
def higher_moment_shape_jvp(
    ...
    *,
    ...
    if coordinatewise_standardized_cap > 0.0:
        cap = tf.cast(coordinatewise_standardized_cap, source.dtype)
        power = tf.cast(coordinatewise_standardized_cap_power, source.dtype)
        scaled_power = tf.pow(standardized_pre_cap / cap, power)
        denominator = tf.pow(1.0 + scaled_power, 1.0 / power)
        standardized = standardized_pre_cap / denominator
        standardized_derivative = tf.pow(
            1.0 + scaled_power, -1.0 / power - 1.0
        )
        standardized_tangent = (
            standardized_derivative[:, :, None]
            * standardized_pre_cap_tangent
        )
    else:
        standardized_derivative = tf.ones_like(standardized_pre_cap)
    ...
    if coordinatewise_standardized_cap > 0.0:
        restored = affine_restore_cloud_jvp(
            source,
            weights,
            source_tangent,
            weights_tangent,
        ...
        output = restored["particles"]
        output_tangent = restored["particles_tangent"]
        restore_valid = restored["valid"]
    else:
        restore_valid = tf.constant(True)
    output_standardized, output_standardized_tangent = _standardize_uniform_jvp(
...
__all__ = ["affine_restore_cloud_jvp", "higher_moment_shape_jvp"]
