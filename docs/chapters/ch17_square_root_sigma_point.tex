...
\section{Pruned Second-Order Models: Executable API}
\label{sec:bf-pruned-direct-factor-api}

For a model with separate first- and second-order state components, use
\texttt{TFPrunedSRUKFModel} and \texttt{tf\_pruned\_srukf\_filter} from
\texttt{bayesfilter.nonlinear}. The parameterized entry point
\texttt{make\_pruned\_srukf\_value\_and\_score} compiles the likelihood and its
total parameter derivative. The field shapes, initialization conventions,
and usage example are in
\texttt{docs/reference/pruned-direct-factor-srukf.md}.

Let $f_t$ and $s_t$ denote first- and second-order deviations, and let $e_t$
be the innovation. With caller-supplied coefficients the recursion is
\[
 f_t=A f_{t-1}+B e_t,\qquad
 s_t=A s_{t-1}+c+\tfrac12 H[(f_{t-1},e_t),(f_{t-1},e_t)].
\]
Both state components remain random and are conditioned jointly on the
observations. The quadratic form uses only first-order deviations and
innovations. Its full Hessian includes state--innovation cross terms, with
the usual one-half convention; the caller supplies the second-order forcing
$c$. The observation equation is
\[
 y_t=d+C(f_t+s_t)+\tfrac12 J[f_t,f_t]+v_t.
\]
This is a coefficient contract, not a solver for the coefficients. In
particular, Taylor-expanding a learned transition defines a different model
from an independently derived equilibrium perturbation solution.

The initial joint factor may have zero rows for a deterministic second-order
component. Scaled Householder QR of the joint observation/state residual
stack propagates those factors without adding noise to fill the null space.
If the lower factor is
\[
 L=\begin{bmatrix}L_y&0\\L_{xy}&L_f\end{bmatrix},
\]
then $L_f L_f^\top$ is the conditional state covariance and the mean correction
is $L_{xy}L_y^{-1}(y_t-\bar y_t)$. Only $L_y$ must be nonsingular. The filter
returns numerical validity flags even inside XLA; singular innovation laws
and nonfinite values cannot be accepted silently. Derivatives apply on a
fixed structural-rank branch, not at arbitrary rank changes.

The positive-weight unscented rule uses $\alpha=1$, $\beta=2$, $\kappa=0$
on the fixed augmented dimension. It does not integrate all fourth moments
exactly. The API therefore computes an approximate Gaussian likelihood;
passing factor and gradient tests does not establish posterior convergence.
...
equation for the SR-UKF factor branch.  Chapter~\ref{ch:bf-factor-derivatives}
supplies the QR and ordinary Cholesky smooth-branch identities used by value and
reconstruction audits.  The direct block-QR and rectangular QR derivatives are
derived below for the nonnegative-weight DZ5 route.  The monograph does
\emph{not} derive a complete ordered signed rank-one update/downdate derivative
for arbitrary negative-weight rules.  Such signed branches remain outside the
admitted default score contract unless a separate reviewed primitive and
reconstruction test are supplied.
...
\[
  ...
  P_{xz,\star}.
\]
The weighted state covariance object itself may be written in the same signed
stack notation as
\begin{equation}
\label{eq:bf-srukf-state-signed-covariance-stack}
  P_{xx,\star}=E_{x,+}E_{x,+}^\top-E_{x,-}E_{x,-}^\top,
\end{equation}
where \(E_{x,+}\) collects the positive covariance-weight state deviations and
\(E_{x,-}\) collects the negative covariance-weight state deviations.  The
innovation factor is the declared SR factor of the observation covariance:
\begin{equation}
...
signed factor branch.  Let the predicted state covariance object be written in
signed-stack form
\[
  P_{xx,\star}=E_{x,+}E_{x,+}^\top-E_{x,-}E_{x,-}^\top,
\]
where \(E_{x,+}\) collects the positive covariance-weight state deviations and
\(E_{x,-}\) collects the negative covariance-weight state deviations.  Let
\(C_{KS}\) denote a factor whose covariance is \(K S_\star K^\top\).  Then the
filtered branch must use the complete positive and negative stacks:
\begin{equation}
\label{eq:bf-srukf-filtered-factor}
  C_{t|t}
  =
  \operatorname{SR}_{\mathcal B_t}
  \left(E_{x,+},\,[E_{x,-}\ \ C_{KS}]\right),
  \qquad
  C_{t|t}C_{t|t}^\top=P_{t|t,\star},
\end{equation}
so that the signed branch reconstructs
\[
  E_{x,+}E_{x,+}^\top-E_{x,-}E_{x,-}^\top-C_{KS}C_{KS}^\top
  =
  P_{xx,\star}-KS_\star K^\top.
\]
When all covariance weights are nonnegative, \(E_{x,-}\) is empty and this
reduces to the usual positive-stack factor update.  On a negative-weight branch,
omitting \(E_{x,-}\) changes the covariance target and is therefore not the same
filtered object.  The first derivative must reconstruct the implemented branch:
\begin{equation}
...
\section{Implemented Direct-QR Recursions}
\label{sec:bf-srukf-implemented-direct-qr}

BayesFilter's default full-rank route is
\texttt{direct\_qr\_block\_conditional}.  For a predicted-state residual stack
\(X\) and an observation residual/noise stack \(Y\), form only the joint direct
stack
\[
 A=\begin{bmatrix}Y\\X\end{bmatrix},\qquad
 A^\top=Q
 \begin{bmatrix}R_{yy}&R_{yx}\\0&R_{xx}\end{bmatrix}.
\]
With positive QR diagonals,
\begin{equation}
\label{eq:bf-srukf-direct-block-qr}
 L_y=R_{yy}^\top,\qquad
 K=R_{yx}^\top R_{yy}^{-\top},\qquad
 L_f=R_{xx}^\top.
\end{equation}
Thus \(L_yL_y^\top\) is the innovation covariance and \(L_fL_f^\top\) is the
conditional state covariance.  The runtime forms neither covariance, no Schur
complement, and performs no SVD or eigendecomposition.  The same QR derivative
produces derivatives of all three objects.  A scale-aware relative pivot floor
of \(10^{-12}\) rejects a numerically unresolved full-rank chart.

Exact singular branches use the rectangular construction in
Section~\ref{sec:bf-fixed-rank-rectangular-qr}.  Rank discovery may use SVD on
the direct stack before the differentiated run.  The score run freezes rank,
row-pivot permutations, positive QR signs, support identity, and tolerances,
then uses QR only.  Sigma points are generated in the retained latent factor
width, not padded ambient zero directions.  The update uses
\[
 Y=URV^\top,\qquad
 K=XV R^{-1}U^\top,\qquad
 X_f=X(I-VV^\top),
\]
where these quantities are obtained from fixed QR charts.  A second fixed
rectangular QR chart factors \(X_f\).  The likelihood is the affine-support
density~\eqref{eq:bf-affine-support-log-density}.

The score is local.  If rank, pivot permutation, sign, chart residual,
observation support, or retained support changes, diagnostics set
\texttt{score\_valid=false} and return a non-finite score sentinel.  The
value-only SVD discovery route is never a hidden score fallback.  The current
DZ5 rule has nonnegative covariance weights; this fixed-support admission does
not extend to arbitrary signed-weight rules or across their PSD boundaries.
