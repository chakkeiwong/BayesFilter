...
optim <- function(par,...,control) {
  control$lmm <- max(5,length(par))
  original_optim(par,...,control=control)
}
rows <- list()
for(i in seq_along(paths)) {
  saved <- readRDS(file.path(args[1],paths[i])); start <- proc.time()[[3]]
  fit <- tryCatch(iapf_fit_gaussian(saved$points,saved$log_targets,
    maxit=5000,fit_mode="relative_l2"),error=function(e)e)
  ...
  rows[[i]] <- data.frame(case=i,path=paths[i],dimension=ncol(saved$points),
    memory=max(5,2*ncol(saved$points)),passed=ok,elapsed=proc.time()[[3]]-start,
  ...
  write.csv(do.call(rbind,rows),file.path(output,"solver-memory.csv"),row.names=FALSE)
}
print(do.call(rbind,rows)[,c("case","dimension","memory","passed","elapsed","residual","gradient","evaluations")])
