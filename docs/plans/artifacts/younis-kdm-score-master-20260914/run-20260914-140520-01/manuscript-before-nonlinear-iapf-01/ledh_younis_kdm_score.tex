...
these results and, for models with regular transition densities, derive a
different use of the same mixture ideas: construct evaluable proposals,
retain the original transition and observation densities in the importance
correction, and estimate the observed-data score by averaging complete-data
scores over possible ancestors. This smoothing recursion does not apply as
written to degenerate DSGE transitions. For those models we instead derive
a disturbance-coordinate filter, its analytical state sensitivities, and an
unbiased unnormalized likelihood--derivative pair. Propositions and proofs
establish the importance correction, the particle induction, exact reference
control variates, and conditional integration. A moving-support Gaussian
example verifies the resulting score identity. Normalization can still
introduce finite-particle bias, and long-horizon variance remains unresolved.
Selective pathwise and importance-weight derivatives supply additional
variance-reduction candidates.

The unbiasedness statement in the Younis--Sudderth construction has a more
specific target. It is an importance-sampling identity for the gradient of a
KDM expectation with the current proposal held fixed. When the integrand is a
conditional log likelihood, the resulting quantity is an expected conditional
log likelihood. It is not, in general, the log of the observed-data marginal
likelihood, whose finite-particle score also contains a random normalization.
...
\subsection{The precise scope of IWSG unbiasedness}
\label{sec:iwsg-scope}

The word ``unbiased'' in the IWSG argument refers to a particular expectation
and a particular differentiation convention. It should not be read as a
claim that every finite-particle log-likelihood score is unbiased. Let
$m_\theta$ be a positive mixture density with respect to a fixed reference
measure and let $F_\theta(z)$ be a scalar downstream integrand. Define
\begin{equation}
 \mathcal J(\theta)
   =\int F_\theta(z)m_\theta(z)\,\dd z.
 \label{eq:iwsg-target-expectation}
\end{equation}
Fix an anchor $\theta_0$, condition on the current incoming cloud, and draw
$z_j\sim q_0$ with $q_0(z)=m_{\theta_0}(z)$. The proposal $q_0$ is detached
during the local differentiation. Assume that the support is common in a
neighbourhood of $\theta_0$ and that differentiation may pass through the
integral. The fixed-proposal estimator is
\begin{equation}
 \widehat G_M
 =\frac1M\sum_{j=1}^M
   \left[
      \left.\nabla_\theta F_\theta(z_j)\right|_{\theta_0}
      +F_{\theta_0}(z_j)
       \left.\nabla_\theta\log m_\theta(z_j)\right|_{\theta_0}
   \right].
 \label{eq:iwsg-local-gradient}
\end{equation}

\begin{proposition}[IWSG expectation-gradient identity]
\label{prop:iwsg-expectation-gradient}
Under the conditions above, conditional on the current cloud,
\begin{equation}
 \mathbb E_{q_0}[\widehat G_M]
 =\left.\nabla_\theta\mathcal J(\theta)\right|_{\theta_0}.
 \label{eq:iwsg-unbiased-expectation-gradient}
\end{equation}
The same statement holds for a general proposal $q_0$ when each summand is
multiplied by $m_{\theta_0}(z_j)/q_0(z_j)$.
\end{proposition}

\begin{proof}
At the anchor, $q_0=m_{\theta_0}$. Therefore the expectation of one summand
in \eqref{eq:iwsg-local-gradient} is
\begin{align*}
 &\int q_0(z)\left[
      \left.\nabla_\theta F_\theta(z)\right|_{\theta_0}
      +F_{\theta_0}(z)
       \left.\nabla_\theta\log m_\theta(z)\right|_{\theta_0}
    \right],\dd z \\
 &=\int\left[
      \left.\nabla_\theta F_\theta(z)\right|_{\theta_0}m_{\theta_0}(z)
      +F_{\theta_0}(z)
       \left.\nabla_\theta m_\theta(z)\right|_{\theta_0}
    \right],\dd z \\
 &=\left.\nabla_\theta\int F_\theta(z)m_\theta(z)\,\dd z\right|_{\theta_0}.
\end{align*}
The average has the same expectation. For a general proposal, inserting the
ratio $m_{\theta_0}/q_0$ gives the same integral. The denominator is fixed
because it is the law used to generate the current sample; differentiating it
would define a different estimator rather than the local derivative in
\eqref{eq:iwsg-target-expectation}.
\end{proof}

The proposition is a conditional statement: it is made after the current
mixture parameters and incoming cloud have been fixed. If those quantities
are random, the identity can be averaged again by the tower property, but
only for the expectation objective actually defined by the complete
recursion. Any upstream dependence of the cloud that the declared objective
retains must still be differentiated. Holding the proposal density fixed is a
local sampling convention; it does not authorize dropping total derivatives
elsewhere in the filter.

The distinction becomes visible when the integrand is a log likelihood. If
\begin{equation}
 F_\theta(z)=\log g_\theta(y\mid z),
 \qquad
 \mathcal J_{\rm cond}(\theta)
 =\mathbb E_{m_\theta}\left[\log g_\theta(y\mid Z)\right],
 \label{eq:iwsg-conditional-log-objective}
\end{equation}
then Proposition~\ref{prop:iwsg-expectation-gradient} estimates the gradient
of the expected conditional log likelihood. In the simple case where
$m_\theta$ is the prior, the corresponding observed-data marginal log
likelihood is
\begin{equation}
 \ell_{\rm marg}(\theta)
 =\log\int g_\theta(y\mid z)m_\theta(z)\,\dd z.
 \label{eq:marginal-log-vs-expected-log}
\end{equation}
The logarithm is outside the integral in
\eqref{eq:marginal-log-vs-expected-log}, whereas it is inside the expectation
in \eqref{eq:iwsg-conditional-log-objective}. Consequently,
\begin{equation}
 \mathbb E_{m_\theta}[\log g_\theta(y\mid Z)]
 \ne
 \log\mathbb E_{m_\theta}[g_\theta(y\mid Z)]
 \label{eq:expected-log-not-log-expectation}
\end{equation}
except in special cases. Jensen's inequality gives the direction in the
ordinary prior case, but it does not turn the two objectives into the same
score. A finite particle average makes the distinction concrete:
\begin{equation}
 \frac1M\sum_j\log g_\theta(y\mid z_j)
 \ne
 \log\left(\frac1M\sum_j g_\theta(y\mid z_j)\right).
 \label{eq:finite-log-order}
\end{equation}

For the generative state-space score, write the model normalizer as
\begin{equation}
 Z(\theta)=\int u_\theta(z)\,\dd z,
 \qquad
 \ell(\theta)=\log Z(\theta),
 \label{eq:iwsg-generative-normalizer}
\end{equation}
where $u_\theta$ contains the initial, transition, and observation factors
in the declared model. With a fixed proposal $q_0$, importance sampling can
give unbiased estimators of the unnormalized value and derivative,
\begin{align}
 \widehat Z_M
 &=\frac1M\sum_j\frac{u_{\theta_0}(z_j)}{q_0(z_j)},
 &\mathbb E[\widehat Z_M]&=Z(\theta_0),\nonumber\\
 \widehat D_M
 &=\frac1M\sum_j\frac{\left.\nabla_\theta u_\theta(z_j)\right|_{\theta_0}}{q_0(z_j)},
 &\mathbb E[\widehat D_M]&=\left.\nabla_\theta Z(\theta)\right|_{\theta_0}.
 \label{eq:iwsg-unnormalized-pair}
\end{align}
The natural finite score is nevertheless
\begin{equation}
 \widehat S_M=\frac{\widehat D_M}{\widehat Z_M},
 \label{eq:iwsg-ratio-score}
\end{equation}
and generally
\begin{equation}
 \mathbb E[\widehat S_M]
 \ne
 \frac{\mathbb E[\widehat D_M]}{\mathbb E[\widehat Z_M]}
 =\nabla_\theta\log Z(\theta_0).
 \label{eq:iwsg-ratio-bias}
\end{equation}
This is a normalization and ratio issue, not a failure of the local IWSG
identity. An expectation of a complete-data log density can equal a marginal
log likelihood only under an additional exact-posterior identity, for example
\begin{equation}
 \log p_\theta(y)
 =\mathbb E_{p_\theta(z\mid y)}
   \left[\log p_\theta(y,z)-\log p_\theta(z\mid y)\right].
 \label{eq:exact-posterior-log-identity}
\end{equation}
A KDM mixture used as an approximate filter or proposal is not automatically
that posterior, and $F=\log g$ omits the second, entropy term in
\eqref{eq:exact-posterior-log-identity}.

For a sequential particle filter, each local KDM/IWSG step can therefore be
mathematically correct for its declared mixture expectation while the full
finite program still differs from the exact marginal score. Repeated
normalization, nonlinear resampling or transport, and a changed positive-
bandwidth filtering measure all affect the complete target. The
\texttt{RESKDM-IWSG-FINITE} route defined below has a valid total derivative
of its own finite scalar; the source IWSG result does not identify that scalar
with $\nabla_\theta\log p_\theta(y_{1:T})$ without a separate target and
finite-particle analysis.
...
or backward acceptance becomes small. ``Always quadratic'' was therefore an
incorrect cost argument. Correcting that argument does not remove the
structural obstruction from degenerate transitions: sampling backward
ancestors cannot create transitions that the model forbids. A comparison
with ancestor-averaged methods is a question for models whose transition and
backward-conditioning assumptions hold.

For those regular-transition models, a reference comparison assigns different
jobs to three components.
...
must be differentiated. This combination has not been tested here. Its
components have explicit roles in the model-score calculation, but that
does not make it a general solution for the degenerate DSGE target. The
support obstruction in Section~\ref{sec:fisher-degenerate} must be resolved
before proposing a score recursion for that target.
...
\section{Proposal-quality branch: twisting, iAPF, covariance, and SGQF}
\label{sec:proposal-quality}

The proposal question is separate from the score-identity question. A proposal
can place particles in regions with large observation likelihood and thereby
reduce weight variation, but that improvement is meaningful only if the
physical target is left unchanged. The common one-step notation makes the
boundary explicit. Given the incoming weighted cloud, let
\begin{equation}
 u_t(x)=g_\theta(y_t\mid x)\sum_a W_{t-1}^a
                 f_\theta(x\mid x_{t-1}^a)
\end{equation}
be the unnormalised predictive target and let $q_t$ be the actual marginal
proposal density. Every proposal in this section uses
\begin{equation}
 v_t^i=\frac{u_t(x_t^i)}{q_t(x_t^i)},\qquad
 \widehat z_t=N^{-1}\sum_i v_t^i.                 \label{eq:proposal-branch-weight}
\end{equation}
Changing $q_t$ is allowed; replacing $u_t$ by a Gaussian or quadrature
approximation is a different filtering target.

\subsection{Twisted particle filters and auxiliary lookahead}

Let $\psi_t(x)>0$ be a lookahead function. A twisted or auxiliary filter uses
it to prefer ancestors and states that are expected to explain future
observations. For example, define
\begin{equation}
 \bar\psi_{t-1}(a)=\int f_\theta(x\mid x_{t-1}^a)\psi_t(x)\,\dd x,
 \qquad
 r_t(a)\propto W_{t-1}^a\bar\psi_{t-1}(a),
 \label{eq:twist-ancestor}
\end{equation}
and draw $A_t\sim r_t$ followed by $X_t\sim k_t(\cdot\mid A_t)$.
The proposal is the joint law $r_t(a)k_t(x\mid a)$, or its marginal in $x$
when the ancestor is integrated out. The importance ratio must therefore use
the same joint or marginal denominator:
\begin{equation}
 v_t(a,x)=
 \frac{W_{t-1}^a f_\theta(x\mid x_{t-1}^a)g_\theta(y_t\mid x)}
      {r_t(a)k_t(x\mid a)}.                       \label{eq:twist-correction}
\end{equation}
Equivalent twisted-kernel formulations multiply a transition by a positive
function and divide by its corresponding normalising factor
\citep{whiteley2014twisted}. These correction factors are what preserve the
original likelihood. The ideal lookahead is a future-likelihood or eigenfunction
object; a practical $\psi_t$ is only an approximation to it. Consequently,
twisting is a proposal-quality experiment, not a new score identity. For
score diagnostics we freeze the fitted $\psi_t$ and differentiate the declared
finite target, while separately recording any derivative of the proposal that
would be needed by a total-derivative claim.

The twisted branch is admissible for a degenerate transition only when the
twist is defined on the same support and the required conditional law is
available. A positive function on an ambient Euclidean space does not create a
density for a transition supported on a lower-dimensional manifold.

\subsection{The iterated auxiliary particle filter}

The iterated auxiliary particle filter (iAPF) constructs the lookahead rather
than assuming that it is known \citep{guarniero2017iapf}. A pilot particle run
produces an approximation to the future value function; a new auxiliary filter
uses that approximation as $\psi_t$; and the procedure is iterated offline.
Each iteration is still an auxiliary particle filter with an explicit proposal
and correction of the form \eqref{eq:twist-correction}. The iteration may
improve ancestor selection, but it does not make the pilot likelihood an exact
normalizer and it does not provide an unbiased derivative of the marginal
likelihood. The pilot data, number of iterations, and fitted $\psi_t$ must be
fixed before a claim run, or the adaptation must be included in a separate
derivation.

In the LEDH implementation an iAPF can replace the UKF only in the proposal
construction: it may supply $r_t$, proposal centres, or a lookahead covariance.
The physical transition and observation factors remain in the numerator, and
the complete proposal density (including ancestor probabilities and any LEDH
Jacobian) remains in the denominator. The comparison therefore asks whether
better lookahead improves coverage at equal cost; it does not ask whether iAPF
is a replacement filtering theory for the target model.

The numerical fitting problem also needs a precise specification. In
Section 5.1, equation (15), \citet{guarniero2017iapf} fit a Gaussian density
to positive backward targets $y_i$ by minimizing
\begin{equation}
 L(m,\Sigma,\lambda)
   =\sum_{i=1}^n\{p_i(m,\Sigma)-\lambda y_i\}^2,
 \qquad p_i(m,\Sigma)=\mathcal N(x_i;m,\Sigma).
 \label{eq:iapf-density-fit}
\end{equation}
Here $y_i=g(x_i,y_t)f(x_i,\psi_{t+1})$ in their backward approximation;
the symbol $y_i$ in this fitting equation denotes a positive function value,
not an observation. The scale $\lambda$ accommodates an arbitrary positive
normalization of that function. A log-density regression is a different
criterion, even if it produces a useful Gaussian lookahead.

\begin{proposition}[Profiled scale and possible absence of a minimizer]
For fixed positive targets and a fixed nonsingular Gaussian, write
$a=\sum_i p_i^2$, $b=\sum_i p_i y_i$, and $c=\sum_i y_i^2$.
Then the unique optimal scale is $\lambda=b/c>0$, and
\begin{equation}
 L=a-\frac{b^2}{c}+c\left(\lambda-\frac bc\right)^2.
 \label{eq:iapf-profiled-loss}
\end{equation}
Over unrestricted means and positive-definite covariances, the infimum of
\eqref{eq:iapf-density-fit} is zero, but it need not be attained.
\end{proposition}
\begin{proof}
Expanding the squares gives $a-2\lambda b+\lambda^2c$; completing the
square gives \eqref{eq:iapf-profiled-loss}. Positivity of the targets and
Gaussian densities gives $b,c>0$.
For the second claim, fix $m$ and take $\Sigma=s^2I_d$. Each density is
bounded by $C_s=(2\pi)^{-d/2}s^{-d}$. Choosing $\lambda=C_s$ gives
\[
 0\le L(m,s^2I_d,C_s)
      \le C_s^2\sum_i(1+y_i)^2\longrightarrow0.
\]
Since the loss is nonnegative, its infimum is zero. To see why a minimizer
may be absent, take $d=1$, $x_i\in\{-1,0,1\}$, and all three targets
equal to one. A zero loss would require equal Gaussian densities at all
three points. Equality at $-1$ and $1$ forces $m=0$; for every finite
positive variance the density at zero is then strictly larger. Thus no
finite Gaussian and scale attain zero in this example.
\end{proof}

The proposition identifies an ambiguity in an unrestricted global-argmin
interpretation; it does not contradict the paper's reported numerical
results. A local optimizer can return a useful finite fit. A compact mean
region and diagonal standard deviations bounded above and away from zero
also ensure that the profiled loss attains a minimum, by continuity on a
compact set. Finding that minimum numerically remains a separate question.
The implementation therefore states its bounds, initialization, local
solver and stopping rule. It records the covariance scale and the normalized
shape residual $\sum_i(p_i-\widehat\lambda y_i)^2/\sum_i p_i^2$ as well as
the absolute loss: broadening a Gaussian can reduce its density amplitude
without supplying an accurate lookahead.

For diagonal standard deviations $\sigma_j$, let
$r_i=p_i-\widehat\lambda y_i$. The normal equation
$\sum_i r_i y_i=0$ cancels the derivative of the profiled scale. Differentiating
the average loss $\overline L=L/n$ therefore gives
\begin{align}
 \frac{\partial\overline L}{\partial m_j}
 &=\frac2n\sum_i r_i p_i\frac{x_{ij}-m_j}{\sigma_j^2},\\
 \frac{\partial\overline L}{\partial\log\sigma_j}
 &=\frac2n\sum_i r_i p_i
       \left\{\frac{(x_{ij}-m_j)^2}{\sigma_j^2}-1\right\}.
\end{align}
These are gradients of the density-scale criterion. Dividing all densities
by a fixed positive constant merely rescales the objective; replacing it by
the normalized shape residual as the optimization target would change the
criterion. The shape residual is a diagnostic here.

Small density amplitudes also require care in floating-point arithmetic.
Set $M=\max_i\log p_i$, $q_i=\exp(\log p_i-M)$ and
$a=\sum_i q_i y_i/\sum_i y_i^2$. Then
\[
 \widehat\lambda=e^M a,\qquad
 L(m,\Sigma,\widehat\lambda)
   =e^{2M}\sum_i(q_i-a y_i)^2.
\]
This evaluates the same density criterion. Computing $\log\widehat\lambda$
with log-sum-exp and the relative shape error from $q_i$ avoids dividing two
underflowed sums. The loss and its analytical gradient still retain the
factor $e^{2M}$. If that factor falls below the numerical range, the
implementation reports the underflow together with the fitted scale, active
bounds and shape error. A tiny absolute loss or gradient can therefore end
the numerical search without demonstrating an accurate lookahead.

The particle-count adaptation is a further part of the algorithm, rather
than an implication of this fit. Algorithm 4 of \citet{guarniero2017iapf}
starts with constant twists, checks the coefficient of variation of the
latest $k+1$ likelihood estimates when the iteration index exceeds $k$,
and may double the particle count when a complete equal-count window fails
to increase monotonically. It then runs an independent final filter. Our
bounded implementation declares the standard-deviation convention, finite
iteration and particle caps, and behavior at an active fit bound. Exhausting
a cap is a reported failure to converge. For the final finite-program score,
the fitted coefficients and realized particle count are frozen; the score
does not differentiate the offline stopping decision. These choices preserve
the distinction between a fitted proposal, a derivative of its final random
likelihood program, and the exact model score.

\subsection{Alternative covariance geometries}

Covariance is a proposal geometry, not a correction to the model. The program
will distinguish the following matrices at each proposal step:
\begin{align}
 P_t^{Q}&=\text{the covariance supplied by the physical transition},\\
 P_t^{\mathrm{UKF}}&=\text{the per-particle UKF covariance},\\
 P_t^{\mathrm{KDM}}&=\text{the weighted covariance of the continuous mixture},\\
 P_t^{\mathrm{LEDH}}&=\text{the local flow/GenUT covariance},\\
 P_t^{\mathrm{SGQF}}&=\text{the covariance projected from a quadrature cloud}.
\end{align}
An optional hybrid has the form
$P_t^{\mathrm{hyb}}=\alpha P_t^{\mathrm{UKF}}+(1-\alpha)P_t^{\mathrm{SGQF}}$,
with $\alpha$ tuned on calibration data and frozen on the claim scope. Every
matrix is checked for symmetry, positive definiteness, scale, and condition
number. If it changes the sampling distribution, the resulting $q_t$ and its
derivatives must change consistently; a covariance label cannot be added to a
weight after the samples have been drawn.

\subsection{Structured Gaussian quadrature as a proposal source}

The fixed structured Gaussian quadrature filter (SGQF) supplies another way to
construct the moments used in a proposal. These moments must respond to each
new observation. For clarity, consider additive, independent process and
observation noise with covariances $Q_\theta$ and $R_\theta$. At time $t-1$,
let a component have conditioned mean $m_{t-1}$ and covariance
$P_{t-1}=L_{t-1}L_{t-1}^{\mathsf T}$. A standardized quadrature rule
$\{(\zeta_\ell,\omega_\ell)\}$, with $\sum_\ell\omega_\ell=1$, gives nodes
$x_{t-1,\ell}=m_{t-1}+L_{t-1}\zeta_\ell$. Propagating these nodes through
$a_\theta$, the deterministic part of the transition, gives
$a_{t,\ell}=a_\theta(x_{t-1,\ell})$ and the prediction
\begin{equation}
 m_t^-=\sum_\ell \omega_\ell a_{t,\ell},\qquad
 P_t^-=Q_\theta+\sum_\ell\omega_\ell
       (a_{t,\ell}-m_t^-)(a_{t,\ell}-m_t^-)^\mathsf T.
                                                        \label{eq:sgqf-moments}
\end{equation}
For nonadditive noise, the quadrature must instead cover the joint state and
noise law; simply adding $Q_\theta$ would then compute the wrong prediction.

Conditioning is a second operation. Form fresh quadrature nodes
$x_{t,\ell}^-=m_t^-+L_t^-\zeta_\ell$, where
$L_t^-(L_t^-)^{\mathsf T}=P_t^-$, and evaluate
$b_{t,\ell}=b_\theta(x_{t,\ell}^-)$, the observation mean map. Define
\begin{align}
 \bar y_t&=\sum_\ell\omega_\ell b_{t,\ell},\qquad
 S_t=R_\theta+\sum_\ell\omega_\ell
 (b_{t,\ell}-\bar y_t)(b_{t,\ell}-\bar y_t)^{\mathsf T},\nonumber\\
 C_t&=\sum_\ell\omega_\ell(x_{t,\ell}^--m_t^-)
                         (b_{t,\ell}-\bar y_t)^{\mathsf T},\qquad
 K_t=C_t S_t^{-1},\nonumber\\
 m_t&=m_t^-+K_t(y_t-\bar y_t),\qquad
 P_t=P_t^--K_t S_t K_t^{\mathsf T}.       \label{eq:sgqf-conditioning}
\end{align}
This is an assumed-Gaussian conditioning approximation, exact for the
linear-Gaussian model when the rule integrates the required second moments.
Likelihood-reweighting the nodes is a different approximation and must be
identified as such. In particular, a sparse quadrature rule may have negative
weights. These are integration weights, not categorical sampling probabilities;
normalising them does not make them valid resampling weights. Positive
definiteness of both $S_t$ and the proposed $P_t$ must be checked.

The conditioned component state $(m_t,P_t)$ is carried into the next
prediction through the declared component-assignment and reset operation.
When an OT reset changes that assignment, the covariance provider must consume
the actual reset state and propagate its derivatives. Reusing a fixed
$(m_0,P_0)$ at every date would omit observation dependence. The analytical
score therefore needs the derivatives of the initial moments, propagated
nodes, noise covariances, conditioning solve, and subsequent reset, including
their dependence on $\theta$. This lifecycle applies separately to every
component if a mixture supplies the proposal.

These moments can parameterise a Gaussian, mixture, or LEDH proposal. The
repository interfaces \texttt{tf\_fixed\_sgqf\_cloud} and
\texttt{tf\_fixed\_sgqf\_filter} provide the cloud and filtering moments; the
current route is an eager/Python-branch, approximate proposal-design lane.
Its sparse-grid point count and memory must be recorded as dimension and level
increase. It is not a claim-bearing replacement for the physical filter, and
its approximate likelihood must never be used in place of $u_t$ in
\eqref{eq:proposal-branch-weight}.

The first SGQF experiment therefore takes the conditioned $m_t$ and $P_t$ to define
$q_t^{\mathrm{SGQF}}$, evaluates the exact model numerator and proposal
denominator, and compares weight tails with UKF, KDM, and LEDH moments. A
high-dimensional result is admissible only after the call chain, batching,
device, and scaling checks show that the proposal is the one actually used by
the claim-bearing lane.

\subsection{Systematic proposal test}

The proposal study is a factorial comparison, with the score estimator held
fixed while proposal quality is measured. Its proposal families are bootstrap,
locally adapted, KDM, LEDH/GenUT, twisted PF, iAPF, SGQF-guided, and defensive
mixtures. Crossed factors are lookahead (none, fixed one-step, iterated $\psi$),
covariance ($P^Q$, UKF, KDM, LEDH, SGQF, and calibrated hybrid), particle count,
horizon, and observation regime. Model-corrected normalizer MSE is the primary
criterion for a normalizer-accuracy claim. A better-score claim requires lower
oracle score MSE at the declared particle or compute budget, after the proposal
identity and target-preservation tests pass. Coefficient of variation of
$\widehat z_t$, effective sample size, and weight tails explain the result;
they do not replace score error as the criterion.
The linear-Gaussian/Kalman model is the first oracle-bearing check; nonlinear
regular-transition models and high-dimensional SGQF cases follow.

Hard vetoes are a missing or non-integrable proposal density, support failure,
non-finite or incorrectly corrected weights, use of an approximate SGQF or
twisted likelihood as the target, and a claim route that cannot be traced to the
declared proposal. A paired uncertainty analysis is required before calling a
candidate better. A passing proposal test establishes a viable proposal arm;
it does not establish an unbiased score.

\section{Directional finite-difference validation ladder}
\label{sec:fd-ladder}

Finite differences provide an independent directional check on a score, not an
automatic bias oracle. For a unit direction $v$, define
$s_v=v^\mathsf T\nabla_\theta\ell(\theta)$ and evaluate the same particle
likelihood program at a fixed symmetric set of perturbations. The three-point
central estimate is
\begin{equation}
 D_2(h;v)=\frac{\ell(\theta+hv)-\ell(\theta-hv)}{2h},
 \qquad D_2(h;v)=s_v+O(h^2),
 \label{eq:fd-three-point}
\end{equation}
and the five-point fourth-order estimate is
\begin{equation}
 D_4(h;v)=\frac{\ell(\theta-2hv)-8\ell(\theta-hv)+8\ell(\theta+hv)-\ell(\theta+2hv)}{12h},
 \qquad D_4(h;v)=s_v+O(h^4).                    \label{eq:fd-five-point}
\end{equation}
For an eleven-point cubic fit, let $k=-5,\ldots,5$ and fit
\begin{equation}
 (\widehat\beta_0,\ldots,\widehat\beta_3)
 =\arg\min_\beta\sum_{k=-5}^{5}
 \left[\widehat\ell(\theta+khv)-\sum_{r=0}^{3}\beta_r(kh)^r\right]^2,
 \qquad D_3(h;v)=\widehat\beta_1.              \label{eq:fd-cubic-fit}
\end{equation}
Write $f(u)=\ell(\theta+uv)$. A bounded third derivative near zero suffices
for the second-order statement in \eqref{eq:fd-three-point}. A bounded fifth
derivative, with $f\in C^5$, suffices for the fourth-order statement in
\eqref{eq:fd-five-point} and for the symmetric cubic fit. Four continuous
derivatives alone do not suffice: $f(u)=\operatorname{sign}(u)|u|^{4+\alpha}$,
$0<\alpha<1$, has four continuous derivatives but gives derivative error
of order $h^{3+\alpha}$ for the five-point rule.

\begin{proposition}[Symmetric cubic derivative weights]
\label{prop:fd-cubic-weights}
For the eleven equally spaced nodes in \eqref{eq:fd-cubic-fit}, define
$S_r=\sum_{j=-5}^{5}j^r$. The fitted derivative is
\begin{equation}
 D_3(h;v)=\frac1h\sum_{j=-5}^{5}c_j\widehat\ell(\theta+jhv),\qquad
 c_j=\frac{S_6j-S_4j^3}{S_2S_6-S_4^2},\qquad
 (S_2,S_4,S_6)=(110,1958,41030).          \label{eq:fd-cubic-weights}
\end{equation}
For a deterministic $f\in C^5$, the leading errors are
$D_4-f'(0)=-f^{(5)}(0)h^4/30+o(h^4)$ and
$D_3-f'(0)=-143f^{(5)}(0)h^4/90+o(h^4)$.
\end{proposition}
\begin{proof}
Symmetry separates the odd and even columns of the least-squares design.
Fitting against the dimensionless columns $j$ and $j^3$ gives the odd normal
matrix $\left(\begin{smallmatrix}S_2&S_4\\S_4&S_6\end{smallmatrix}\right)$.
Its first inverse row gives $c_j$; division by $h$ converts the fitted
coefficient to a derivative. Hence $\sum c_j=0$, $\sum jc_j=1$,
$\sum j^2c_j=\sum j^3c_j=\sum j^4c_j=0$.
Taylor expansion leaves $h^4 f^{(5)}(0)\sum j^5c_j/5!$.
The last sum divided by $5!$ is $-143/90$. Applying the same calculation to
the coefficients $(1,-8,0,8,-1)/12$ at $j=(-2,-1,0,1,2)$ gives $-1/30$.
\end{proof}
Thus more points do not give a higher formal order. The eleven-point cubic fit
has a different truncation constant and a different response to correlated
likelihood noise; a symmetric linear or quadratic fit generally retains an
$O(h^2)$ term.

\begin{proposition}[Finite-difference mean squared error]
\label{prop:fd-mse}
Let $\widehat L_j$ be the random log-likelihood estimate at node $j$, with
finite second moments and joint covariance matrix $\Sigma$. For coefficients
$c$ that reproduce a first derivative, put
\begin{equation}
 T_h=\frac1h\sum_j c_j\ell(\theta+jhv)-s_v,\qquad
 B_{N,h}=\frac1h\sum_j c_j\{\mathbb E\widehat L_j-\ell(\theta+jhv)\}.
\end{equation}
Then
\begin{equation}
 \mathbb E\left[\left(\frac1h\sum_jc_j\widehat L_j-s_v\right)^2\right]
 =(T_h+B_{N,h})^2+\frac{c^{\mathsf T}\Sigma c}{h^2}.
                                                        \label{eq:fd-mse}
\end{equation}
\end{proposition}
\begin{proof}
The estimator's mean error is $T_h+B_{N,h}$. Its centered part is
$h^{-1}c^{\mathsf T}(\widehat L-\mathbb E\widehat L)$, with variance
$h^{-2}c^{\mathsf T}\Sigma c$. Expanding the squared error, the mixed term
with the centered part has zero expectation. The squared bias still contains
$2T_hB_{N,h}$; the truncation and particle biases need not have the same sign.
\end{proof}
A universal U-shaped error curve does not follow. Shared additive noise
cancels because $\sum_jc_j=0$. A mean-square differentiable coupled random
likelihood can have a bounded derivative variance as $h\downarrow0$.
Independent node noise with nonvanishing variance instead yields the familiar
$h^{-2}$ variance growth. The joint coupling, as well as the stencil, matters.

Use a deterministic positive ladder $\mathcal H=\{h_0,h_0/2,\ldots\}$,
with $h_0$ in parameter units and $v$ normalised. An unrestricted normal draw
of $h$ is excluded because values close to zero divide likelihood noise by a
small number. A truncated positive distribution is a later randomized arm,
not the default; the negative evaluations are the paired points in the same
symmetric stencil. Common random numbers or a declared coupling are used to
reduce variance, but they do not remove finite-particle, log-normalisation, or
truncation bias.

For a $d$-parameter score, use $d$ independent coordinate directions or a
full-row-rank matrix $V=(v_1,\ldots,v_m)\in\mathbb R^{d\times m}$. With
directional estimates $b$, the least-squares solution is
\begin{equation}
 \widehat s=(VV^{\mathsf T})^{-1}Vb.        \label{eq:fd-direction-solve}
\end{equation}
Implementation uses QR or SVD rather than an explicit inverse. Singular
values, directional-error propagation, parameter scale and dtype determine
whether reconstruction is informative; an arbitrary condition-number cutoff
does not establish that. Every perturbed parameter must remain in the declared
model domain and be distinct at the working precision. If directions are
defined in transformed coordinates, the reported likelihood score must include
the corresponding coordinate chain rule.

The experiment has two separate checks. Deterministic analytic functions or
exact Kalman log likelihoods test truncation slopes near two or four before
roundoff dominates. Stochastic particle runs estimate the full error and
replicate covariance in \eqref{eq:fd-mse}; those curves need not have either
slope. Direct averages of squared oracle errors estimate MSE, whereas squaring
a noisy sample mean does not give an unbiased squared-bias estimate. Calibrate
the positive ladder and coupling on oracle-bearing data, validate and freeze
them, then evaluate on untouched data. Without an oracle the ladder remains a
diagnostic or parity calculation; agreement alone cannot certify model-score
bias or supply a known control-variate center.

\section{A model score that averages over ancestors}
\label{sec:fisher}

A well-chosen proposal can improve the particle approximation, but a separate question
...
supplies another, aimed directly at the observed-data score. The recursion
below is a smoothing construction, even though its history averages are
updated forward in time. It requires evaluable transition densities and a
valid backward conditional. It does not apply as written to the degenerate
DSGE transitions discussed in Section~\ref{sec:fisher-degenerate}.
...
\subsection{Why degenerate transitions obstruct this recursion}
\label{sec:fisher-degenerate}

Consider a state equation with fewer independent shocks than state
coordinates,
\begin{align}
 x_t&=A_\theta x_{t-1}+G_\theta\varepsilon_t,
 \qquad \varepsilon_t\sim N(0,I_r),\\
 \mathcal S_j&=A_\theta x_{t-1}^j+\operatorname{range}(G_\theta),
 \qquad \operatorname{rank}(G_\theta)=r<d_x.
 \label{eq:degenerate-ancestor-supports}
\end{align}
Here $x_t\in\R^{d_x}$, and $\mathcal S_j$ is the set of successors allowed
by ancestor $j$. Each conditional transition is supported on that affine
subspace and has no full-dimensional Lebesgue density. Thus
\eqref{eq:backward-particle-probabilities} cannot be evaluated with ordinary
ambient Gaussian densities. A pseudoinverse and pseudodeterminant alone do
not repair it: a rank-aware density must also enforce the correct support.

The AR(2) example in \eqref{eq:ar2-support-distinction} makes the failure
concrete. A child drawn from ancestor $a$ satisfies
$x_{t,2}=x_{t-1,1}^a$. At a fixed parameter, conditioning the finite mixture
on this supporting line excludes every ancestor with a different first
coordinate. If these coordinates are distinct, the compatible ancestor
probabilities are
\begin{equation}
 b_t^{ji}=\mathbf 1\{j=a\}.
 \label{eq:degenerate-ancestor-collapse}
\end{equation}
The proposed history average then collapses to the sampled genealogy. It
does not recover the alternative histories needed to repair genealogical
degeneracy. PaRIS changes how backward ancestors are sampled; it cannot
give positive probability to these incompatible transitions.

This obstruction concerns the conditional transitions between fixed
particles. The exact predictive marginal can still be full dimensional
after integrating over earlier continuous state uncertainty, as the same
AR(2) example shows. That fact does not make the finite mixture of distinct
conditional supports full dimensional. An ambient Gaussian proposal assigns
zero probability to their union, so it cannot provide the exact importance
correction for that finite singular mixture. Adding full-dimensional process
noise changes the model; smoothing the finite predictive mixture into an
ambient KDM changes the approximation. Neither operation establishes the
displayed model-corrected recursion for the original degenerate transition.

Innovation coordinates are a possible starting point for a different
derivation. They require the joint likelihood to be written in those
coordinates, including the initial law and the dependence of the state map
on $\theta$. Substituting an innovation density for
$f_\theta(x_t\mid x_{t-1})$ in the existing backward weights is not such a
derivation. In particular, moving support contributes state-map derivatives
to the observation factors, as \eqref{eq:moving-support-score} demonstrates.

Some degenerate models admit an exact reduction to common stochastic
coordinates, or another specially derived smoothing method. The obstruction
therefore does not rule out every form of smoothing. It does rule out the
ordinary all-ancestor recursion above as a general method for the degenerate
DSGE target. A successful full-rank Gaussian comparison would establish no
extension to that target. The disturbance construction below instead needs
its own support and differentiation assumptions.

\subsection{A disturbance-coordinate proposal and its score}
...
A useful distinction is between a KDM estimate of the model score and the
score of the normalized KDM density itself. The latter does have a known
center. This gives a concrete control that can be tested alongside LEDH.
Its construction uses the mixture-density derivative in the IWSG identity
with integrand $F=1$; the application as an LEDH control is a local deduction,
not an improvement theorem from the Younis paper.

\begin{proposition}[A centered KDM density-score control]
\label{prop:kdm-density-control}
Condition on a pilot mixture with fixed positive weights $\pi_j$ summing
to one, means $\mu_j$, and positive definite covariances $B_j$. Define
\begin{equation}
 m_{a,s}(z)=\sum_{j=1}^M\pi_j
   \mathcal N(z;\mu_j+a,e^{2s}B_j),\qquad
 C(z)=\left.\nabla_{(a,s)}\log m_{a,s}(z)\right|_{a=0,s=0}.
 \label{eq:kdm-density-control}
\end{equation}
For $Z\sim m_{0,0}$, $\mathbb E[C(Z)\mid\text{pilot}]=0$.
Let a baseline vector score $G$ use any coupling with $Z$ that preserves
these marginals. If a coefficient matrix $\boldsymbol{\beta}$ is fixed, or fitted on
calibration randomness independent of the evaluation randomness conditional
on the pilot, then
\begin{equation}
 \mathbb E[G-\boldsymbol{\beta} C(Z)\mid\text{pilot,calibration}]
 =\mathbb E[G\mid\text{pilot,calibration}].
\end{equation}
Assume the required second moments exist when comparing variances.
\end{proposition}
\begin{proof}
Differentiating the normalized finite Gaussian mixture under its integral
is valid for translation and scale in a neighborhood of the fixed positive
definite covariances. Thus
\[
 \mathbb E C(Z)=\int m_{0,0}(z)
  \frac{\left.\nabla_{(a,s)}m_{a,s}(z)\right|_{0,0}}{m_{0,0}(z)}\,\dd z
  =\left.\nabla_{(a,s)}\int m_{a,s}(z)\,\dd z\right|_{0,0}=0.
\]
Conditioning on the independent pilot and calibration makes $\boldsymbol{\beta}$ fixed
and leaves the evaluation mixture law unchanged. Linearity of conditional
expectation gives the second assertion.
\end{proof}

Writing $r_j(z)$ for the mixture responsibilities, the translation control
is $\sum_j r_j(z)B_j^{-1}(z-\mu_j)$, and the scale control is
$\sum_jr_j(z)\{(z-\mu_j)^\top B_j^{-1}(z-\mu_j)-d\}$.
These expressions differentiate the density at fixed $z$. They do not
differentiate a sampled point along its generating path. For example, draw
$A\sim\operatorname{Categorical}(\pi)$ independently of
$\varepsilon\sim\mathcal N(0,I)$ and set
$Z=\mu_A+\operatorname{chol}(B_A)\varepsilon$. Reusing $\varepsilon$ in
LEDH initialization preserves both marginal laws and may produce useful
covariance. Averaging several such controls retains the zero center.

The coupling matters. If instead $G$ is measurable with respect to a cloud
$\mathcal F$ and additional KDM sampling satisfies
$\mathbb E[C\mid\mathcal F]=0$, then
$\mathbb E[(G-\mathbb EG)C^\top]
=\mathbb E[(G-\mathbb EG)\mathbb E(C^\top\mid\mathcal F)]=0$.
Adding independent conditional mixture noise cannot reduce the variance of
that already-computed cloud score through a fixed linear control. Shared
estimating randomness, a correct center, and independent coefficient fitting
are therefore separate requirements. Even when the resulting covariance is
useful, Proposition~\ref{prop:kdm-density-control} preserves the baseline's
bias relative to the model score.
...
The literature audit identifies several named comparators that should be added
before the score study is treated as complete. Nemeth, Fearnhead, and
Mihaylova use kernel density approximation and Rao--Blackwellisation for a
linear-cost score and observed-information estimator
\citep{nemeth2016score}. This is the closest published precedent for the KDM
row in the present program. Its bandwidth, kernel, and regular-transition
assumptions must be carried into the experiment; a positive-bandwidth estimate
is not an exact finite-particle score. For additive complete-data terms, the
sequential smoother of Fearnhead, Wyncoll, and Tawn
\citep{fearnhead2010smoothing} and the PaRIS recursion of Olsson and Westerborn
\citep{olsson2017paris} provide explicit linear-cost alternatives to a generic
``backward pair'' label. PaRIS's number of backward draws and its mixing
conditions are part of the method, not implementation details.

A qualitatively different regular-model comparator is the coupled conditional
particle smoother of Jacob, Lindsten, and Schön
\citep{jacob2020coupled}. Rhee--Glynn debiasing of coupled CPF chains gives an
unbiased smoothing expectation under meeting and moment assumptions; the
authors explicitly note that Fisher's identity then yields an unbiased score
when the transition density is tractable. The construction has random cost and
does not make a singular or intractable DSGE transition score exact. It should
be tested as a separate debiasing arm rather than folded into a variance-
minimising linear combination of two biased estimates.

The continuous-likelihood econometric literature, including Malik--Pitt and
DeJong et al.\ \citep{malik2011continuous,dejong2013efficient}, is also a
required comparator for the motivation behind differentiable resampling. A
continuous particle likelihood can remove a resampling discontinuity while
still defining a different finite-particle measure, so its derivative requires
the same target and normalization audit used here. When a transition can be
simulated but its density is unavailable, iterated filtering
\citep{ionides2011iterated} supplies a separate artificial-parameter-noise
approximation with an explicit bias--variance--mixing trade-off. It is the
natural simulation-only comparator for the DSGE branch, but it is not an exact
finite-$N$ score.

Twisted particle filters \citep{whiteley2014twisted} and multilevel particle
filters \citep{jasra2017multilevel} belong in proposal and discretization
variance studies. Twisting preserves a likelihood through a change-of-measure
correction, while multilevel estimators telescope coupled discretization
levels; neither paper supplies a score identity. A score-level telescope or
coupling must therefore be proved before either can support an unbiased-score
claim. Finally, variational particle objectives and learned smoothers remain a
target-boundary issue: an ELBO or a learned conditional objective cannot be
reported as the observed-data score without a separate measure identity.
...
For models with regular transition densities, a reference study can check
model identity and complete initialization, establish a corrected
mixture/Fisher reference, compare selective gradient estimators where needed,
and then investigate scalable ancestor approximations or learned proposals.
For degenerate DSGE transitions, the first unresolved task is instead a
score derivation that respects the deterministic dynamics and parameter
dependence of their support. Control variates, analytic
...
Section~\ref{sec:fisher-degenerate} explains why the proposed ordinary
ancestor recursion is excluded for the degenerate DSGE target. The following
examples separate that obstruction from the support of a filtering marginal
and from possible coordinate-based constructions.
...
For models with regular transition densities, the reference question is
whether model-corrected mixture proposals and ancestor averaging reduce
model-score error at equal compute. This is a restricted comparison, not an
implementation plan for degenerate DSGE transitions. A method intended for
that target must first pass the conditional-support analysis in
Section~\ref{sec:fisher-degenerate}; more accurate results on full-rank
Gaussian models cannot remove that obstruction. Proposal
...
The subsequent accuracy study should use a linear-Gaussian model with
nondegenerate transitions and an independent Kalman score, a short
Gaussian-mixture model with evaluable transition densities whose modes can be
enumerated exactly, and a scalar nonlinear model with a regular transition
density and a numerically converged quadrature score. Kalman checks the basic
integration and complete derivatives.
...
For the regular-transition comparison, the next evidence needed is correct
...
The September 14 literature-gap audit added local technical copies and source
checks for Nemeth et al. (kernel-density/Rao--Blackwell score), Fearnhead et
al. (sequential smoothing), Jacob et al. (coupled conditional-particle
debiasing), Ionides et al. (iterated filtering), Whiteley and Lee (twisted
particle filters), and Jasra et al. (multilevel particle filters). Their
algorithms and assumptions are summarized in
\path{docs/plans/artifacts/particle-filter-score-literature-gap-audit-20260914/}.
The audit treats these methods as regular-model comparators or proposal and
discretization controls unless a separate target identity is proved. It found
no generic theorem that makes ambient Gaussian KDM or PaRIS valid for a
deterministic or singular DSGE transition.
