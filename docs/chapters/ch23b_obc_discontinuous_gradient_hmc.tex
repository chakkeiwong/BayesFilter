...
\chaptermark{Constraints and Gradient Discontinuities}
...
The hard max $i_t = \max\{\ell, i_t^\star\}$ has a derivative that jumps from
0 to 1 at the boundary; the derivative is undefined at equality. This can
produce a continuous potential with a discontinuous gradient. The validity
of a Metropolis-corrected leapfrog proposal rests on its reversible,
volume-preserving composition of momentum kicks and position drifts, with a
fixed deterministic force convention and the exact endpoint energy. The
measure-zero boundary alone does not establish accurate integration or
efficient exploration.

Boundary methods must match the type of boundary.
\citet[Section~2]{Pakman2014} solve the quadratic Hamiltonian exactly between
reflections at truncation constraints. Their Bayesian Lasso extension
(Section~4, equations~4.12--4.22) instead crosses a continuous kink with
continuous position and velocity, updating the region's quadratic dynamics.
That special construction does not supply an integrator for an arbitrary
hard-max state-space posterior. \citet[Algorithm~1 and Section~5]{Afshar2015}
incorporate reflection or refraction at a jump $\Delta U$ in potential energy,
with their volume-preservation proof covering affine partition boundaries.
When a crossing has $\Delta U=0$ and nonzero normal momentum, their refraction
rule leaves that momentum unchanged. Thus this rule alone does not correct
the integration error caused by a continuous gradient kink.
...
\item Partial centering chosen before sampling. Variationally inferred
parameterization learns the interpolation between centered and non-centered
coordinates during variational preprocessing, then fixes it for HMC
\citep[Section~5.2 and Appendix~C]{Gorinova2020}. The variational objective
does not guarantee that all posterior modes are represented.
\end{itemize}
...
\item \citet{Pakman2014} for truncated Gaussian and piecewise-quadratic
dynamics, and \citet{Afshar2015} for potential jumps across affine boundaries,
with the distinctions above for a continuous kink.
