...
def evaluate_gaussian(row, context):
    ...
    with tf.device("/GPU:0" if settings["device"] == "GPU" else "/CPU:0"):
        ...
        if proposal in ("kalman", "ukf"):
            ...
            kernel, fitted_output, fitted_diagnostics, calls = execute_iapf(
                row, settings, theta, observations, seed, context=context)
            value, score, _ = fitted_output
            diagnostics.update(fitted_diagnostics, initial_particle_count=N,
                               particle_count=fitted_diagnostics["actual_particle_count"])
