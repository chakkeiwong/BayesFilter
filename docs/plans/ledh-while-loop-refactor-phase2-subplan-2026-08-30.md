...
The master program's original design specified tangent state `[m, K, dim]` with batched K-direction evaluation. **That design is not implementable against the current model contract**, and this subplan replaces it. The correction is recorded here and was incorporated into the master program's API contract section on 2026-09-01.

`PerPointScoreModel` holds user-supplied callbacks whose tangent arguments are rank-2. Inserting a leading K axis changes the signature every model factory and test fixture implements — a breaking change to the public model contract, which the refactor contract places out of scope.

### Design Alternatives Evaluated

**(a) Rank-3 tangents `[m, K, dim]` through the callbacks.** Rejected. Breaks the model contract for every existing caller, forbidden by refactor contract.

**(b) Fold K into the point axis: reshape `[m, K, dim] → [m*K, dim]` and tile the primal.** Rejected on measured evidence. This is the batched-direction form already diagnosed in `docs/benchmarks/diagnose_direction_cost_scaling_20260830.py`: graph size 1.00× but arithmetic 3.51× for 5 directions, because tiling the primal defeats Grappler CSE. Warm eval measured 0.485 s against 0.185 s for the swept form. Rebuilding the rejected arm is not progress.

**(c) Leading-K loop state with k-looped tangent evaluation inside the while_loop body. CHOSEN.**
The loop body computes the primal once and evaluates the tangent recursion K times against it in a k-loop. Because all K tangent evaluations live in the same `tf.while_loop` body as the single shared primal, the sharing is structural — it does not depend on Grappler recognizing a common subexpression across separately traced calls.
