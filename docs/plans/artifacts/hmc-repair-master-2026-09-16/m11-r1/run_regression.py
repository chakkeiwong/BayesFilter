"""M11 fresh no-hint fits after bounded metric-boundary step nomination."""
import argparse
...
def worker(args,output):
    ...
    seed = seed_for(2026092050,"m11-bounded-boundary",args.case,args.phase,args.replication,"tuning")
    cfg = HMCKernelTuningConfig(preset="serious",metric_evidence_policy="finite_window",use_xla=True,target_scope="inference_validation",
...
def main():
    ...
    output = ROOT/f"regression-bounded-boundary-{args.phase}-{args.replication}-gpu-r1"
    if args.worker:
        try:
            worker(args,output)
    ...
    if len(costs) >= 6 or sum(r["elapsed_seconds"] for r in costs) + args.seconds > 7200.:
        raise RuntimeError("M11 attempt/worker-wall budget exhausted")
    if args.phase == "fresh":
        pilot = read(ROOT/"fresh-0-bounded-replay-gpu-r1/gpu-diagnostic-run.json")
        prepared = read(ROOT/"fresh-0-bounded-replay-gpu-r1/preparation_progress.json")
