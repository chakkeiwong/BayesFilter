#!/usr/bin/env python3
"""Run the frozen Phase 8 q=20 tempered reverse-KL cost pilot.

This runner measures GPU/XLA compile and update cost for the two declared batch
hypotheses.  It also verifies immutable temperature-chart checkpoints and the
held-out Gaussian-pullback diagnostics.  It cannot select or promote a sampler.
"""
...
PLAN = ROOT / "docs/plans/bayesfilter-ssl-lstm-q20-tempered-rkl-transport-ensemble-phase8-calibration-subplan-2026-08-29.md"
...
COMPONENT_COUNT = 2
STEADY_UPDATE_COUNT = 4
VALIDATION_SIZE = 256
OCCUPANCY_SIZE = 4096
PILOT_MEMORY_CAP_BYTES = 4 * 1024**3
SCHEMA = "bayesfilter.ssl_lstm_q20.tempered_rkl_phase8_cost_pilot.v1"
DEFAULT_GPU_ID = "0"
...
def _checkpoint_scope(
    *,
    ...
    validation_ids = (
        [
            f"phase8-cost-endpoint-b{batch_index}-c{component_index}"
            f"-beta{beta_label}-n{int(validation_size)}"
        ]
        if float(beta) == 0.0
        else [
            f"phase8-cost-validation-b{batch_index}-c{component_index}"
            f"-beta{beta_label}-n{int(validation_size)}",
            f"phase8-cost-occupancy-b{batch_index}-c{component_index}"
            f"-beta{beta_label}-n{int(validation_size)}",
        ]
    )
    return {
        ...
        "training_seed_derivation": {
            "initialization_root": [20260829, 11001],
            "preflight_roots": [[20260829, 11002], [20260829, 11003]],
            "training_root": [20260829, 21001],
            "folds": {
...
def _protocol(
    ...
    *,
    ...
    occupancy_size: int = OCCUPANCY_SIZE,
) -> Mapping[str, Any]:
    mode_value = str(mode)
    if mode_value not in {"cost-pilot", "target-localization"}:
        raise Phase8PilotError(f"unsupported Phase 8 mode: {mode_value}")
    payload = {
        "schema": SCHEMA,
        "mode": mode_value,
        ...
        "betas": [0.0, 0.5],
        "compile_updates_per_chart": 1,
        "steady_updates_per_chart": STEADY_UPDATE_COUNT,
        "validation_size": int(validation_size),
        "occupancy_size": int(occupancy_size),
        "memory_cap_bytes": PILOT_MEMORY_CAP_BYTES,
        "role": "feasibility_only_no_candidate_selection",
    }
    if mode_value == "target-localization":
        payload.update(
            {
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--mode", choices=("cost-pilot", "target-localization"), default="cost-pilot"
    )
    ...
    parser.add_argument(
        ...
        help="held-out validation bank size; 256 is the frozen default",
    )
