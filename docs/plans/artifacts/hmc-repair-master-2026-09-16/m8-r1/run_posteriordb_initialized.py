...
def worker(args,output):
    ...
    seed = seed_for(2026091840,"posteriordb-data-start",args.case,args.phase,args.replication,"tuning")
...
def main():
    ...
    output = ROOT/f"posteriordb-data-start-{tag}-{args.phase}-{args.replication}-gpu-r1"
    if args.worker:
        try:
            worker(args,output)
