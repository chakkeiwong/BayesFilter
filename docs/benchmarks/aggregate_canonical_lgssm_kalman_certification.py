#!/usr/bin/env python3
"""Aggregate one frozen paired canonical LGSSM Kalman certification rung."""
...
from docs.benchmarks import run_canonical_lgssm_kalman_certification_arm as arm_spec
from scripts.filtering_value_gradient_benchmark_generate_p8_datasets import (
    _lgssm_dataset,
)


CAMPAIGN_ID = arm_spec.CAMPAIGN_ID
...
def _kalman(time_steps: int) -> tuple[float, list[float]]:
    observations = tf.convert_to_tensor(
        _lgssm_dataset(arm_spec.DATASET_SEED)["observations"][:time_steps],
        tf.float64,
    )
    theta = tf.constant(arm_spec.THETA, tf.float64)
...
def main() -> None:
    ...
    time_steps = int(contract_e["time_steps"])
    if time_steps not in arm_spec.ESTIMATOR_SEEDS_BY_HORIZON:
        raise ValueError(f"unplanned time_steps: {time_steps}")
    if contract_e["estimator_seeds"] != list(
        arm_spec.ESTIMATOR_SEEDS_BY_HORIZON[time_steps]
    ):
        raise ValueError("unplanned estimator seed block")
    if int(contract_e["num_particles"]) != arm_spec.NUM_PARTICLES:
        raise ValueError("unplanned particle count")
    ...
    kalman_value, kalman_score = _kalman(time_steps)
    ...
    payload = {
        "schema_version": "bayesfilter.canonical_lgssm_kalman_certification.v1",
        ...
        "nonclaims": [
            ...
            "not XLA production-loop certification",
        ],
    }
