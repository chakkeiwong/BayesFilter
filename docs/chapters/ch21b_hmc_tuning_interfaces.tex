...
Bounding $s(v)$ alone does not bound the residual stiffness: if $s$ remains
bounded as $v\to-\infty$, the latent conditional precision
$e^{2s(v)-v}$ diverges. A useful diagnostic partial-whitening family instead
sets $s(v)=v/2+\delta(v)$ with $\delta(v)=a\tanh(v/(2\sigma))$.
For $S=z_1^2+z_2^2$, its log determinant is
$\log\sigma+v+2\delta(v)$, and substitution in the normalized funnel
density yields
\[
 U(z)=\tfrac12 z_0^2+\tfrac12 e^{2\delta(\sigma z_0)}S
       -2\delta(\sigma z_0)+\tfrac32\log(2\pi).
\]
The child-coordinate curvature lies in $[e^{-2a},e^{2a}]$ for $a\geq0$;
the zero-amplitude limit recovers the exact control. The entire Hessian is
still unbounded because derivatives with respect to $z_0$ contain $S$.
The validation fixture uses $\sigma=3$ and $a=1/2$ as an explicit development
hypothesis, not a learned-map or sampling default. Even the exact latent
Gaussian control can require many draws for precise means of the model's
heavy-tailed children.
...
and verification counts. The shared execution configuration can replace
these allocations and supply an explicit \path{pilot_num_results}. Pilots never
...
Exact-score TFP bindings optionally share a compiled runner across leapfrog
counts at each chunk size with
\path{execution_config.reuse_leapfrog_graphs=True}; the default is false.
Each cache belongs to one frozen target and geometry with fixed chain topology,
trace policy and backend. Independent scalar chains keep their original seed
folding. The complete execution artifact records this option and restart
preserves it. Within the same source snapshot it does not change numerical
seeds; a source change still changes scope identity and can change seeds.
The proposal-field route rejects the option. Reuse can reduce first-call
compilation while increasing warmed execution cost, so assess its cost on
complete fits. Candidate qualification and posterior diagnostics are unchanged.
...
An analytic posterior permits an additional direct check of fitted outputs.
For a zero-mean normal prior with variance $\tau^2$ and independent observations with
known variance $\sigma^2$, the conditional posterior has variance
$s_y^2=(\tau^{-2}+n\sigma^{-2})^{-1}$ and mean
$\mu_y=s_y^2\sum_i y_i/\sigma^2$. If a predetermined output $X_j$ from each
independent complete fit has that conditional posterior law, then
$Z_j=(X_j-\mu_y)/s_y$ are independent standard normals. Consequently,
$\sum_j Z_j/\sqrt{J}$ is standard normal and $\sum_j Z_j^2$ has a
$\chi^2_J$ distribution. Two two-sided tests, each at half the declared size,
check location and second moment. The union bound controls their combined
size under this correct-output null. The second check can expose errors that
leave the mean unchanged.

The diagnostic fixes the member, chain and terminal-draw rule before seeing
posterior outcomes. Missing, capped or unqualified fits make its planned
experiment incomplete; they are never counted as successful defect detections.
Shared-stream controls cannot count as independent fits. The normal laws above
describe the hypothesis under test. Adaptive stopping does not prove that a
finite HMC endpoint follows them, and these tests do not establish coverage of
intervals for stopped posterior means. Arithmetic adversaries and activated
whole-fit defects answer different questions and are reported separately.
...
A complementary diagnostic uses all retained draws from one qualified fit to
test its posterior mean. With $\theta\sim N(0,\tau^2)$ and conditionally
independent observations $y_i\mid\theta\sim N(\theta,\sigma^2)$, completing
the square in the prior times likelihood gives the independent reference
\[
 s_*^2=(\tau^{-2}+n\sigma^{-2})^{-1},\qquad
 m_*=s_*^2\sigma^{-2}\sum_{i=1}^n y_i.
\]
The validation engine \path{reference_mean} compares
$|\widehat m-m_*|/\widehat{\mathrm{MCSE}}$ with the declared normal critical
value. It recomputes lugsail MCSE independently from the archived model draws;
the reference never enters tuning. The normal approximation defines the alarm,
but its achieved false-alarm and detection rates require independent complete
fits with fresh data. A capped, missing or unqualified fit is a possible alarm
in the conservative null rate and a nondetection in the defect rate. This
answers a location-error question distinct from endpoint invariance and SBC;
arithmetic and execution tests alone do not establish its power.
...
Use the validation CLI's \path{run} command with \path{--profile-execution} for paired
comparisons: it preserves the numerical design and random streams. With
isolated fits, the profiler runs in the numerical child after framework
imports, with setup time recorded separately. It records Python call boundaries
without native builtins tracing, avoiding an observed Python 3.13/TensorFlow
interaction that loses outer calls. This is host cost attribution, not device
kernel timing. The run index records profile availability. A completed fit is
never repeated to replace a missing profile, and profiling failures do not
replace numerical outcomes.
...
denominator. Numerical search, accuracy, stopping and reference-mean designs enable this
...
\input{generated/inference_validation_coverage}
