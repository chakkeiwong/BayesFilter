...
Coefficient or audit discrepancy against SGQF diagnoses representation error;
it does not alone determine whether a TT is useful for filtering.  When the TT
supplies a conditional particle proposal, the downstream quantities are the
importance-corrected filtering moments and log evidence, particle effective
sample size, weight concentration, resampling and ancestor counts.  These are
compared with an independent reference and simple proposals.  SGQF supplies a
Gaussian-joint guide and comparator, so a worse fit discrepancy can motivate a
repair without precluding that filtering comparison.  Particle-proposal
evidence must be distinguished from validation of the uncorrected retained TT
density and Gram normalizer developed in this chapter: importance corrections
change the computation being assessed.
...
\begin{equation}
\begin{aligned}
  a_t(z_1)&=\frac{196}{45}-\frac89z_1+\frac{26}{9}z_1^2,\\
  \dot a_t(z_1)&=-8+\frac{256}{9}z_1-\frac{80}{9}z_1^2.
\end{aligned}
...
Chapter~\ref{ch:bf-highdim-fixed-branch-same-scalar} must use the same
retained/reference/Jacobian ownership story as this certificate.  The saved
evaluator consumes retained reference coordinates, the physical-to-reference
conversion is owned by the target-construction path, and the route-audit note
remains a convention boundary rather than a theorem.
