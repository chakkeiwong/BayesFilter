"""Offline M12 integrity, attempt, test, and worker-wall reconciliation."""
...
def main():
    ...
    sys.path.insert(0, str(ROOT / "source-gpu-r1"))
    ...
    opening_path = ROOT.parent / "m11-r1/reconciliation-terminal.json"
    opening = read(opening_path)["budget_seconds"]
    expected = ["fresh-2-trajectory-replay-gpu-r1", "fresh-2-trajectory-replay-gpu-r2",
                "fresh-2-sequential-probe-replay-gpu-r1",
                "fresh-0-trajectory-replay-gpu-r1", "inactive-option-parity-gpu-r1",
                *(f"regression-sequential-probe-fresh-{i}-gpu-r1" for i in range(3))]
    ...
    for path in sorted(ROOT.glob("*-run.json")) + sorted(ROOT.glob("*/gpu-diagnostic-run.json")):
        ...
        diagnostic_path = path.parent / "trajectory_diagnostics/instrumentation.json"
        if diagnostic_path.exists():
            diagnostic = read(diagnostic_path)
            if sha(path.parent / "trajectory_harness.py.txt") != diagnostic["script_sha256"]:
                invalid.append(str(path) + ": trajectory instrumentation checksum")
        if record["returncode"]:
            ...
            numerical_failure = any(
                str(event.get("details", {}).get("diagnostics", {}).get("hmc_error_message", ""))
                .startswith("operational warmup produced a nonfinite trace") for event in events)
            harness_failure = path.parent.name == "fresh-2-trajectory-replay-gpu-r1" and any(
                "Tensor conversion requested dtype float64 for Tensor with dtype bool" in str(
                    e.get("details", {}).get("diagnostics", {}).get("hmc_error_message", "")) for e in events)
            if not numerical_failure and not harness_failure:
                invalid.append(str(path) + ": unclassified execution failure")
            attempt["failure_classification"] = "preparation_numerical_veto" if numerical_failure else (
                "diagnostic_harness_dtype_error" if harness_failure else "unclassified")
            attempt["eligible_numerical_failure_evidence"] = numerical_failure
    ...
    diagnosis = read(ROOT / "trajectory-assessment.json")
    final_checks = read(ROOT / "final-checks-assessment.json")
    parity = final_checks["inactive_option_parity"]
    if not all(parity[key] for key in ("all_latent_tensors_equal", "final_factor_equal", "final_epsilon_equal")):
        invalid.append("inactive-option numerical parity failed")
    if not final_checks["fresh_failure"]["completed_windows_equal_except_runtime"]:
        invalid.append("fresh failure replay did not reproduce completed windows")
    for row in diagnosis["transitions"] + final_checks["fresh_failure"]["trajectories"]:
        if sha(REPO / row["path"]) != row["sha256"]:
            invalid.append("trajectory assessment checksum mismatch: " + row["path"])
    ...
    exceeded = (tranche["cpu_reference"] > 5000. or tranche["gpu"] > 9000.
                or gpu_attempts > 8 or any(v < 0 for v in remaining.values()))
    summary = {"schema": "bayesfilter.hmc_m12_reconciliation.v1", "terminal": True,
        ...
        "diagnostic_assessments": {
            "original_failure": sha(ROOT / "trajectory-assessment.json"),
            "final_checks": sha(ROOT / "final-checks-assessment.json"),
            "fit_summary": sha(ROOT / "fit-summary.json"),
        },
        "budget_seconds": {"authorized": opening["authorized"], "opening_charged": opening["cumulative_charged"],
            "m12_measured": costs, "overhead": overhead, "m12_total": tranche,
