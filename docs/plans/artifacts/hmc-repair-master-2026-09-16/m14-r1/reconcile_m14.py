...
def main():
    ...
    sys.path.insert(0,str(ROOT/"source-schedule-r1"))
    ...
    matrix=read(ROOT/"public-model-matrix-continuation-r1/run_index.json")
    matrix_status={}
    ...
    for p in sorted([*ROOT.glob("source*/source_snapshot.json"), ROOT.parent/"m13-r1/source-final-r2/source_snapshot.json", ROOT.parent/"m13-r1/source-gpu-r1/source_snapshot.json"]):
    ...
    for p in sorted(ROOT.glob("*/tuning/candidate_set_result.json"))+sorted(ROOT.glob("public-model-matrix-continuation-r1/*/replication-*/tuning/candidate_set_result.json"))+sorted(ROOT.glob("repaired-public-gpu-r1/*/replication-*/tuning/candidate_set_result.json")):
    ...
    public=read(ROOT/"repaired-public-gpu-r1/run_index.json")
    for name,job in public["jobs"].items():
        matrix_status[name]=job["status"]
        if job["status"] not in ("complete","failed","timed_out"): invalid.append(name+": unfinished")
        for a in job["attempts"]:
            costs["gpu"]+=a["elapsed_seconds"]
            attempts.append({"path":a["log"],"seconds":a["elapsed_seconds"],"device":"gpu","status":a["status"]})
    parity={"default_numerical_route_unchanged":"M13 disabled-option parity plus M14 unchanged-default schedule regression"}
    ...
    opening=read(ROOT.parent/"m13-r1/reconciliation-terminal.json")["budget_seconds"]
    ...
    budget={"authorized":opening["authorized"],"opening_charged":opening["cumulative_charged"],"measured":costs,"overhead":overhead,"phase_total":total,"cumulative_charged":charged,"remaining":remaining,"exceeded":total["cpu_reference"]>12000 or total["gpu"]>7500 or min(remaining.values())<0}
    d={"schema":"bayesfilter.hmc_m14_reconciliation.v1","terminal":True,"source_snapshots":snapshots,"attempts":attempts,"candidate_inventories":inventories,"numerical_receipts_checked":receipts,"tensor_checksums_checked":tensors,"tests":{s:sum(v["status"]==s for v in latest.values()) for s in ("passed","failed","skipped")},"test_details":latest,"inactive_option_parity":parity,"matrix_status":matrix_status,"invalid_artifacts":invalid,"outstanding_workers":[],"budget_seconds":budget,"manifest":{"command":sys.argv,"environment":sys.executable,"script_sha256":sha(Path(__file__)),"elapsed_seconds":time.monotonic()-started,"gpu_intentionally_hidden":True},"interpretation":"Integrity and accounting only; resource continuations close execution while posterior caps remain failures."}
